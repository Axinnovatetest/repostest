...
namespace Infrastructure.Data.Access.Tables.CRP
{
	public static class __crp_FertigungsnummerAccess
	{
		...
		public static int GetMaxFertigungsnummer()
		{
			using(var sqlConnection = new SqlConnection(Settings.ConnectionString))
			{
				sqlConnection.Open();
				string query = "SELECT ISNULL(MAX(ISNULL(Fertigungsnummer,0)),0) FROM [__crp_Fertigungsnummer]";
				var sqlCommand = new SqlCommand(query, sqlConnection);

				return int.TryParse(DbExecution.ExecuteScalar(sqlCommand)?.ToString(), out var _x) ? _x : 0;
			}
		}
		...
		public static int DeleteByFertigungsnummer(int fertigungsnummer, SqlConnection connection, SqlTransaction transaction)
		{
		...
		public static int DeleteByFertigungsnummers(List<int> fertigungsnummers)
		{
			string query = $"DELETE FROM [__crp_Fertigungsnummer] WHERE [Fertigungsnummer] IN ({string.Join(",", fertigungsnummers)})";
			using(var connection = new SqlConnection(Settings.ConnectionString))
			using(var sqlCommand = new SqlCommand(query, connection))
			{
				connection.Open();
		...
		public static int DeleteByAngebotNr(int angebotNr, SqlConnection connection, SqlTransaction transaction)
		{
