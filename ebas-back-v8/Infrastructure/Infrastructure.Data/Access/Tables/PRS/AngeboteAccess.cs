	...
	public class AngeboteAccess
	{
		...
		public static int GetCustomerOrdersByNeuOrderCount(bool? neuOrder, IEnumerable<int> customerNrs)
		{
			...
			using(var sqlConnection = new SqlConnection(Settings.ConnectionString))
			{
				...
				var sqlCommand = new SqlCommand(query, sqlConnection);
				sqlCommand.Parameters.AddWithValue("typ", TYP_CONFIRMATION);
				return int.TryParse(DbExecution.ExecuteScalar(sqlCommand).ToString(), out int _x) ? _x : 0;
			}
		}
		...
		public static Infrastructure.Data.Entities.Tables.PRS.AngeboteEntity GetByAngebotNr(int angebotNr)
		{
			var dataTable = new DataTable();
		...
		public static List<Tuple<int, string>> Get_TupleKundenNrBezug_Like_Bezug(string searchText)
		{
			...
			searchText = searchText.SanitizeForSql();
			...
			Data.Access.Settings.PaginModel paging, bool onlyExpiredRahmens = false)
		{
			...
			using(var sqlConnection = new SqlConnection(Access.Settings.ConnectionString))
			{
				...
				using(var sqlCommand = new SqlCommand())
				{
					...
					{
						query += $" {(isFirstCondition ? " WHERE " : " AND ")} a.[Angebot-Nr] = '{(int.TryParse(angebotNr.Trim(), out var ang) ? ang : -1)}' ";
						isFirstCondition = false;
					}
		...
		//
		public static Infrastructure.Data.Entities.Tables.PRS.AngeboteEntity GetByAngebotNr(int projektNr, string typ)
		{
			var dataTable = new DataTable();
			using(var sqlConnection = new SqlConnection(Settings.ConnectionString))
			{
				...
				string query = "SELECT * FROM [Angebote] WHERE [Projekt-Nr]=@projektNr AND [Typ]=@typ";
				var sqlCommand = new SqlCommand(query, sqlConnection);
				sqlCommand.Parameters.AddWithValue("Id", projektNr);
				sqlCommand.Parameters.AddWithValue("typ", typ);
		...
		public static int CloseRaWithEmptyQuantities(IEnumerable<int> positionIds, SqlConnection connection, SqlTransaction transaction)
		{
			if(positionIds == null || positionIds.Count() <= 0)
				return 0;

			string query = $"UPDATE [Angebote] SET [Erledigt]=1 WHERE [Nr] IN ({string.Join(", ", positionIds)})";
			var sqlCommand = new SqlCommand(query, connection, transaction);
...
{string.Join(",", lsIds)}
	...
	WHERE r.Nr_lie is null";
		...
		////
		//public static List<Infrastructure.Data.Entities.Tables.PRS.AngeboteEntity> GetLandFromAddressen(string kundenNr, SqlConnection connection, SqlTransaction transaction)
		//{
		//	var dataTable = new DataTable();
		//	string query = @"
  //          SELECT a.Land
  //          FROM [ERP_WEB].[dbo].[Angebote] an
  //          INNER JOIN [ERP_WEB].[dbo].[adressen] a ON an.[Kunden-Nr] = a.Nr
  //          WHERE an.[Kunden-Nr] = @KundenNr
		//	";
		//	var sqlCommand = new SqlCommand(query, connection, transaction);
		//	sqlCommand.Parameters.AddWithValue("@KundenNr", kundenNr);
		//	DbExecution.Fill(sqlCommand, dataTable);
		//	return dataTable.Rows.Cast<DataRow>()
		//	.Select(row => new Infrastructure.Data.Entities.Tables.PRS.AngeboteEntity(row, true))
		//	.ToList();
		//}
		...
		public static int GetBezugByRahmenCount(string bezug)
		{
			try
			{
				var sqlConnection = new SqlConnection(Infrastructure.Data.Access.Settings.ConnectionString);
				...
				string query = @"select count(*) as OrdersCount from Angebote a inner join __CTS_AngeboteBlanketExtension e
                                on a.Nr=e.AngeboteNr where a.Bezug=@bezug and a.Typ='Rahmenauftrag' and e.BlanketTypeId=1";
