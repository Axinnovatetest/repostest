...
namespace Infrastructure.Data.Access.Tables.CTS
{
	public class __E_rechnung_CreatedAccess
	{
		...
		public static List<Infrastructure.Data.Entities.Tables.CTS.__E_rechnung_CreatedEntity> GetSentWithoutAttachment_temp()
		{
			var dataTable = new DataTable();
			using(var sqlConnection = new SqlConnection(Settings.ConnectionString))
			{
				sqlConnection.Open();
				string query = "SELECT * FROM [__E_rechnung_Created] where CONVERT(date,SentTime)='06-11-2025' and CONVERT(TIME,SentTime)>'16:00:00'";
				var sqlCommand = new SqlCommand(query, sqlConnection);
