...
namespace Infrastructure.Data.Access.Tables.CTS
{
	public class ErrorAccess
	{
		...
		public static int GetCountBySendersDuns(List<string> sendersIds, bool? isValidated = null, bool includeNoCustomer=true)
		{
			if(sendersIds == null || sendersIds.Count <= 0)
				return 0;
			...
			{
				sqlConnection.Open();
				string query = $"SELECT COUNT(*) FROM [__EDI_DLF_Error] WHERE ISNULL([BuyerDuns],'') IN ({string.Join(", ", sendersIds.Select(x => $"'{x.Trim()}'"))}) {(!isValidated.HasValue ? "" : $" AND ISNULL([Validated],0)={(isValidated.Value ? 1 : 0)}")}";
				var sqlCommand = new SqlCommand(query, sqlConnection);
