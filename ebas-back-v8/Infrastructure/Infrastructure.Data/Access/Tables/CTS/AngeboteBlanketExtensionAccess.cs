...
namespace Infrastructure.Data.Access.Tables.CTS
{
	public class AngeboteBlanketExtensionAccess
	{
		...
		public static int UpdateBlaqnketStatus(IEnumerable<int> raIds, int statusId, string statusName, SqlConnection connection, SqlTransaction transaction)
		{
			...
			{
				sqlConnection.Open();
				string query = $"UPDATE [__CTS_AngeboteBlanketExtension] SET [StatusId]=@statusId, [StatusName]=@statusName WHERE [AngeboteNr] IN ({string.Join(", ", raIds)})";
				var sqlCommand = new SqlCommand(query, sqlConnection);

				sqlCommand.Parameters.AddWithValue("statusId", statusId);
				sqlCommand.Parameters.AddWithValue("statusName", statusName ?? "");
