...
namespace Infrastructure.Data.Access.Tables.WPL
{
	public class StandardOperationAccess
	{
		...
		public static int Insert(Infrastructure.Data.Entities.Tables.WPL.StandardOperationEntity element)
		{
			...
			string query = "INSERT INTO StandardOperation (Code,Creation_Date,Creation_User_Id,Delete_Date,Delete_User_Id,Operation_Value_Adding,RelationOperationTime,Name,Is_Archived,Last_Edit_User_Id,Last_Edit_Date) " +
										" VALUES (@Code,@Creation_Date,@Creation_User_Id,@Delete_Date,@Delete_User_Id,@Operation_Value_Adding,@RelationOperationTime,@Name,@IsArchived,@Last_Edit_User_Id,@Last_Edit_Date);";
			query += "SELECT SCOPE_IDENTITY();";

			var sqlCommand = new SqlCommand(query, sqlConnection);
			sqlCommand.Parameters.AddWithValue("Code", element.Code == null ? (object)DBNull.Value : element.Code);
		...
		public static int Update(Infrastructure.Data.Entities.Tables.WPL.StandardOperationEntity element)
		{
			...
			string query = "UPDATE StandardOperation SET Code=@Code,Creation_Date=@Creation_Date,Creation_User_Id=@Creation_User_Id,Delete_Date=@Delete_Date,Delete_User_Id=@Delete_User_Id," +
		...
		private static int update(List<Infrastructure.Data.Entities.Tables.WPL.StandardOperationEntity> elements)
		{
			if(elements != null && elements.Count > 0)
			{
				...
				foreach(var element in elements)
				{
					...
					sqlCommand.Parameters.AddWithValue("Code" + i, element.Code == null ? (object)DBNull.Value : element.Code);
		...
		public static Infrastructure.Data.Entities.Tables.WPL.StandardOperationEntity GetByCode(string code)
		{
			...
			string query = "SELECT * FROM StandardOperation WHERE [Code]=@code AND Is_Archived = @IsArchived";
		...
		public static Infrastructure.Data.Entities.Tables.WPL.StandardOperationEntity GetByName(int id, string name)
		{
			if(string.IsNullOrEmpty(name))
			{
				return null;
			...
			string query = "SELECT * FROM StandardOperation WHERE Name=@Name AND Is_Archived = @IsArchived AND [Id]<>@id";
		...
		public static Infrastructure.Data.Entities.Tables.WPL.StandardOperationEntity GetByCode(int id, string code)
		{
			if(string.IsNullOrEmpty(code))
			{
				return null;
			...
			string query = "SELECT * FROM StandardOperation WHERE [Code]=@code AND Is_Archived = @IsArchived AND [Id]<>@id";
