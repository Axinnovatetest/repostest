...
namespace Infrastructure.Data.Access.Tables.WPL
{
	...
	public class WorkAreaAccess
	{
		...
		//
		public static List<Infrastructure.Data.Entities.Tables.WPL.WorkAreaEntity> GetList(bool IsArchived = false)
		{
			var dataTable = new DataTable();
			using(var sqlConnection = new SqlConnection(Infrastructure.Data.Access.Settings.ConnectionString))
			{
				sqlConnection.Open();
				string query = @"select wa.*, c.Name as CountryName ,h.Name as HallName, d.Name as DepartmentName  from Work_Area wa
inner join Countries c on wa.Country_Id = c.Id
inner join Hall h on wa.Hall_Id= h.Id
inner join Departement d on wa.Department_Id = d.Id WHERE wa.Is_Archived=@IsArchived";
				var sqlCommand = new SqlCommand(query, sqlConnection);
				sqlCommand.Parameters.AddWithValue("IsArchived", GetArchived);
			...
			if(dataTable.Rows.Count > 0)
			{
				return dataTable.Rows.Cast<DataRow>().Select(x => new Infrastructure.Data.Entities.Tables.WPL.WorkAreaEntity(x, true)).ToList();
			}
			else
			{
				return new List<Infrastructure.Data.Entities.Tables.WPL.WorkAreaEntity>();
