...
namespace Infrastructure.Data.Access.Tables.BSD
{
	...
	public class CountryISOAccess
	{
		...
		public static string? GetAlpha2Code(string? countryName)
		{
			...
			string normalized = countryName.Trim().ToLower();
			...
			using(var connection = new SqlConnection(Settings.ConnectionString))
			{
				...
				var query = @"
				SELECT TOP 1 alpha2Code
				FROM [__BSD_CountryISO]
				WHERE LOWER(Name) = @country
			    OR LOWER(NativeName) = @country";

				using(var sqlCommand = new SqlCommand(query, connection))
				{
					sqlCommand.Parameters.AddWithValue("@country", normalized);
					var result = DbExecution.ExecuteScalar(sqlCommand);
