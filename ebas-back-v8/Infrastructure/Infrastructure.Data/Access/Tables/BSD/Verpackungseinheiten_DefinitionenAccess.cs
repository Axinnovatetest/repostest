...
namespace Infrastructure.Data.Access.Tables.BSD
{
	...
	public class Verpackungseinheiten_DefinitionenAccess
	{
		...
		public static Infrastructure.Data.Entities.Tables.BSD.Verpackungseinheiten_DefinitionenEntity GetByNummer(string nummer)
		{
			nummer = nummer ?? "";
			var dataTable = new DataTable();
			using(var sqlConnection = new SqlConnection(Settings.ConnectionString))
			{
				sqlConnection.Open();
				string query = "SELECT * FROM [Verpackungseinheiten Definitionen] WHERE [Artikelnummer]=@nummer";
				var sqlCommand = new SqlCommand(query, sqlConnection);
