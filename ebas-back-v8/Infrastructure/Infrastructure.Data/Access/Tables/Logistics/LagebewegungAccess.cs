...
{
	public class LagebewegungAccess
	{
		...
		public static int AddlistUmbuchungPositionWithTransaction(Entities.Tables.Logistics.LagerbewegungDetailsEntity element, SqlConnection connection, SqlTransaction transaction)
		{
			int response = -1;
		...
		public static int DeletelistPositionByIdLagerbewegungWithTransaction(long idLagerbewegung, SqlConnection connection, SqlTransaction transaction)
		{
			...
			var results = DbExecution.ExecuteNonQuery(sqlCommand);
			response = results;
			// }

			return response;
		}
		public static int AddlistEntnahmePositionWithTransaction(Entities.Tables.Logistics.LagerbewegungDetailsEntity element, SqlConnection connection, SqlTransaction transaction)
		{
			int response = -1;
		...
		public static int AddZugangPositionWithTransaction(Entities.Tables.Logistics.LagerbewegungDetailsEntity element, SqlConnection connection, SqlTransaction transaction)
		{
			int response = -1;
			...
			using(var sqlConnection = new SqlConnection(Settings.ConnectionString))
			{
				...
				else if(type == 2)
				{
					query = "SELECT Lagerbewegungen.Datum as Datum" +
							  ...
							  " and Lagerbewegungen_Artikel.Anzahl<>0"+
							  option1 +
							 " order by Datum";
				}
