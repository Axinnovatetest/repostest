namespace Infrastructure.Data.Access.Joins.CTS
{
	public class Divers
	{
		...
		public static int GetMaxDocumentNumberRaPurchase(bool isPurchase = false)
		{
			using(var sqlConnection = new SqlConnection(Access.Settings.ConnectionString))
			{
				sqlConnection.Open();

				string query = $@"SELECT MAX(TRY_CAST(SUBSTRING([Bezug], CHARINDEX('-', [Bezug]) + 1, LEN([Bezug])) AS INT)) AS MaxBezugNum FROM [Angebote] r
								JOIN [__CTS_AngeboteBlanketExtension] e on e.AngeboteNr=r.Nr  WHERE [Bezug] LIKE 'RA-%' AND typ ='rahmenauftrag' AND e.BlanketTypeId={(isPurchase?"1":"0")}";
