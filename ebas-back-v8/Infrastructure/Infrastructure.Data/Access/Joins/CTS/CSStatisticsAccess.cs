...
";
		...
		public static List<Infrastructure.Data.Entities.Joins.CTS.CapacityFCEntity> GetCapacityStatus_FC(int articleId, DateTime? dateFrom, DateTime? dateTo)
		{
			var dataTable = new DataTable();
			using(var sqlConnection = new SqlConnection(Settings.ConnectionString))
			{
				...
				string query = $@"WITH LastVersion as (
									SELECT [TypeId], [kundennummer], MAX(Version) [Version] from [Forecasts]
									GROUP BY [TypeId], [kundennummer])
									SELECT ArtikelNr as Artikel_Nr, p.Artikelnummer, 
									f.Id, p.Id PositionId, f.kunden CustomerName, p.Datum PositionDate, SUM(Menge) Anzahl
									FROM [LastVersion] l JOIN [Forecasts] f ON l.kundennummer=f.kundennummer AND l.TypeId=f.TypeId AND l.[Version]=f.[Version]
									JOIN [ForecastsPosition] p on p.IdForcast=f.Id 
									WHERE p.Datum IS NOT NULL AND ISNULL(p.IsOrdered,0)=0 AND [ArtikelNr]=@articleId
									{(dateTo.HasValue ? $" AND p.Datum <= '{dateTo.Value.ToString("yyyyMMdd")}'" : "")} 
									{(dateFrom.HasValue ? $" AND p.Datum >= '{dateFrom.Value.ToString("yyyyMMdd")}'" : "")}
									GROUP BY [ArtikelNr], p.Artikelnummer, f.Id, p.Id, f.kunden, p.Datum
									HAVING SUM(Menge)<>0";
				var sqlCommand = new SqlCommand(query, sqlConnection);
				sqlCommand.CommandTimeout = 180;
				sqlCommand.Parameters.AddWithValue("articleId", articleId);
			...
			if(dataTable.Rows.Count > 0)
			{
				return dataTable.Rows.Cast<DataRow>().Select(x => new Infrastructure.Data.Entities.Joins.CTS.CapacityFCEntity(x)).ToList();
			}
			else
			{
				return new List<Infrastructure.Data.Entities.Joins.CTS.CapacityFCEntity>();
			}
		}
