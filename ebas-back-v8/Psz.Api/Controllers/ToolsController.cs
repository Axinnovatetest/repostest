	...
	public class ToolsController: ControllerBase
	{
		...
		public IActionResult GetApiCalls(bool excludeIds = true, bool file = true, string forwardEmail = "")
		{
			try
			{
				...
				{
					if(string.IsNullOrWhiteSpace(currentUser?.Email) || !string.Equals(currentUser.Email, forwardEmail, StringComparison.OrdinalIgnoreCase))
					{
						return Forbid();
					}
		...
		public IActionResult resendInvoices()
		{
			try
			{
				...
				{/*1299780, 1299843, 1299903
					...
					/*2026-03-24 - resend for Horsch all from 2026-02-03 - Lang
					1475863,1475864,1475865,1475866,1475867,1475868,1475869,1475870,1475871,1475872,
					1475873,1475874,1475875,1475876,1475877,1475878,1475879,1475880,1475881,1475882,
					1475883,1475884,1475885,1475886,1475887,1475888,1475889,1475890,1475891,1475892,
					1475893,1475894,1475895,1475896,1475897,1475898,1475899,1475900,1475901,1475902,
					1475903,1475904,1475905,1475906,1475907,1475908,1475909,1475910,1475911,1475912,
					1475913,1475914,1475915,1475916,1475917,1475918,1475919,1475920,1475921,1475922,
					1475923,1475924,1475925,1475926,1475927,1475928,1475929,1475930,1475931,1475932,
					1475933,1475934,1475935,1475936,1475937,1475938,1475939,1475940,1475941,1475942,
					1475943,1475944,1475945,1475946,1475947,1475948,1475949,1475950,1475951,1475952,
					1475953,1475954,1475955,1475956,1475957,1475958,1475959,1475960,1475961,1475962,
					1475963,1475964,1475965,1475966,1475967,1475968,1475969,1475970,1475971,1475972,
					1475973,1475974,1475975,1475976,1475977,1475978,1475979,1475980,1475981,1475982,
					1475983,1475984,1475859,1475860,1475861,1475862 */
				};
				...
				if(failed.Count > 0)
				{
					System.IO.File.WriteAllLines($@"C:\inetpub\wwwroot\EITECH\LIVE\API\vtest\Logs\failed{DateTime.Now.ToString("yyyyMMddHHmmss")}.txt", failed);
				}
				if(succeeded.Count > 0)
				{
					System.IO.File.WriteAllLines($@"C:\inetpub\wwwroot\EITECH\LIVE\API\vtest\Logs\succeeded{DateTime.Now.ToString("yyyyMMddHHmmss")}.txt", succeeded);
				}
				return Ok();
			} catch(Exception e)
			{
		...
		public async Task<IActionResult> SaveLogsToDB(FeedbackLogRequestModel request)
		{
			try
			{
				LogParser logParser = new LogParser(this._configuration["LogFolderPath"], this._configuration["LogDateTimeRange"], this._configuration.GetSection("ExcludeUserIds").Get<List<int>>(), this._configuration.GetSection("Email").Get<Infrastructure.Services.Email.EmailParamtersModel>());
		...
		[HttpPost]
		[AllowAnonymous]
		public IActionResult LogClientSlow([FromBody] ClientTelemetryModel data)
		{
			try
			{
				var slowClientThreshold = this._configuration
					?.GetSection("Telemetry:Frontend")
					?.Get<TelemetrySettings.BaseSettings>()
					?.SlowThresholdMs ?? 3000;
				if(slowClientThreshold < data.Elapsed)
				{
					var user = this.GetCurrentUser();
					Infrastructure.Services.Logging.Logger.LogWarning(
						$"{(user is null ? "" : $"{user.Id}|")}SLOW CLIENT|{data.Url} " +
						$"| {data.Elapsed / 1000} seconds | CorrelationId: {data.CorrelationId}"
					);
				}
		...
		[HttpGet]
		[AllowAnonymous]
		public IActionResult RebuildRequiredCapacitiesForDefinedWorkPlan(bool cleanSlate=true)
		{
			var logPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "logs", $"crp_debug_{DateTime.Now:yyyyMMdd}.txt");
			...
			try
			{
				Log("=== CRP Process Started ===");
				Log($"Parameters: cleanSlate={cleanSlate}");

				if(cleanSlate)
				{
					Log("cleanSlate=true → Calling ArchiveAll(-2)...");
					Infrastructure.Data.Access.Tables.MTM.CapacityRequiredAccess.ArchiveAll(-2);
					Log("ArchiveAll(-2) completed.");
				}

				Log("Fetching WorkPlan entries (active only)...");
				var workPlans = Infrastructure.Data.Access.Tables.WPL.WorkPlanAccess.Get(true)
					?? new List<Infrastructure.Data.Entities.Tables.WPL.WorkPlanEntity>();
				var articleIds = workPlans.Select(x => x.ArticleId).ToList();
				Log($"WorkPlan fetch complete. Count={workPlans.Count}, ArticleIds=[{string.Join(", ", articleIds)}]");

				Log("Fetching open Fertigungs by ArticleId...");
				var openFas = Infrastructure.Data.Access.Tables.PRS.FertigungAccess.GetOpenByArticle(articleIds);
				Log($"Open FAs fetched. Count={openFas?.Count ?? 0}");

				if(openFas?.Count > 0)
				{
					Log("Processing each open FA via SpecialHelper.Perform...");
					openFas.ForEach(fa =>
					{
						Log($"  → Performing FA: Id={fa.Fertigungsnummer}");  
						Psz.Core.CRP.Helpers.SpecialHelper.Perform(fa);
						Log($"  ✓ FA Id={fa.Fertigungsnummer} processed successfully.");
					});
					Log("All FAs processed.");
				}
				else
				{
					Log("No open FAs found. Nothing to process.");
				}

				Log("=== CRP Process Completed Successfully ===");
				return Ok();
			} catch(Exception e)
			{
				Log($"!!! EXCEPTION CAUGHT !!!");
				Log($"Type   : {e.GetType().FullName}");
				Log($"Message: {e.Message}");
				Log($"Stack  :{Environment.NewLine}{e.StackTrace}");
				if(e.InnerException != null)
				{
					Log($"Inner  : {e.InnerException.GetType().FullName} → {e.InnerException.Message}");
					Log($"Inner Stack:{Environment.NewLine}{e.InnerException.StackTrace}");
				}
				Log("=== CRP Process Failed ===");
				return this.HandleException(e);
			}
		}
	}
