	...
	public class CountriesController: ControllerBase
	{
		...
		public IActionResult GetMyCountries()
		{
			...
			try
			{
				...
				var countriesModel = new List<Core.Apps.WorkPlan.Models.Country.CountryViewModel>();

				var userCountries = connectedUser.IsGlobalDirector
						? Core.Apps.WorkPlan.Helpers.User.GetAllCoutntries()
						: Core.Apps.WorkPlan.Helpers.User.GetUserCoutntries(connectedUser.Id);
				if(userCountries != null)
				{
					...
					countriesDb.ForEach(c => countriesModel.Add(new Core.Apps.WorkPlan.Models.Country.CountryViewModel(c)));
				}

				if(errors.Count == 0)
					return Ok(new { response = new Models.Response<List<Core.Apps.WorkPlan.Models.Country.CountryViewModel>>(true, countriesModel, errors) });
				else
					return Ok(new { response = new Models.Response<string>(false, "", errors) });
			} catch(Exception e)
			{
		...
		public IActionResult Get(int Id)
		{
			...
			try
			{
				var country = new Core.Apps.WorkPlan.Models.Country.CountryViewModel();
				var connectedUser = this.GetCurrentUser();
				...
				else
				{
					...
					if(countryDb != null)
						country = new Core.Apps.WorkPlan.Models.Country.CountryViewModel(countryDb);
					else
						errors.Add("country: Item not found.");
				}

				if(errors.Count == 0)
					return Ok(new { response = new Models.Response<Core.Apps.WorkPlan.Models.Country.CountryViewModel>(true, country, errors) });
				else
					return Ok(new { response = new Models.Response<string>(false, "", errors) });
			} catch(Exception e)
			{
		...
		[HttpPost]
		[SwaggerOperation(Tags = new[] { MODULE })]
		public IActionResult Add(Core.Apps.WorkPlan.Models.Country.CountryViewModel model)
		{
			var errors = new List<string>();
			try
			{
				...
				if(connectedUser != null)
				{
					if(connectedUser.Access == null || !connectedUser.Access.WorkPlan.CountryUpdate)
						return Ok(new { response = new Models.Response<string>(true, "", new List<string> { "User not authorized" }) });
					...
					{
						return Ok(new { response = new Models.Response<string>(true, addedId + "", errors) });
					}
				}
				else
				{
		...
		[HttpPost]
		[SwaggerOperation(Tags = new[] { MODULE })]
		public IActionResult Edit(Core.Apps.WorkPlan.Models.Country.CountryViewModel model)
		{
			var errors = new List<string>();
			...
			try
			{
				...
				if(errors.Count == 0)
				{
					...
					return Ok(new { response = new Models.Response<string>(true, "", errors) });
				}
		...
		public IActionResult Delete(int Id)
		{
			...
			try
			{
				...
				return Ok(new { response = new Models.Response<string>(false, "Delete: database error. Try again later.", errors) });
			} catch(Exception e)
			{
				return this.HandleException(e);
			}
