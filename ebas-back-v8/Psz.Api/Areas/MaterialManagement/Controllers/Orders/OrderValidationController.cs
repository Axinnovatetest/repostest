...
namespace Psz.Api.Areas.MaterialManagement.Controllers.Orders
{
	...
	public class OrderValidationController: Controller
	{
		...
		public async Task<IActionResult> GetBestellungReport(int bestellungNr)
		{
			try
			{
				var response = await new Core.MaterialManagement.Orders.Handlers.OrderValidation.GenerateReportHandler(this.GetCurrentUser(false), bestellungNr)
				   .HandleAsync();
		...
		public async Task<IActionResult> PlaceOrder([FromForm] Core.MaterialManagement.Orders.Models.OrderValidation.placeOrderRequestModel data = null)
		{
			try
			{
				var response = await new Core.MaterialManagement.Orders.Handlers.OrderValidation.PlaceOrderHandler(this.GetCurrentUser(false), data)
				   .HandleAsync();
