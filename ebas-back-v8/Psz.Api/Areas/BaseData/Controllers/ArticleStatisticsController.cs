...
namespace Psz.Api.Areas.BaseData.Controllers
{
	...
	public class ArticleStatisticsController: ControllerBase
	{
		...
		public async Task<IActionResult> AddProjectMessageMulti([FromBody] List<Psz.Core.BaseData.Models.Article.Statistics.ControllingAnalysis.ProjectMessageItemRequestModel> data)
		{
			try
			{
				var results = await new Core.BaseData.Handlers.Article.Statistics.ControllingAnalysis.AddProjectMessageMultiHandler(this.GetCurrentUser(), data).HandleAsync();
		...
		public async Task<IActionResult> AddnPrintProjectMessageMulti([FromBody] List<Psz.Core.BaseData.Models.Article.Statistics.ControllingAnalysis.ProjectMessageItemRequestModel> data)
		{
			try
			{
				var results = await new Core.BaseData.Handlers.Article.Statistics.ControllingAnalysis.AddProjectMessageMultiHandler(this.GetCurrentUser(), data).HandleAsync();
		...
		[HttpPost]
		[SwaggerOperation(Tags = new[] { MODULE })]
		public async Task<ActionResult> DownloadControllingAnalysis_ProjectMessage([FromBody] List<Psz.Core.BaseData.Models.Article.Statistics.ControllingAnalysis.ProjectMessageItemModel> data)
		{
			try
			{
				var results = await Core.BaseData.Handlers.Article.Statistics.ControllingAnalysis.GetProjectMessagePDFDataHandler.GetData(data, null);
				if(results != null && results.Length > 0)
				{
		...
		[SwaggerOperation(Tags = new[] { MODULE })]
		[ProducesResponseType(typeof(Core.Models.ResponseModel<string>), 200)]
		public async Task<IActionResult> BS_GetCartonsPDF(Core.BaseData.Models.Article.Statistics.Basics.CartonsCirculationRequestModel data)
		{
			try
			{
				return Ok(await new Core.BaseData.Handlers.Article.Statistics.Basics.GetCartonsPDFHandler(this.GetCurrentUser(), data).HandleAsync());
			} catch(Exception e)
			{
				return this.HandleException(e);
			}
		...
		[SwaggerOperation(Tags = new[] { MODULE })]
		[ProducesResponseType(typeof(Core.Models.ResponseModel<string>), 200)]
		public async Task<IActionResult> BS_GetCirculationsPDF(Core.BaseData.Models.Article.Statistics.Basics.CartonsCirculationRequestModel data)
		{
			try
			{
				return Ok(await new Core.BaseData.Handlers.Article.Statistics.Basics.GetCirculationsPDFHandler(this.GetCurrentUser(), data).HandleAsync());
			} catch(Exception e)
			{
				return this.HandleException(e);
			}
		...
		[SwaggerOperation(Tags =new[] { MODULE })]
		[ProducesResponseType(typeof(Core.Models.ResponseModel<int>), 200)]
		public IActionResult GetArtikelNr(string artikelnummer)
		{
			var response=new Psz.Core.BaseData.Handlers.GetArticleNrHandler(this.GetCurrentUser(), artikelnummer).Handle();
			return Ok(response);
		}
	}
}