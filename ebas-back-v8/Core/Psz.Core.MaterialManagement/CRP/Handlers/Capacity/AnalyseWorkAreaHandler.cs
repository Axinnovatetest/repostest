...
{
	public class AnalyseWorkAreaHandler: IHandle<GetAnalyseReportRequestModel, ResponseModel<AnalyseWorkAreaResponseModel>>
	{
		private GetAnalyseReportRequestModel data { get; set; }
		private Identity.Models.UserModel user { get; set; }

		public AnalyseWorkAreaHandler(GetAnalyseReportRequestModel data,
		...
		public ResponseModel<AnalyseWorkAreaResponseModel> Handle()
		{
		...
		public static ResponseModel<AnalyseWorkAreaResponseModel> Perform(Identity.Models.UserModel user,
			GetAnalyseReportRequestModel data)
		{
			...
			{
				return ResponseModel<AnalyseWorkAreaResponseModel>.FailureResponse("Country is not found");
			}
			#endregion
			...
			var workAreaEntities = WorkAreaAccess.Get()?.Where(x => x.IsArchived != true
			...
			var response = new AnalyseWorkAreaResponseModel();

			var minValue = data.RoundToMinute.HasValue && data.RoundToMinute.Value ? Helpers.Config.MinAttendance : 0m;
			foreach(var workArea in workAreaEntities)
			{
				var capacityItem = new AnalyseWorkAreaResponseModel.Item()
				{
				...
				foreach(var capacityEntity in capacityEntities.Where(e => e.WorkAreaId == workArea.Id && e.WeekNumber > todaysWeekNumber))
				{
				...
				var requestedCapacityItem = new AnalyseWorkAreaResponseModel.Item()
				{
					WorkAreaId = workArea.Id,
					WorkAreaName = workArea.Name,
					Attendance = 0,
				...
				foreach(var requestedCapacityEntity in requestedCapacityEntities.Where(e => e.WorkAreaId == workArea.Id))
				{
			...
			return ResponseModel<AnalyseWorkAreaResponseModel>.SuccessResponse(response);
		}

		public ResponseModel<AnalyseWorkAreaResponseModel> Validate()
		{
