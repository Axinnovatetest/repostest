...
{
	public class SetCapacityPlanKwHandler: IHandle<SetCapacityPlanKwModel, ResponseModel<object>>
	{
		private SetCapacityPlanKwModel data { get; set; }
		private Psz.Core.Identity.Models.UserModel user { get; set; }

		public SetCapacityPlanKwHandler(SetCapacityPlanKwModel data,
			...
			lock(Locks.CapacityLock)
			{
				try
				{
					...
					if(this.data.Year < DateTime.Today.Year)
					{
						...
						{
							Value = "Year should be current or later",
						});
					}
					...
					foreach(var item in this.data.Items)
					{
						...
						// -
						var newWeeks = new List<int> { };
						if(data.FirstWeekNumber < data.OldFirstWeekNumber)
							newWeeks.AddRange(Enumerable.Range(data.FirstWeekNumber, data.OldFirstWeekNumber - data.FirstWeekNumber).ToList());

						if(data.LastWeekNumber > data.OldLastWeekNumber)
							newWeeks.AddRange(Enumerable.Range(data.OldLastWeekNumber + 1, data.LastWeekNumber - data.OldLastWeekNumber).ToList());

						foreach(var ii in newWeeks)
						{
							var capacityPlanEntities = CapacityPlanAccess.Get_BY_Country_Hall_Week_Operation_Department_WorkArea(
								data.CountryId,
								data.HallId,
								data.Year,
								ii,
							...
							if(capacityPlanEntities != null && capacityPlanEntities.Count > 0)
							{
								errors.Add(new ResponseModel<object>.ResponseError()
								{
									Value = $"Position {i}: Capacity Plan exists with {item.OperationName}/{item.DepartementName}/{item.HallName}/{item.WorkAreaName} in week {i}.",
								});
							}
						}
					}
					...
					foreach(var item in data.Items)
					{
						oldCapacities.AddRange(CapacityPlanAccess.Get_BY_Country_Hall_Week_Operation_Department_WorkArea_BETWEEN_Weeks(
								...
								item.WorkAreaId,
								item.WorkStationId,
								data.OldFirstWeekNumber,
								data.OldLastWeekNumber,
		...
		ResponseModel<object> IHandle<SetCapacityPlanKwModel, ResponseModel<object>>.Validate()
		{
