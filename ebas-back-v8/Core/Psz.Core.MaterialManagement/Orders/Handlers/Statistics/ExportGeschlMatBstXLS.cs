...
{
	public class ExportGeschlMatBstXLS: IHandle<OffenematbstRequestModel, ResponseModel<byte[]>>
	{
		private UserModel _user { get; set; }
		private OffenematbstRequestModel _data { get; set; }
		public ExportGeschlMatBstXLS(UserModel user, OffenematbstRequestModel data)
		{
			this._user = user;
			this._data = data;
		}
		...
		public byte[] GetData()
		{
			List<GeschMat_BstEntity> fetchedArticles = new();

			fetchedArticles = Infrastructure.Data.Access.Joins.MTM.Order.Statistics.Offene_mat_bst_access.GetGeschlMatBst(null, null, "", _data.fromdate, _data.todate, 1);
			return SaveToExcelFile(fetchedArticles);
		}

		internal byte[] SaveToExcelFile(
			List<Infrastructure.Data.Entities.Joins.MTM.Order.Statistics.GeschMat_BstEntity> articlesEntities
			)
		{
			...
			try
			{
				...
				using(var package = new ExcelPackage(file))
				{
					...
					ExcelWorksheet worksheet = package.Workbook.Worksheets.Add($"Geschl Mat Bst");

					// Keep track of the row that we're on, but start with four to skip the header
					var headerRowNumber = 2;
					var startColumnNumber = 1;
					var numberOfColumns = 21; // updated
					...
					worksheet.Cells[1, 1].Value = $"Geschl Mat Bst";
					...
					worksheet.Cells[headerRowNumber, startColumnNumber + 4].Value = "Start Anzhal";//
					...
					worksheet.Cells[headerRowNumber, startColumnNumber + 11].Value = "Preis Gesamt";//
					...
					worksheet.Cells[headerRowNumber, startColumnNumber + 20].Value = "Standardlieferant";//
					...
					if(articlesEntities is not null && articlesEntities.Count > 0)
					{
						...
						foreach(var w in articlesEntities)
						{
							...
							worksheet.Cells[rowNumber, startColumnNumber + 4].Value = w?.StartAnzhal;//
							...
							worksheet.Cells[rowNumber, startColumnNumber + 11].Value = w?.Preis_Gesamt;//
							...
							worksheet.Cells[rowNumber, startColumnNumber + 20].Value = w.Standardlieferant ? "YES" : "NO";//
					...
					package.Workbook.Properties.Title = $"Geschl Mat Bst";
