...
{
	public class GetHighRunnerHandler: IHandleAsync<UserModel, ResponseModel<HighRunnerArticleResponseModel>>
	{
		private HighRunnerArticleRequestModel _data { get; set; }
		private UserModel _user { get; set; }
		public GetHighRunnerHandler(UserModel user, HighRunnerArticleRequestModel data)
		{
			this._user = user;
			this._data = data;
		}
		public async Task<ResponseModel<HighRunnerArticleResponseModel>> HandleAsync()
		{
			...
			var responseBody = new HighRunnerArticleResponseModel();
			...
			responseBody.Items = (results)?.Select(x => new HighRunnerArticleItem(x))?.ToList();

			return await ResponseModel<HighRunnerArticleResponseModel>.SuccessResponseAsync(responseBody);
		}
		public Task<ResponseModel<HighRunnerArticleResponseModel>> ValidateAsync()
		{
			if(this._user == null/*
                || this._user.Access.____*/)
			{
				return ResponseModel<HighRunnerArticleResponseModel>.AccessDeniedResponseAsync();
			}

			if(this._data.DateTo >= this._data.DateFrom && (this._data.DateTo - this._data.DateFrom).TotalDays > 370)
				return ResponseModel<HighRunnerArticleResponseModel>.FailureResponseAsync("Date range too big");

			return ResponseModel<HighRunnerArticleResponseModel>.SuccessResponseAsync();
		}
		public async Task<byte[]> SaveToExcelFile()
		{
			...
			{
				var allCount = await Infrastructure.Data.Access.Joins.ArticleStatisticsAccess.ControllingAnalysis.GetHighRunner_Count(this._data.ArticleNumber, this._data.DateFrom, this._data.DateTo);
				var results = await Infrastructure.Data.Access.Joins.ArticleStatisticsAccess.ControllingAnalysis.GetHighRunner(this._data.ArticleNumber, this._data.DateFrom, this._data.DateTo, 0, allCount);
