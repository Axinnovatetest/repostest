...
namespace Psz.Core.CRP.Handlers.Delfor
{
	public partial class DelforService
	{
		...
		public ResponseModel<int> ValidateSaveDelfor(UserModel user, DeliveryForcastModel data)
		{
			...
			if(!IsValidDate(data.Header.ValidFrom.Value))
				return ResponseModel<int>.FailureResponse($"Wrong Header ValidFrom Date [{data.Header.ValidFrom}]");
			if(!IsValidDate(data.Header.ValidTo.Value))
				return ResponseModel<int>.FailureResponse($"Wrong Header ValidTo Date [{data.Header.ValidTo}]");
			if(!IsValidDate(data.Header.Date.Value))
				return ResponseModel<int>.FailureResponse($"Wrong Header Date [{data.Header.Date}]");
