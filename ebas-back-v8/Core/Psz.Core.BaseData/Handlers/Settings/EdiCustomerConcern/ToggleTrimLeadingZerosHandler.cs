...
{
	public class ToggleTrimLeadingZerosHandler: IHandle<Identity.Models.UserModel, ResponseModel<int>>
	{
		private Identity.Models.UserModel _user { get; set; }
		private int _data { get; set; }

		public ToggleTrimLeadingZerosHandler(Identity.Models.UserModel user, int data)
		{
			this._user = user;
			this._data = data;
		}
		...
		public ResponseModel<int> Handle()
		{
			...
			try
			{
				...
				Infrastructure.Data.Access.Tables.PRS.ObjectLogAccess.InsertWithTransaction(ObjectLogHelper.getLog(this._user, this._data,
						  $"Concern: TrimLeadingZeros",
						  $"{entity.TrimLeadingZeros}",
						  $"{!entity.TrimLeadingZeros}", Enums.ObjectLogEnums.Objects.ArticleConfig_EdiConcerns.GetDescription(),
						  Enums.ObjectLogEnums.LogType.Edit), botransaction.connection, botransaction.transaction);
				// -
				if(botransaction.commit())
				{
					return ResponseModel<int>.SuccessResponse(
						Infrastructure.Data.Access.Tables.CTS.EDI_CustomerConcernAccess.ToggeTrimLeadingZeros(this._data, this._user.Id, this._user.Name, botransaction.connection, botransaction.transaction));
				}
				else
				{
