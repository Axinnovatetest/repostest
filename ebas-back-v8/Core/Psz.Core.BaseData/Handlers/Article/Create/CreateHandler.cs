	...
	public class CreateHandler: IHandle<Identity.Models.UserModel, ResponseModel<int>>
	{
		...
		public ResponseModel<int> Handle()
		{
			...
			try
			{
				...
				Infrastructure.Data.Entities.Tables.WPL.CountryEntity countryEntity = null;
				Infrastructure.Data.Entities.Tables.WPL.HallEntity siteEntity = null;

				if(this._data.GoodsGroupName?.Trim()?.ToLower() == "ef")
				{
					countryEntity = Infrastructure.Data.Access.Tables.WPL.CountryAccess.Get(this._data.ProductionCountryId);
					siteEntity = Infrastructure.Data.Access.Tables.WPL.HallAccess.Get(this._data.ProductionSiteId);
				...
				var insertedNr = Infrastructure.Data.Access.Tables.PRS.ArtikelAccess.InsertWithTransaction(
					new Infrastructure.Data.Entities.Tables.PRS.ArtikelEntity
					{
						...
						CustomerPrefix = this._data.GoodsGroupName?.Trim()?.ToLower() == "ef" ? itemParts[0]:"",
		...
		public ResponseModel<int> Validate()
		{
			...
			var itemParts = this._data.ArticleNumber?.Trim().Split('-')?.ToList();
			if((this._data.GoodsGroupName?.Trim()?.ToLower() == "ef" || this._data.GoodsGroupName?.Trim()?.ToLower() == "roh") && (itemParts == null || itemParts.Count < 3))
			{
