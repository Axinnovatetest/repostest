			...
			try
			{
				...
				if(preisGruppenEntites != null)
				{
					...
					if(margeEntities?.Count() > 0)
					{
						...
						{
							var m = margeEntities.FirstOrDefault(x => x.PriceType.Equals(response[i].Type.RemoveAllSpaces(), StringComparison.OrdinalIgnoreCase) == true);
							if(m != null)
							{
								response[i].DbWithCU = m.DBMitCu;
								response[i].DbWithoutCU = m.DB;
								response[i].MargeWithCU = m.MargeMitCu;
								response[i].MargeWithoutCU = m.Marge;
							}
						}
					}
				}
