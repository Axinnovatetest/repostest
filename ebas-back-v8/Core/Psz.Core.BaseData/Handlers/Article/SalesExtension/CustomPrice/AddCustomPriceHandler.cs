...
namespace Psz.Core.BaseData.Handlers.Article.SalesExtension.CustomPrice
{
	public class AddCustomPriceHandler: IHandle<UserModel, ResponseModel<int>>
	{
		...
		public ResponseModel<int> Handle()
		{
			...
			try
			{
				...
				if(staffelPrice != null)
				{
					...
					responseBody = Infrastructure.Data.Access.Tables.PRS.Staffelpreis_KonditionzuordnungAccess_2.Update(data, botransaction.connection, botransaction.transaction);
				}
				else
				{
					responseBody = Infrastructure.Data.Access.Tables.PRS.Staffelpreis_KonditionzuordnungAccess_2.Insert(this._data.ToEntity_2(), botransaction.connection, botransaction.transaction);
				}
				...
				// -
				SetLastCustomPriceQuantityToInf(this._data.ArticleId, botransaction);
		...
		public static void SetLastCustomPriceQuantityToInf(int articleNr, Infrastructure.Services.Utils.TransactionsManager botransaction)
		{
			var entity = Infrastructure.Data.Access.Tables.PRS.PreisgruppenAccess.GetByArtikelNr(articleNr, botransaction.connection, botransaction.transaction);
			if(entity != null)
			{
				...
				// -
				Infrastructure.Data.Access.Tables.PRS.PreisgruppenAccess.UpdateWithTransaction(entity, botransaction.connection, botransaction.transaction);
			}
		}
		public static void SetLastCustomPriceQuantityToInf(List<int> articleNrs, Infrastructure.Services.Utils.TransactionsManager botransaction)
		{
			var entities = Infrastructure.Data.Access.Tables.PRS.PreisgruppenAccess.GetByArtikelNr(articleNrs, botransaction.connection, botransaction.transaction);
			var me2 = new List<KeyValuePair<int, decimal>>(entities.Count);
			var me3 = new List<KeyValuePair<int, decimal>>(entities.Count);
			var me4 = new List<KeyValuePair<int, decimal>>(entities.Count);
			if(entities != null && entities.Count>0)
			{
				for(int i = 0; i < entities.Count; i++)
				{
					if((!entities[i].Staffelpreis3.HasValue || entities[i].Staffelpreis3 == 0) && (!entities[i].ME3.HasValue || entities[i].ME3.Value <= 0))
					{
						if(entities[i].Staffelpreis2.HasValue && entities[i].Staffelpreis2.Value > 0)
							me2.Add(new KeyValuePair<int, decimal>(entities[i].Nr,Infrastructure.Data.Access.Tables.PRS.PreisgruppenAccess.PRICE_INFINITY));
					}
					else if((!entities[i].Staffelpreis4.HasValue || entities[i].Staffelpreis4 == 0) && (!entities[i].ME4.HasValue || entities[i].ME4.Value <= 0))
					{
						if(entities[i].Staffelpreis3.HasValue && entities[i].Staffelpreis3.Value > 0)
							me3.Add(new KeyValuePair<int, decimal>(entities[i].Nr, Infrastructure.Data.Access.Tables.PRS.PreisgruppenAccess.PRICE_INFINITY));
					}
					else if(entities[i].Staffelpreis4.HasValue && entities[i].Staffelpreis4 != 0 && entities[i].ME4.HasValue)
					{
						me4.Add(new KeyValuePair<int, decimal>(entities[i].Nr, Infrastructure.Data.Access.Tables.PRS.PreisgruppenAccess.PRICE_INFINITY));
					}
				}

				// -
				Infrastructure.Data.Access.Tables.PRS.PreisgruppenAccess.UpdateME2(me2, botransaction.connection, botransaction.transaction);
				Infrastructure.Data.Access.Tables.PRS.PreisgruppenAccess.UpdateME3(me3, botransaction.connection, botransaction.transaction);
				Infrastructure.Data.Access.Tables.PRS.PreisgruppenAccess.UpdateME4(me4, botransaction.connection, botransaction.transaction);
			}
		}
	}
}
