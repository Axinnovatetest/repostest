...
namespace Psz.Core.BaseData.Handlers.Article.SalesExtension
{
	...
	public class GetArticleMaterialCostDBwoCUPercentHandler: IHandle<UserModel, ResponseModel<decimal>>
	{
		...
		public GetArticleMaterialCostDBwoCUPercentHandler(UserModel user, int articleNr)
		{
			this._user = user;
		...
		public ResponseModel<decimal> Handle()
		{
			try
			{
				...
				return ResponseModel<decimal>.SuccessResponse(Infrastructure.Data.Access.Tables.PRS.ArtikelAccess.GetMaterialCostDBwoCUPercent(articleEntity.ArtikelNummer));
