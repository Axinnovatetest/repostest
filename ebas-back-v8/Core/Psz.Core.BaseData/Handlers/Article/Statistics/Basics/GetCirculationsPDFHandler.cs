...
namespace Psz.Core.BaseData.Handlers.Article.Statistics.Basics
{
	...
	public class GetCirculationsPDFHandler: IHandleAsync<UserModel, ResponseModel<string>>
	{
		...
		public async Task<byte[]> GetFilePDF(List<Models.Article.Statistics.Basics.CirculationResponseModel> dataEntities)
		{
			try
			{
				...
				{
					BodyData = data,
					BodyTemplate = "bsd_article_circulation_body",
					DocumentTitle = "",
					FooterCenterText = "",
					FooterData = null,
				...
				return response;
				//Module.ReportingService.Generate_BSD_ArticlesStatisticsCirculations(Infrastructure.Services.Reporting.Helpers.ReportType.BSD_ART_STATS_CIRCULATIONS, data);
			} catch(Exception exception)
			{
