...
namespace Psz.Core.BaseData.Handlers.Article.Statistics.ControllingAnalysis
{
	...
	public class QuickPoHandler: IHandle<UserModel, ResponseModel<int>>
	{
		...
		public ResponseModel<int> Validate()
		{
			...
			if(_user.Number is null || _user.Number <= 0)
			{
				return ResponseModel<int>.FailureResponse($"User does not have a valid PO Creation Number [{_user.Number}].");
			}
			...
			try
			{
				...
				try
				{
					...
					var orderData = new Infrastructure.Data.Entities.Tables.MTM.BestellungenEntity()
					{
						...
						Offnen = false,
						Kanban = false,
						Benutzer=$"{this._user.LegacyUserName} [{this._user.Username}] {DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss")}"
					};
