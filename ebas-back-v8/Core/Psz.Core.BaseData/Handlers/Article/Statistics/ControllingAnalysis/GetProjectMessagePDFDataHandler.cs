...
{
	public class GetProjectMessagePDFDataHandler: IHandleAsync<UserModel, ResponseModel<byte[]>>
	{
		...
		public async Task<ResponseModel<byte[]>> HandleAsync()
		{
			try
			{
				...
				return ResponseModel<byte[]>.SuccessResponse(await GetData(this._data));
		...
		public async static Task<byte[]> GetData(List<Psz.Core.BaseData.Models.Article.Statistics.ControllingAnalysis.ProjectMessageItemModel> data, int? pmeldung = 1)
		{
			...
			return await SaveToPdfFile(results);
		}
		internal static async Task<byte[]> SaveToPdfFile(List<Infrastructure.Data.Entities.Joins.ArticleStatisticsEntities.ControllingAnalysis_ProjectMessagePDF> dataEntities)
		{
			try
			{
				var data = new Reporting.Models.StatisticsProjectMessage();

				// -
				data.ReportData = new Reporting.Models.StatisticsProjectMessage.Header { Title = $"'N' Meldung" };

				// - Items
				data.Items = dataEntities
					//.Where(x => data.Suppliers.Exists(y => y.Name1 == x.Name1))
					.Select(x => new Reporting.Models.StatisticsProjectMessage.Item
					{
				...
				{
					BodyData = data,
					BodyTemplate = "bsd_project_message_body",
					DocumentTitle = $"",
					FooterCenterText = $"Druckdatum {DateTime.Now.ToString("dd.MM.yyyy HH:mm:ss")}",
					FooterData = null,
					FooterLeftText = "PSZ From: R_164_01_N-Meldung",
				...
				return response;
				//Module.ReportingService.Generate_BSD_ArticlesStatisticsProjectMessage(Infrastructure.Services.Reporting.Helpers.ReportType.BSD_ART_STATS_PROJECTMSG, data);
			} catch(Exception exception)
			{
