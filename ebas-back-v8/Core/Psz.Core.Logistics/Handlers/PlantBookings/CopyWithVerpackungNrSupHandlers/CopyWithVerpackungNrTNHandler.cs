...
internal class CopyWithVerpackungNrTNHandler: IHandle<Identity.Models.UserModel, ResponseModel<PlantBookingDetailsResponseModel>>
{
	private int _verpackungNr;
	private int _lagerId;
	private Core.Identity.Models.UserModel _user;
	public CopyWithVerpackungNrTNHandler(int VerpackungNr, int lager, Core.Identity.Models.UserModel user)
	{
	...
	public ResponseModel<PlantBookingDetailsResponseModel> Handle()
	{
		...
		try
		{
			...
			var plantBookingByVerpackungNr =  Infrastructure.Data.Access.Tables.Logistics.PlantBookingAccess.GetByVerpackungNrTn(_verpackungNr);
			...
			vernum = Infrastructure.Data.Access.Tables.CTS.PSZ_Eingangskontrolle_TNAccess.InsertWithTransaction(new Infrastructure.Data.Entities.Tables.CTS.PSZ_Eingangskontrolle_TNEntity(plantBookingByVerpackungNr, null), botransaction.connection, botransaction.transaction);


			if(!botransaction.commit())
				return ResponseModel<PlantBookingDetailsResponseModel>.FailureResponse("Failed to copy the Data !");

			var newlyInsertedPlantBooking = Infrastructure.Data.Access.Tables.CTS.PSZ_Eingangskontrolle_TNAccess.GetBynummer_verpackung(vernum);
