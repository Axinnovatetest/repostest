...
{
	public class Entnahme2WertHandler: IHandle<Identity.Models.UserModel, ResponseModel<EntnahmeWertKompletModel>>
	{
		...
		public Entnahme2WertHandler(DateTime D1, DateTime D2, int L1, int Type, string Artikelnummer, Identity.Models.UserModel user)
		{
			_user = user;
			_DateD = D1;
			_DateF = D2;
				...
				if(entnahmeWertWithoutEKEntities != null && entnahmeWertWithoutEKEntities.Count > 0)
				{
				   ...
				   .Select(m => new Groupping
				   {
					   ...
					   gesmtPreis = Math.Round((from num in entnahmeWertWithoutEKEntities
												where num.datum == m.Key.datum
												select num.kosten).Sum(), 2),
					   percentPreis = (from num in entnahmeWertWithoutEKEntities
									   select num.kosten).Sum()!=0?
					   Math.Round((from num in entnahmeWertWithoutEKEntities
												  where num.datum == m.Key.datum
												  select num.kosten).Sum() /
									(from num in entnahmeWertWithoutEKEntities
									 select num.kosten).Sum() * 100, 2):0
				   }).ToList();
					foreach(var item in _groupping)
					{
