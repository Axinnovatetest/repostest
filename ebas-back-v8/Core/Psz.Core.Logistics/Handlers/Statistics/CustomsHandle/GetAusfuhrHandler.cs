...
public class GetAusfuhrHandler: IHandle<AusfuhrRequestModel, ResponseModel<IPaginatedResponseModel<AusfuhrModel>>>
{
	...
	public GetAusfuhrHandler(Identity.Models.UserModel user, AusfuhrRequestModel data)
	{
		this._user = user;
		this._data = data;
	}
	...
	private ResponseModel<IPaginatedResponseModel<AusfuhrModel>> Perform(Identity.Models.UserModel user, AusfuhrRequestModel data)
	{
		try
		{


			var fetchedData = Infrastructure.Data.Access.Joins.Logistics.CustomsAccess.CustomsAccess.GetAusfuhr(data.fromdate, data.todate, null, null, 0);
