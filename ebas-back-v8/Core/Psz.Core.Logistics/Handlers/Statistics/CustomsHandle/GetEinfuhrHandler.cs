...
public class GetEinfuhrHandler: IHandle<EinfuhrRequestModel, ResponseModel<IPaginatedResponseModel<EinfuhrModel>>>
{
	...
	public GetEinfuhrHandler(Identity.Models.UserModel user, EinfuhrRequestModel data)
	{
		this._user = user;
		this._data = data;
	}
	...
	private ResponseModel<IPaginatedResponseModel<EinfuhrModel>> Perform(Identity.Models.UserModel user, EinfuhrRequestModel data)
	{
		try
		{
			var fetchedData = Infrastructure.Data.Access.Joins.Logistics.CustomsAccess.CustomsAccess.GetEinfuhr(data.fromdate, data.todate, data.gruppe, null, null, 0);
