...
{
	public class DownloadExceltInventurlisteEFHandle: IHandle<Identity.Models.UserModel, ResponseModel<byte[]>>
	{
		private Identity.Models.UserModel _user { get; set; }
		private RohmaterialSearch _data { get; set; }
		public DownloadExceltInventurlisteEFHandle(RohmaterialSearch _data, Identity.Models.UserModel user)
		{
			this._user = user;
			this._data = _data;
		}
		public ResponseModel<byte[]> Handle()
		{
			try
			{
				...
				var response = new List<InventurlisteEFModel>();
				var PackingListEntity = Infrastructure.Data.Access.Joins.Logistics.StatisticsAccess.GetInventurlisteEF(
				...
				if(PackingListEntity != null && PackingListEntity.Count > 0)
					response = PackingListEntity.Select(k => new InventurlisteEFModel(k)).ToList();
				return ResponseModel<byte[]>.SuccessResponse(getExcel(response, _data, "Inventur listeEF"));
			} catch(Exception e)
			{
		...
		static byte[] getExcel(List<InventurlisteEFModel> MainRequestModel, RohmaterialSearch _data, string title)
		{

			string filePath = Path.Combine(Path.GetTempPath(), $"PSZ_Inventurliste EF {DateTime.Now.ToString("yyyy-MM-ddTHHmmss.fff")}.xlsx");
			try
			{
				...
				using(var package = new ExcelPackage(file))
				{
					...
					ExcelWorksheet worksheet = package.Workbook.Worksheets.Add($"PSZ_Inventurliste EF - {DateTime.Now.ToString("yyyy/MM/dd")}");
