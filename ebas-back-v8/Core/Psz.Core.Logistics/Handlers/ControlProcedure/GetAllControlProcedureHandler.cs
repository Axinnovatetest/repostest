...
{
	public class GetAllControlProcedureHandler: IHandle<Identity.Models.UserModel, ResponseModel<GetControlProcedureResponseModel>>
	{
		private Core.Identity.Models.UserModel _user;
		private CreateControlProcedureRequestModel _data;

		public GetAllControlProcedureHandler(Core.Identity.Models.UserModel user, CreateControlProcedureRequestModel data)
		{
			this._user = user;
			this._data = data;
		}
		public ResponseModel<GetControlProcedureResponseModel> Handle()

		{
			try
			{
				...
				int totalCount = Infrastructure.Data.Access.Tables.Logistics.PlantBookingArticleControlProcedureAccess.CountArticleProcedureData(_data.SearchValue, _data.ArticleId, _data.SupplierId);
				...
				var results = Infrastructure.Data.Access.Tables.Logistics.PlantBookingArticleControlProcedureAccess.GetDataArtikleProcedure(_data.SearchValue, _data.ArticleId, _data.SupplierId, dataSorting, dataPaging);
				var r = this._data.PageSize > 0 ? totalCount / decimal.Parse((this._data.PageSize).ToString()) : 0;
				if(results is null || results.Count == 0)
					return ResponseModel<GetControlProcedureResponseModel>.SuccessResponse(
						new GetControlProcedureResponseModel()
						{
							Items = new List<ControlProcedureResponseModel>(),
				...
				return ResponseModel<GetControlProcedureResponseModel>.SuccessResponse(new GetControlProcedureResponseModel
				{
					Items = results.Select(x => new ControlProcedureResponseModel(x)).ToList(),
					PageRequested = this._data.RequestedPage,
					PageSize = this._data.PageSize,
					TotalCount = totalCount,
					TotalPageCount = (int)Math.Ceiling(decimal.Parse(r.ToString())),
		...
		public ResponseModel<GetControlProcedureResponseModel> Validate()
		{
			if(this._user == null/*|| this._user.Access.____*/)
			{
				return ResponseModel<GetControlProcedureResponseModel>.AccessDeniedResponse();
			}
			return ResponseModel<GetControlProcedureResponseModel>.SuccessResponse();
		}
	}
