		...
		ResponseModel<List<KeyValuePair<int, string>>>>
	{
		...
		public ResponseModel<List<KeyValuePair<int, string>>> Handle()
		{
			try
			{
				...
				var adressenEntities = Infrastructure.Data.Access.Tables.FNC.AdressenAccess.GetLikeSupplierNames(this._data.Trim().ToLower());

				if(adressenEntities != null && adressenEntities.Count > 0)
				{
					foreach(var adressenEntity in adressenEntities)
					{
						responseBody.Add(new KeyValuePair<int, string>(adressenEntity.Nr, $"{adressenEntity.Name1.Trim()} {adressenEntity.Name2.Trim()} {adressenEntity.Name3.Trim()}".Trim()));
					}
				}
