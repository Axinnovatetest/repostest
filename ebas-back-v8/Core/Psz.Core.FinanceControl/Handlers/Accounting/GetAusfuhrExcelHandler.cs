...
public class GetAusfuhrExcelHandler: IHandle<AusfuhrRequestModel, ResponseModel<byte[]>>
{

	private AusfuhrRequestModel _data { get; set; }
	private UserModel _user { get; set; }
	public GetAusfuhrExcelHandler(UserModel user, AusfuhrRequestModel data)
	{
		this._user = user;
		this._data = data;
	}
	...
	public byte[] GetData()
	{
		List<AusfuhrEntity> fetchedData = new();

		fetchedData = Infrastructure.Data.Access.Joins.FNC.Accounting.AccountingAccess.GetAusfuhr(_data.fromdate, _data.todate, null, null, 1);
		return SaveToExcelFile(fetchedData);
	}

	internal byte[] SaveToExcelFile(
		List<Infrastructure.Data.Entities.Joins.FNC.Accounting.AusfuhrEntity> entities
		)
	{
		...
		try
		{
			...
			var filePath = System.IO.Path.Combine(tempFolder, $"Ausfuhr-{DateTime.Now.ToString("yyyyMMddTHHmmff")}.xlsx");
			...
			using(var package = new ExcelPackage(file))
			{
				...
				ExcelWorksheet worksheet = package.Workbook.Worksheets.Add($"Ausfuhr");
				...
				worksheet.Cells[1, 1].Value = $"Ausfuhr";
				...
				worksheet.Cells[headerRowNumber, startColumnNumber + 0].Value = "Vorname";//
				worksheet.Cells[headerRowNumber, startColumnNumber + 1].Value = "Zolltarif_nr";//
				worksheet.Cells[headerRowNumber, startColumnNumber + 2].Value = "VK-Nettosumme";//
				worksheet.Cells[headerRowNumber, startColumnNumber + 3].Value = "Gewicht in kg";//
				worksheet.Cells[headerRowNumber, startColumnNumber + 4].Value = "Ursprungsland";//
				...
				if(entities is not null && entities.Count > 0)
				{
					...
					{
						// -
						worksheet.Cells[rowNumber, startColumnNumber + 0].Value = w?.Vorname_NameFirma;//
						worksheet.Cells[rowNumber, startColumnNumber + 1].Value = (w?.Zolltarif_nr) == null ? "" : w?.Zolltarif_nr;//
						worksheet.Cells[rowNumber, startColumnNumber + 2].Value = w?.VK_Nettosumme;//
						worksheet.Cells[rowNumber, startColumnNumber + 3].Value = w?.Gewicht_in_kg;//
						worksheet.Cells[rowNumber, startColumnNumber + 4].Value = w?.Ursprungsland;//
				...
				package.Workbook.Properties.Title = $"Ausfuhr";
