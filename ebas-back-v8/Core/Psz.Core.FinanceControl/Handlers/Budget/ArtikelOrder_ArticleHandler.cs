...
{
	public class ArtikelOrder_ArticleHandler
	{
		public Identity.Models.UserModel _user { get; set; }
		private int _data { get; set; }
		private int _data2 { get; set; }

		public ArtikelOrder_ArticleHandler(Identity.Models.UserModel user, int id, int id_Article)
		{
			this._user = user;
			this._data = id;
			this._data2 = id_Article;
		...
		public ResponseModel<List<Models.Budget.ArtikelOrderConcatNameModel>> Handle()
		{
			try
			{
				...
				//var artikel_order = Infrastructure.Data.Access.Tables.FNC.Article_OrderAccess.GetArtikelOrder_Article(this._data,this._data2);
