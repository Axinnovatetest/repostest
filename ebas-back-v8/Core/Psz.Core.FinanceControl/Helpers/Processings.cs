...
namespace Psz.Core.FinanceControl.Helpers
{
	...
	out List<string> errors)
				...
				public static void updateArticlePrices(List<int> orderIds)
				{
					...
					var orderEntities = Infrastructure.Data.Access.Tables.FNC.BestellungenExtensionAccess.GetByOrderIds(orderIds, null);
				...
				public static void applyLeasingFees(bool sendSummaryEmail = true)
				{
					...
					try
					{
						...
						var leasingValidationHistory = (Infrastructure.Data.Access.Tables.FNC.OrderValidationLeasingAccess.GetFirstByOrderId(orderIds) ?? new List<Infrastructure.Data.Entities.Tables.FNC.OrderValidationLeasingEntity>());
						if(leasingValidationHistory is not null && leasingValidationHistory.Count > 0)
						{
				...
				{
					{
						...
						if(nbYearUnpaid > 0)
						{
							...
							for(int i = 1; i < nbYearUnpaid + 1; i++)
							{
								...
								var userBudgetEntity = Infrastructure.Data.Access.Tables.FNC.BudgetAllocationUserAccess.GetByUserAndYear(userEntity?.Id ?? -1, currentDate.Year);
						...
						if(sendSummaryEmail == true && nextValidatorEmail.Count > 0)
						{
							...
							try
							{
								...
								Module.EmailingService.SendEmailAsync(emailTitle, emailContent, nextValidatorEmail, attachments, new List<string> { Module.EmailingService.EmailParamtersModel.AdminEmail },
									saveHistory: true, senderEmail: userEntity.Email, senderName: userEntity.Username, senderId: userEntity?.Id ?? -1, senderCC: false, attachmentIds: attachmentIds);
							} catch(Exception ex)
							{
