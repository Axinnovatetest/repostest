...
namespace Psz.Core.ManagementOverview.Administration.Handlers.AccessProfiles
{
	public class EditNameHandler: IHandle<Identity.Models.UserModel, ResponseModel<int>>
	{
		...
		private Administration.Models.AccessProfiles.AccessProfileAddRequestModel _data { get; set; }

		public EditNameHandler(Identity.Models.UserModel user, Administration.Models.AccessProfiles.AccessProfileAddRequestModel model)
		{
			this._user = user;
			this._data = model;
		}
		...
		public ResponseModel<int> Handle()
		{
			try
			{
				...
				Infrastructure.Data.Access.Tables.MGO.AccessProfileAccess.UpdateName(accessProfileEntity);
		...
		public ResponseModel<int> Validate()
		{
			...
			if(Infrastructure.Data.Access.Tables.MGO.AccessProfileAccess.Get(this._data.Id) == null)
				return ResponseModel<int>.FailureResponse(key: "1", value: $"Access profile [{this._data.AccessProfileName.Trim()}] not found.");

			var accessProfileEntity = Infrastructure.Data.Access.Tables.MGO.AccessProfileAccess.GetByName(this._data.AccessProfileName);
