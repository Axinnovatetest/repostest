...
namespace Psz.Core.CustomerService.Helpers
{
	...
	public class SaveMailHelper
	{
		...
		// - 07-05-2025
		public static byte[] SaveMailMessage(string fileName, string From, List<string> To, string subject, string Body, List<MailAttachment> attachments, bool autoBCC = true)
		{
			try
			{
				MailMessage mail = new MailMessage();
				...
				// Create file attachments for this email message
				foreach(var attachment in attachments)
				{
					var data = new Attachment(new MemoryStream(attachment.Content), attachment.MimeType);
					ContentDisposition disposition = data.ContentDisposition;
