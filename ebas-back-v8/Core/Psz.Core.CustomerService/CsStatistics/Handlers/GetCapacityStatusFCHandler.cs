...
namespace Psz.Core.CustomerService.CsStatistics.Handlers
{
	...
	public class GetCapacityStatusFCHandler: IHandle<Identity.Models.UserModel, ResponseModel<List<CapacityFcResponseModel>>>
	{
		private Models.CapacityAbFaRequestModel _data { get; set; }
		private Identity.Models.UserModel _user { get; set; }
		public GetCapacityStatusFCHandler(Identity.Models.UserModel user, Models.CapacityAbFaRequestModel data)
		{
			this._user = user;
			this._data = data;
		}
		public ResponseModel<List<CapacityFcResponseModel>> Handle()
		{
			...
			return ResponseModel<List<CapacityFcResponseModel>>.SuccessResponse(
				   Infrastructure.Data.Access.Joins.CTS.CSStatisticsAccess.GetCapacityStatus_FC(_data.ArticleId, dateFrom, dateTo)
				   ?.Select(x => new CapacityFcResponseModel(x))
				   ?.ToList());

		}

		public ResponseModel<List<CapacityFcResponseModel>> Validate()
		{
			if(this._user == null/*
                || this._user.access.____*/)
			{
				return ResponseModel<List<CapacityFcResponseModel>>.AccessDeniedResponse();
			}
			return ResponseModel<List<CapacityFcResponseModel>>.SuccessResponse();
		}
	}
