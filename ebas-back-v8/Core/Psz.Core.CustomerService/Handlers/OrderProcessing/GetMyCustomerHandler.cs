...
{
	public class GetMyCustomersHandler: IHandle<Identity.Models.UserModel, ResponseModel<List<CustomerModel>>>
	{
		private bool? _data { get; set; }
		private Identity.Models.UserModel _user { get; set; }
		public GetMyCustomersHandler(bool? data, Identity.Models.UserModel user)
		{
			this._user = user;
			this._data = data;
		}
		public ResponseModel<List<CustomerModel>> Handle()
		{
			...
			try
			{
				...
				{
					return ResponseModel<List<CustomerModel>>.SuccessResponse(Get(Infrastructure.Data.Access.Tables.PRS.KundenAccess.Get()?.Where(x => archivedCustomerIds?.Contains(x.Nr) == false)?.ToList(), _data));
				}
				...
				return ResponseModel<List<CustomerModel>>.SuccessResponse(Get(Infrastructure.Data.Access.Tables.PRS.KundenAccess.GetByNummers(customersNumbers)?.Where(x => archivedCustomerIds?.Contains(x.Nr) == false)?.ToList(), _data));
