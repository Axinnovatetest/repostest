...
namespace Psz.Core.Apps.WorkPlan.Handlers
{
	public class ExportArticlesWoWPLHandler: IHandle<Identity.Models.UserModel, ResponseModel<byte[]>>
	{
		...
		private ArticlesWoWorkPlanRequestModel data { get; set; }

		public ExportArticlesWoWPLHandler(Identity.Models.UserModel user, ArticlesWoWorkPlanRequestModel data)
		{
			this.user = user;
			this.data = data;
		}
		...
		public static ResponseModel<byte[]> Perform(Identity.Models.UserModel user, ArticlesWoWorkPlanRequestModel data)
		{
			var articleEntities = (Infrastructure.Data.Access.Joins.MTM.CRP.ArticleAccess.GetArticlesWoWPL(data.warengruppeEF, data.wStuckliste, data.wFa, data.wOpenFa, data.lager, data.faDateFrom, data.faDateTill)
				   ?? new List<Infrastructure.Data.Entities.Joins.MTM.CRP.ArticleWpl>())
				   .Select(x => new ArticlesWoWPLModel(x))?.ToList();
		...
		static byte[] getExcel(string selectedLanguage, List<ArticlesWoWPLModel> fertigungFaultyTimeEntities)
		{
