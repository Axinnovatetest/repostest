		...
		public static Core.Models.ResponseModel<object> ValidateUpdate(int orderId, Core.Identity.Models.UserModel user, bool UpdateChoice = false)
		{
			...
			try
			{
				...
				lock(Locks.OrdersLock)
				{
					...
					var articlePrices = Infrastructure.Data.Access.Tables.PRS.PreisgruppenAccess.GetByArtikelNrs(postionsDb?.Select(x => x.ArtikelNr ?? 0)?.ToList());
					if(postionsDb != null || postionsDb.Count > 0)
					{
						for(int i = 0; i < postionsDb.Count; i++)
						{
							...
							// - 2026-03-17 - set position type to Serie if not exists - Reil
							if(UpdateChoice && (postionsDb[i].Typ is null || postionsDb[i].Typ < 0))
							{
								var articlePrice = articlePrices?.FirstOrDefault(x => x.Artikel_Nr == postionsDb[i].ArtikelNr && x.Verkaufspreis is not null && x.Verkaufspreis>0);
								if(articlePrice is not null)
								{
									postionsDb[i].Typ = (int)CustomerService.Enums.OrderEnums.ItemType.Serie;
								}
								else
								{
									positionErrors.Add($"Position {postionsDb[i].Position}: invalid value [{postionsDb[i].Typ}] for Position Type.");
									continue;
								}
							}
