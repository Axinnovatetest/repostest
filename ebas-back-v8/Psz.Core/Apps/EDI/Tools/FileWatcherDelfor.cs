	...
	public class FileWatcherDelfor
	{
		...
		private bool validateDataHeader(Models.Delfor.XMLHeaderModel headerData, out List<string> errors)
		{
			...
			else
			{
				...
				// - 2025-11-28
				if(Infrastructure.Data.Access.Tables.BSD.KundenExtensionAccess.IsArchived(kundenNr))
				{
					errors.Add($"Customer [{headerData.BuyerPartyIdentification}] is archived");
				}
			}
			...
			if(!string.IsNullOrWhiteSpace(headerData.BuyerDUNS) && !string.IsNullOrEmpty(headerData.BuyerDUNS))
			{
				...
				// - 2025-11-28
				var kundenEntity = Infrastructure.Data.Access.Tables.PRS.KundenAccess.GetByNummer(adress?.Nr ?? -1);
				if(Infrastructure.Data.Access.Tables.BSD.KundenExtensionAccess.IsArchived(kundenEntity?.Nr ?? -1))
				{
					errors.Add($"Customer [{headerData.BuyerPartyIdentification}] is archived");
				}
			}
			...
			SqlConnection connection, SqlTransaction transaction, out List<string> errors)
		{
			...
			foreach(var item in lineItems)
			{
				...
				var stdEdiConcernArticles = Infrastructure.Data.Access.Tables.PRS.ArtikelAccess.GetDefaultEdi(
					...
					?? new List<Infrastructure.Data.Entities.Tables.PRS.MinimalArtikelEntity>();
				var customersConcernItems = Infrastructure.Data.Access.Tables.CTS.EDI_CustomerConcernItemsAccess.GetListByConcernId(customerConcern?.Id ?? -1, connection, transaction);
