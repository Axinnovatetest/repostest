	...
	public static class Helpers
	{
		...
		public static string Template(string template, Infrastructure.Data.Entities.Tables.PRS.AngeboteEntity rahmen, string action, string username, Enums.RahmenRedirect redirect)
		{
			if(!string.IsNullOrEmpty(template))
			{
				...
				var link = $"{(redirect == Enums.RahmenRedirect.INS ? Module.INSEmailAppDomaineName : Module.PurchaseEmailAppDomaineName)}{Module.EmailingService.EmailParamtersModel.AppDomaineName}/#/{(redirect == Enums.RahmenRedirect.INS ? "order-responses" : "rahmen/info")}/{rahmen.Nr}";
				if(File.Exists(path))
				{
