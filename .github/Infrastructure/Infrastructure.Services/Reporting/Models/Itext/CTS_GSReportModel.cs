...
{
	public class CTS_GSReportModel
	{
		...
		//Items values
		public List<CTS_GSReportItemsModel> Items { get; set; }
		//Summaries
		...
		public CTS_GSReportModel(Infrastructure.Data.Entities.Tables.PRS.OrderReportEntity reportEntity, Infrastructure.Data.Entities.Tables.PRS.AngeboteEntity orderEntity,
			Infrastructure.Data.Entities.Tables.PRS.OrderExtensionBuyerEntity buyerEntity, Infrastructure.Data.Entities.Tables.PRS.AdressenEntity adressEntity, decimal ust)
		{
			...
			var rechungItem = Infrastructure.Data.Access.Tables.PRS.AngeboteAccess.Get(orderEntity.Nr_rec ?? -1);
			...
			UST_IDTitle = reportEntity.UST_ID;
			...
			LastPageText8 = reportEntity.LastPageText8 ?? "";
			LastPageText9 = reportEntity.LastPageText9 ?? "";
			...
			LastPageText9 = $"Verwendungszweck: F-6444 Deb-Nr.: {orderEntity.Unser_Zeichen} Re.-Nr.: {rechungItem?.Angebot_Nr} Re.-Datum: {(rechungItem?.Datum.HasValue == true ? rechungItem?.Datum.Value.ToString("dd.MM.yyyy") : "")}";
		}
	}
	public class CTS_GSReportItemsModel
	{
		...
		public CTS_GSReportItemsModel(Infrastructure.Data.Entities.Tables.PRS.AngeboteneArtikelEntity itemEntity)
		{
			...
			BasisPrice150Value = $"{Convert.ToDecimal(itemEntity.VKEinzelpreis, System.Globalization.CultureInfo.InvariantCulture).ToString("0.00")} €";
			TotalPrice150Value = $"{Convert.ToDecimal(itemEntity.VKGesamtpreis, System.Globalization.CultureInfo.InvariantCulture).ToString("0.00")} €";
			Cu_GValue = $"{itemEntity.EinzelCuGewicht} Kg";
			CU_SurchargeValue = $"{Convert.ToDecimal(itemEntity.Einzelkupferzuschlag, System.Globalization.CultureInfo.InvariantCulture).ToString("0.00")} €";
			...
			CU_TotalValue = $"{Convert.ToDecimal(itemEntity.Gesamtkupferzuschlag, System.Globalization.CultureInfo.InvariantCulture).ToString("0.00")} €";
			UnitPriceValue = $"{Convert.ToDecimal(itemEntity.Einzelpreis, System.Globalization.CultureInfo.InvariantCulture).ToString("0.00")} €";
			UnitTotalValue = $"{Convert.ToDecimal(itemEntity.Gesamtpreis, System.Globalization.CultureInfo.InvariantCulture).ToString("0.00")} €";
			...
			ExternComment = itemEntity?.GSExternComment;
		}
