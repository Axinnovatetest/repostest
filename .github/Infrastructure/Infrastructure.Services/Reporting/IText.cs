...
namespace Infrastructure.Services.Reporting
{
	public class IText
	{
		...
		public class PdfHeaderFooterHandler: IEventHandler
		{
			...
			public PdfHeaderFooterHandler(string companyName, string address, byte[] logoData = null, bool withFooter = true)
			{
			...
			public void HandleEvent(Event @event)
			{
				...
				// Header background
				pdfCanvas.SetFillColor(new DeviceRgb(245, 245, 245))
						 .Rectangle(pageSize.GetLeft(), pageSize.GetTop() - 55, pageSize.GetWidth(), 55)
						 .Fill();

				float headerHeight = 55;
				float marginLeft = pageSize.GetLeft() + 36;
				float marginRight = pageSize.GetRight() - 36;
				float headerBottom = pageSize.GetTop() - headerHeight;

				// ---- Build header layout using a Table ----
				var headerTable = new Table(UnitValue.CreatePercentArray(new float[] { 1, 4 }))
					.UseAllAvailableWidth()
					.SetFixedPosition(marginLeft, headerBottom + 10, pageSize.GetWidth() - 72) // 36px margins left/right
					.SetHeight(35);

				// --- Logo cell ---
				if(_logoData is not null && _logoData.Length > 0)
				{
					var img = new iText.Layout.Element.Image(ImageDataFactory.Create(_logoData))
						.SetAutoScale(true)
						.SetTextAlignment(TextAlignment.CENTER)
						;
					var logoCell = new Cell().Add(img)
				...
				var infoParagraph = new Paragraph()
					.Add(new Text(_companyName + "\n").SetBold().SetFontSize(11))
					.Add(new Text(_address).SetFontSize(9).SetFontColor(ColorConstants.DARK_GRAY));

				var infoCell = new Cell().Add(infoParagraph)
					.SetBorder(Border.NO_BORDER)
					.SetVerticalAlignment(VerticalAlignment.MIDDLE)
				...
				if(_withFooter)
				{
					...
					pdfCanvas.SetStrokeColor(ColorConstants.LIGHT_GRAY)
							 .MoveTo(pageSize.GetLeft() + 36, footerY + 15)
							 .LineTo(pageSize.GetRight() - 36, footerY + 15)
							 .Stroke();

					int pageNum = pdfDoc.GetPageNumber(page);
					canvas.ShowTextAligned(
						new Paragraph($"Page {pageNum}")
							.SetFontSize(9)
							.SetFontColor(ColorConstants.GRAY),
						marginRight, footerY + 5, TextAlignment.RIGHT);
				}
		...
		public static byte[] GenerateMultiTablePdf(
			...
			string documentTitle = "Data Export",
			string documentSubTitle = "",
			string documentDescription = "",
			bool pageBreakBetweenTables = false,
			bool skipEmtptyTables = false,
			string? companyName = null,
			string? companyAddress = null,
			byte[] logoData = null)
		{
			if(sections == null || !sections.Any())
				throw new ArgumentException("No data provided for PDF generation.");
			...
			{
				var headerHandler = new PdfHeaderFooterHandler(companyName, companyAddress ?? "", logoData);
			...
			// --- Document subTitle ---
			if(!string.IsNullOrWhiteSpace(documentSubTitle))
			{
				var description = new Paragraph(documentSubTitle)
					.SetFontSize(14)
					.SetTextAlignment(TextAlignment.CENTER)
					.SetMarginTop(-15)
					.SetMarginBottom(20);
			...
			{
				var description = new Paragraph(documentDescription)
					.SetFontSize(12)
					.SetTextAlignment(TextAlignment.LEFT)
					.SetMarginBottom(20);
				document.Add(description);
			...
			foreach(var section in sectionList)
			{
				...
				if(!sectionType.IsGenericType ||
					sectionType.GetGenericTypeDefinition() != typeof(PdfTableSection<>))
					continue;

				var genericType = sectionType.GetGenericArguments()[0];
				var titleProp = sectionType.GetProperty("TableTitle");
				var itemsProp = sectionType.GetProperty("Items");
				var tableTitle = titleProp?.GetValue(section)?.ToString() ?? genericType.Name;
				var items = (IEnumerable<object>)itemsProp?.GetValue(section)!;
				if(items == null || !items.Any() && skipEmtptyTables)
					continue;

				AddTable(document, items, genericType, tableTitle);

				sectionIndex++;
				bool isLast = sectionIndex == sectionList.Count;
				if(pageBreakBetweenTables && !isLast)
		...
		private static void AddTable(iText.Layout.Document document, IEnumerable<object> items, Type itemType, string title)
		{
			// --- Table title ---
			...
			var props = itemType.GetProperties(BindingFlags.Public | BindingFlags.Instance)
				.Where(p => !p.IsDefined(typeof(PdfIgnoreAttribute)))
				.ToList();

			var table = new Table(props.Count)
				.UseAllAvailableWidth();
			...
			foreach(var prop in props)
			{
				...
				var headerCell = new Cell()
					.Add(new Paragraph(header).SetFontSize(11).SetBold().SetFontColor(ColorConstants.WHITE))
					.SetBackgroundColor(new DeviceRgb(0, 121, 182))
					.SetTextAlignment(first ? TextAlignment.LEFT : TextAlignment.CENTER)
					.SetPadding(5);
			...
			foreach(var item in items)
			{
				...
				foreach(var prop in props)
				{
					...
					var cell = new Cell()
						.Add(new Paragraph(value.ToString()).SetFontSize(10))
						.SetTextAlignment(first ? TextAlignment.LEFT : TextAlignment.CENTER)
						.SetPadding(5);
					table.AddCell(cell);
					first = false;
		...
		#region table with total
		public static byte[] GenerateMultiTableWithTotalPdf(
			IEnumerable<object> sections,
			string documentTitle = "Data Export",
			string documentSubTitle = "",
			string documentDescription = "",
			...
			byte[] logoData = null)
		{
			...
			for(int i = 0; i < sectionList.Count; i++)
			{
				...
				var titleProp = sectionType.GetProperty("TableTitle");
				var itemsProp = sectionType.GetProperty("Items");
				var totalLabelProp = sectionType.GetProperty("TotalLabel");
				var totalPropertyNameProp = sectionType.GetProperty("TotalPropertyName");
				var showTotalProp = sectionType.GetProperty("ShowTotal");

				var tableTitle = titleProp?.GetValue(section)?.ToString() ?? genericType.Name;
				var items = (IEnumerable<object>)itemsProp?.GetValue(section)!;
				var totalLabel = totalLabelProp?.GetValue(section)?.ToString();
				var totalProperty = totalPropertyNameProp?.GetValue(section)?.ToString();
				var showTotal = (bool)(showTotalProp?.GetValue(section) ?? false);

				if(items == null || !items.Any() && skipEmtptyTables)
					continue;

				AddTableWithTotal(document, items, genericType, tableTitle, showTotal, totalLabel, totalProperty);

				bool isLast = i == sectionList.Count - 1;
				if(pageBreakBetweenTables && !isLast)
					document.Add(new AreaBreak(AreaBreakType.NEXT_PAGE));
				else if(!isLast)
		...
		private static void AddTableWithTotal(iText.Layout.Document document, IEnumerable<object> items, Type itemType,
									 string title, bool showTotal,
									 string? totalLabel, string? totalProperty)
		{
			// --- Section title ---
			document.Add(new Paragraph(title)
				.SetFontSize(14)
			...
			var table = new Table(props.Count).UseAllAvailableWidth();
			...
			// --- Optional total row ---
			if(showTotal && !string.IsNullOrWhiteSpace(totalProperty))
			{
				var totalProp = props.FirstOrDefault(p =>
					string.Equals(p.Name, totalProperty, StringComparison.OrdinalIgnoreCase));
				...
				if(totalProp != null)
				{
					...
					foreach(var item in items)
					{
						var val = totalProp.GetValue(item);
						if(val != null && decimal.TryParse(val.ToString(), out var num))
							totalValue += num;
					}
					...
					for(int i = 0; i < props.Count; i++)
					{
						if(i == props.Count - 2)
						{
							table.AddCell(new Cell()
								.Add(new Paragraph(totalLabel ?? "Total").SetBold())
						...
						{
							table.AddCell(new Cell()
								.Add(new Paragraph(totalValue.ToString("N2")).SetBold())
								.SetBackgroundColor(ColorConstants.LIGHT_GRAY)
								.SetTextAlignment(TextAlignment.CENTER)
								.SetPadding(5));
						}
		...
		public class PdfIgnoreAttribute: System.Attribute
		{
		}

		public class PdfTableSection<T>
		{
			public string TableTitle { get; set; } = typeof(T).Name;
			public IEnumerable<T> Items { get; set; }
			// Optional total row
			public string? TotalLabel { get; set; }            // e.g. "Total"
			public string? TotalPropertyName { get; set; }     // e.g. "Price"
			public bool ShowTotal => !string.IsNullOrWhiteSpace(TotalPropertyName);
		}

		public class LGT
		{
			public static byte[] GenerateInventurfreigabeReport(
								string companyName = "PSZ electronic GmbH",
								string companyAddress = "Musterstraße 12, 12345 Berlin, Germany",
								byte[]? logoBytes = null,
								string title = "Inventurfreigabe",
								string standort = "[Eintragen]",
								string startDate = "",
								string closeDate = "",
								string jahr = "[Eintragen]",
								string responsibleValidateTime = "[Eintragen]",
								string responsible = "[Name / Abteilung]",
								string examinerValidateTime = "[Eintragen]",
								string examiner = "[Pruefer Mustermann]",
								string remarksHL = "",
								string remarksPL = "")
			{
				...
				if(!string.IsNullOrWhiteSpace(companyName))
				{
					var headerHandler = new PdfHeaderFooterHandler(companyName, companyAddress ?? "", logoBytes, false);
					pdf.AddEventHandler(PdfDocumentEvent.START_PAGE, headerHandler);
				}

				var doc = new Document(pdf, PageSize.A4);
				doc.SetMargins(90, 36, 40, 36);
				...
				doc.Add(new Paragraph("1. Grundinformationen")
					.SetBold().SetFontSize(12).SetMarginBottom(5));

				var grundTable = new Table(UnitValue.CreatePercentArray(new float[] { 2, 3 }))
					.UseAllAvailableWidth();
				AddHeaderRow(grundTable, "Feld", "Inhalt / Eintrag");
				AddRow(grundTable, "Standort", standort);
				AddRow(grundTable, "Startdatum", startDate);
				AddRow(grundTable, "Schließdatum", closeDate);
				AddRow(grundTable, "Jahr der Inventur", jahr);
				AddRow(grundTable, "Freigabedatum", responsibleValidateTime);
				AddRow(grundTable, "Verantwortlicher", responsible);
				AddRow(grundTable, "Prüfer / Genehmiger", examiner);
				doc.Add(grundTable);

				doc.Add(new Paragraph("\n"));

				// --- 2. Inventurstatus ---
				doc.Add(new Paragraph("2. Inventurstatus")
					.SetBold().SetFontSize(12).SetMarginBottom(5));

				var statusTable = new Table(UnitValue.CreatePercentArray(new float[] { 3, 2, 2 }))
					.UseAllAvailableWidth();
				AddHeaderRow(statusTable, "Bereich / Lager", "Status", "Bemerkungen");
				AddRow(statusTable, "Hauptlager (HPL)", "[X] Freigegeben", remarksHL?.Substring(0, Math.Min(remarksHL.Length, 100)));
				AddRow(statusTable, "Produktionslager (PROD)", "[X] Freigegeben", remarksPL?.Substring(0, Math.Min(remarksPL.Length, 100)));
				doc.Add(statusTable);

				doc.Add(new Paragraph("\n"));

				// --- 3. Bestätigung ---
				doc.Add(new Paragraph("3. Bestätigung")
					.SetBold().SetFontSize(12).SetMarginBottom(5));

				doc.Add(new Paragraph(
					"Hiermit wird die Inventur für den oben genannten Standort und das Inventurjahr freigegeben.")
					.SetFontSize(10)
					.SetMarginBottom(15));

				var unterschriftTable = new Table(UnitValue.CreatePercentArray(new float[] { 3, 2, 2 }))
					.UseAllAvailableWidth();
				AddHeaderRow(unterschriftTable, "Name / Position", "Unterschrift", "Datum");

				// Apply handwriting font only to “Unterschrift” column
				var fontPath = getFontPath();
				if(!string.IsNullOrWhiteSpace(fontPath))
				{
					var fontProgram = FontProgramFactory.CreateFont(fontPath);
					var handwritingFont = PdfFontFactory.CreateFont(fontProgram, PdfEncodings.IDENTITY_H, PdfFontFactory.EmbeddingStrategy.FORCE_EMBEDDED);
					AddRow(unterschriftTable, new[] { $"Verantwortlicher: {responsible}", responsible.Split(' ').Last(), responsibleValidateTime }, handwritingFont);
					AddRow(unterschriftTable, new[] { $"Prüfer / Genehmiger: {examiner}", examiner.Split(' ').Last(), examinerValidateTime }, handwritingFont);
				}
				else
				{
					AddRow(unterschriftTable, new[] { $"Verantwortlicher: {responsible}", responsible.Split(' ').Last(), responsibleValidateTime });
					AddRow(unterschriftTable, new[] { $"Prüfer / Genehmiger: {examiner}", examiner.Split(' ').Last(), examinerValidateTime });
				}
			...
			private static void AddHeaderRow(Table table, params string[] headers)
			{
				foreach(var header in headers)
				{
					table.AddHeaderCell(new Cell()
						...
						.SetBackgroundColor(new DeviceRgb(34, 38, 84))
			...
			{
				string templatePath = @$"Reporting\Templates\IText\Fonts\Halimun-W7jn.ttf";
				var path = System.IO.Path.Combine(System.IO.Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location), templatePath);
				if(File.Exists(path))
				{
					return path;
				}
			...
			private static void AddRow(Table table, params string[] values)
			{
				...
				{
					table.AddCell(new Cell()
						.Add(new Paragraph(val).SetFontSize(10))
						.SetPadding(4));
				}
			}
			private static void AddRow(Table table, string[] values, PdfFont? handwritingFont = null)
			{
				for(int i = 0; i < values.Length; i++)
				{
					var paragraph = new Paragraph(values[i]).SetFontSize(10);

					// Apply handwriting font if this is the "Unterschrift" column (index 1)
					if(handwritingFont != null && i == 1)
					{
						paragraph.SetFont(handwritingFont).SetFontSize(8);
					}
