...
namespace Infrastructure.Services.Files.Parsing
{
	...
	public class LogParser: ILogParser
	{
		...
		public LogParser(string folderPath, List<int> excludedIds = null, Email.EmailParamtersModel emailParams = null)
		{
		...
		public async Task<List<KeyValuePair<string, int>>> SaveFeedbacksLogsToDBAsync(List<int> idsToExclude)
		{
			return await Task.Run(() =>
			{
				...
				try
				{
					...
					// Get last log date
					var lastLogDate = ERP_LogsAccess.GetLastLogDate(botransaction.connection, botransaction.transaction).FirstOrDefault()?.LogCaptureDate ?? DateTime.Today;

					// Generate log filenames
					var datesTimeRange = Reporting.DateHelpers.getDatesRange(lastLogDate, DateTime.Today);
					var filenames = datesTimeRange.Select(date => $"nlog-all-{date:yyyy-MM-dd}.log").ToList();

					// Fetch log file paths
					var allLogsFilesNames = GetLogsFilesPath(FolderPath, filenames);
					...
					// Delete existing logs for those capture dates
					ERP_LogsAccess.DeleteAllByLogCaptureDate(datesList.ToList(), botransaction.connection, botransaction.transaction);

					// Fetch all users in a single transaction
					var allUsers = UserAccess.GetWithTransaction(botransaction.connection, botransaction.transaction).ToList();

					// Insert logs
					insertedValuesList = ParseAndInsertFeedbacksLogsToDB(allLogsFilesNames, idsToExclude, allUsers, botransaction.connection, botransaction.transaction);

					Logging.Logger.Log("Start committing changes in transaction");

					return botransaction.commit() ? insertedValuesList : new List<KeyValuePair<string, int>>();
				} catch(Exception e)
				{
					botransaction.rollback();
