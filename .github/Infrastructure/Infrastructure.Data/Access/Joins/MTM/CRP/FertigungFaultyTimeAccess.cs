	...
	public class FertigungFaultyTimeAccess
	{
		public static List<Infrastructure.Data.Entities.Joins.MTM.CRP.FertigungFaultyTimeEntity> Get(List<int> lagerortId, DateTime? dateFrom, DateTime dateUntil)
		{
			...
			using(var sqlConnection = new SqlConnection(Settings.ConnectionString))
			{
				...
				//rami 08/09
				string query = $@"SELECT 
    F.Termin_Bestätigt1 AS FaDate,
    F.Fertigungsnummer AS FaNumber,
    A.Artikelnummer AS FaArticle,
    F.Anzahl AS FaQuantity,
    F.Zeit AS FaUnitTime,
    F.Zeit / 60.0 * F.Anzahl AS FaTotalTime,
    COALESCE(A.[Artikel-Nr], 0) AS WPL_ArticleId,
    COALESCE(W.Id, 0) AS WorkScheduleId
FROM DBO.Fertigung F
LEFT JOIN DBO.Artikel A 
       ON A.[Artikel-Nr] = F.Artikel_Nr
LEFT JOIN DBO.WorkSchedule W 
       ON W.Article_Id = A.[Artikel-Nr] AND W.Is_Active = 1
WHERE F.Kennzeichen = 'offen' 
  AND F.Lagerort_id IN ({string.Join(",", lagerortId)})
  AND F.Termin_Bestätigt1 >= @dateFrom 
  AND F.Termin_Bestätigt1 <= @dateUntil
ORDER BY F.Termin_Bestätigt1 ASC;
";
