		...
		public static List<Infrastructure.Data.Entities.Joins.FAPlannung.FALagersPlannungEntity> GetLagersProduktionPlannung(string productionDate, string lager)
		{
			...
			using(var sqlConnection = new SqlConnection(Settings.ConnectionString))
			{
				...
				string query = @"SELECT Fertigung.Lagerort_id AS Werk, Fertigung.Planungsstatus, 
                               ...
                               Fertigung.Datum AS erstelldatum, Fertigung.Bemerkung_Kommissionierung_AL, FertigungType, a.[Angebot-Nr] AS ProjectNr
								,CASE WHEN Fertigung.Originalanzahl < COALESCE(Artikel.losgroesse, p.losgroesse) THEN 0 ELSE 1 END ProdBiggerThanMOQ
                               FROM (Fertigung LEFT JOIN Artikel ON Fertigung.Artikel_Nr = Artikel.[Artikel-Nr]) 
								LEFT JOIN (SELECT DISTINCT [Angebot-Nr], Nr FROM Angebote WHERE ISNULL([Angebot-Nr],0)<>0) a on a.Nr=Fertigung.Angebot_nr
                               LEFT JOIN [View_PSZ_Artikel Kundenzuweisung2] ON Artikel.Artikelnummer = [View_PSZ_Artikel Kundenzuweisung2].Artikelnummer
								LEFT JOIN __BSD_ArtikelSalesExtension p on p.articleNr=Artikel.[Artikel-Nr] AND p.ArticleSalesType=Fertigung.FertigungType
