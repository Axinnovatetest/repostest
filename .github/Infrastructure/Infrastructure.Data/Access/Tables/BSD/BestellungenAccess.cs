...
namespace Infrastructure.Data.Access.Tables.BSD
{
	public class BestellungenAccess
	{
		...
		public static List<Infrastructure.Data.Entities.Tables.BSD.BestellungenEntity> GetByLieferant(int lieferantNr)
		{
			var dataTable = new DataTable();
			using(var sqlConnection = new SqlConnection(Settings.ConnectionString))
			{
				sqlConnection.Open();
				string query = $"SELECT * FROM [Bestellungen] WHERE Rahmenbestellung=0 AND Typ ='Bestellung' AND [Lieferanten-Nr] = {lieferantNr}";
				var sqlCommand = new SqlCommand(query, sqlConnection);
