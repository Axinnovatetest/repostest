...
namespace Infrastructure.Data.Access.Tables.BSD
{
	...
	public class KundenExtensionAccess
	{
		...
		public static IEnumerable<int> GetArchivedIds()
		{
			var dataTable = new DataTable();
			using(var sqlConnection = new SqlConnection(Settings.ConnectionString))
			{
				sqlConnection.Open();
				string query = "SELECT Nr FROM [__BSD_KundenExtension] WHERE ISNULL([IsArchived],0)=1;";
				var sqlCommand = new SqlCommand(query, sqlConnection);
		...
		public static bool IsArchived(int id)
		{
			using(var sqlConnection = new SqlConnection(Settings.ConnectionString))
			{
				sqlConnection.Open();
				string query = "SELECT COUNT(*) FROM [__BSD_KundenExtension] WHERE [IsArchived]=1 AND [Nr]=@Id";
				var sqlCommand = new SqlCommand(query, sqlConnection);
				sqlCommand.Parameters.AddWithValue("Id", id);
