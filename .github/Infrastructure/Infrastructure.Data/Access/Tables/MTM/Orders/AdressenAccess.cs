...
namespace Infrastructure.Data.Access.Tables.MTM
{
	public class AdressenAccess
	{
		...
		public static Infrastructure.Data.Entities.Tables.MTM.AdressenEntity Get(int nr)
		{
			...
			{
				return new Infrastructure.Data.Entities.Tables.MTM.AdressenEntity(dataTable.Rows[0]);
			}
			else
			{
				return null;
		...
		public static List<Infrastructure.Data.Entities.Tables.MTM.AdressenEntity> Get()
		{
			var dataTable = new DataTable();
			using(var sqlConnection = new SqlConnection(Settings.ConnectionString))
			{
			...
			if(dataTable.Rows.Count > 0)
			{
				return dataTable.Rows.Cast<DataRow>().Select(x => new Infrastructure.Data.Entities.Tables.MTM.AdressenEntity(x)).ToList();
			}
			else
			{
				return new List<Infrastructure.Data.Entities.Tables.MTM.AdressenEntity>();
			}
		}
		public static List<Infrastructure.Data.Entities.Tables.MTM.AdressenEntity> Get(List<int> ids)
		{
			if(ids != null && ids.Count > 0)
			{
				int maxQueryNumber = Settings.MAX_BATCH_SIZE;
				List<Infrastructure.Data.Entities.Tables.MTM.AdressenEntity> results = null;
				...
				else
				{
					...
					results = new List<Infrastructure.Data.Entities.Tables.MTM.AdressenEntity>();
		...
		private static List<Infrastructure.Data.Entities.Tables.MTM.AdressenEntity> get(List<int> ids)
		{
			if(ids != null && ids.Count > 0)
			{
				var dataTable = new DataTable();
		...
		public static int Insert(Infrastructure.Data.Entities.Tables.MTM.AdressenEntity item)
		{
			...
			using(var sqlConnection = new SqlConnection(Settings.ConnectionString))
			{
				...
				string query = "INSERT INTO [Adressen] ([Abteilung],[Adresstyp],[Anrede],[Auswahl],[Bemerkung],[Bemerkungen],[Briefanrede],[Dienstag (Anliefertag)],[Donnerstag (Anliefertag)],[DUNS],[EDI-Aktiv],[eMail],[erfasst],[Fax],[Freitag (Anliefertag)],[Funktion],[Kundennummer],[Land],[Lieferantennummer],[Mittwoch (Anliefertag)],[Montag (Anliefertag)],[Name1],[Name2],[Name3],[Ort],[Personalnummer],[PLZ_Postfach],[PLZ_Straße],[Postfach],[Postfach bevorzugt],[Sortierbegriff],[sperren],[Straße],[stufe],[Telefon],[Titel],[von],[Vorname],[WWW]) OUTPUT INSERTED.[Nr] VALUES (@Abteilung,@Adresstyp,@Anrede,@Auswahl,@Bemerkung,@Bemerkungen,@Briefanrede,@Dienstag__Anliefertag_,@Donnerstag__Anliefertag_,@DUNS,@EDI_Aktiv,@eMail,@erfasst,@Fax,@Freitag__Anliefertag_,@Funktion,@Kundennummer,@Land,@Lieferantennummer,@Mittwoch__Anliefertag_,@Montag__Anliefertag_,@Name1,@Name2,@Name3,@Ort,@Personalnummer,@PLZ_Postfach,@PLZ_Strasse,@Postfach,@Postfach_bevorzugt,@Sortierbegriff,@sperren,@Strasse,@stufe,@Telefon,@Titel,@von,@Vorname,@WWW); ";
		...
		public static int Insert(List<Infrastructure.Data.Entities.Tables.MTM.AdressenEntity> items)
		{
			if(items != null && items.Count > 0)
			{
				int maxParamsNumber = Settings.MAX_BATCH_SIZE / 41; // Nb params per query
				int results = 0;
		...
		private static int insert(List<Infrastructure.Data.Entities.Tables.MTM.AdressenEntity> items)
		{
			if(items != null && items.Count > 0)
			{
				int results = -1;
		...
		public static int Update(List<Infrastructure.Data.Entities.Tables.MTM.AdressenEntity> items)
		{
			if(items != null && items.Count > 0)
			{
				int maxParamsNumber = Settings.MAX_BATCH_SIZE / 41; // Nb params per query
		...
		private static int update(List<Infrastructure.Data.Entities.Tables.MTM.AdressenEntity> items)
		{
			if(items != null && items.Count > 0)
			{
				int results = -1;
		...
		public static Infrastructure.Data.Entities.Tables.MTM.AdressenEntity GetWithTransaction(int nr, SqlConnection connection, SqlTransaction transaction)
		{
			var dataTable = new DataTable();
		...
		public static List<Infrastructure.Data.Entities.Tables.MTM.AdressenEntity> GetWithTransaction(SqlConnection connection, SqlTransaction transaction)
		{
			var dataTable = new DataTable();
		...
		public static List<Infrastructure.Data.Entities.Tables.MTM.AdressenEntity> GetWithTransaction(List<int> ids, SqlConnection connection, SqlTransaction transaction)
		{
			if(ids != null && ids.Count > 0)
			{
				int maxQueryNumber = Settings.MAX_BATCH_SIZE;
		...
		private static List<Infrastructure.Data.Entities.Tables.MTM.AdressenEntity> getWithTransaction(List<int> ids, SqlConnection connection, SqlTransaction transaction)
		{
			if(ids != null && ids.Count > 0)
			{
				var dataTable = new DataTable();
		...
		public static int InsertWithTransaction(Infrastructure.Data.Entities.Tables.MTM.AdressenEntity item, SqlConnection connection, SqlTransaction transaction)
		{
			//int response = int.MinValue;
		...
		public static int InsertWithTransaction(List<Infrastructure.Data.Entities.Tables.MTM.AdressenEntity> items, SqlConnection connection, SqlTransaction transaction)
		{
			if(items != null && items.Count > 0)
			{
				int maxParamsNumber = Settings.MAX_BATCH_SIZE / 41; // Nb params per query
		...
		private static int insertWithTransaction(List<Infrastructure.Data.Entities.Tables.MTM.AdressenEntity> items, SqlConnection connection, SqlTransaction transaction)
		{
			if(items != null && items.Count > 0)
			{
				string query = "";
		...
		public static int UpdateWithTransaction(Infrastructure.Data.Entities.Tables.MTM.AdressenEntity item, SqlConnection connection, SqlTransaction transaction)
		{
			int results = -1;
		...
		public static int UpdateWithTransaction(List<Infrastructure.Data.Entities.Tables.MTM.AdressenEntity> items, SqlConnection connection, SqlTransaction transaction)
		{
			if(items != null && items.Count > 0)
			{
				int maxParamsNumber = Settings.MAX_BATCH_SIZE / 41; // Nb params per query
		...
		private static int updateWithTransaction(List<Infrastructure.Data.Entities.Tables.MTM.AdressenEntity> items, SqlConnection connection, SqlTransaction transaction)
		{
			if(items != null && items.Count > 0)
			{
				//int results = -1;
		...
		public static List<int> GetForOrders()
		{
			var dataTable = new DataTable();
			using(var sqlConnection = new SqlConnection(Settings.ConnectionString))
			{
				sqlConnection.Open();
				string query = "SELECT Nr FROM [Adressen] WHERE Lieferantennummer Is Not Null AND IsNULL(sperren, 0)=0";
				var sqlCommand = new SqlCommand(query, sqlConnection);
		...
		public static List<Infrastructure.Data.Entities.Tables.MTM.AdressenEntity> GetForOrdersFiltered(List<int> ids, string filter)
		{
			if(ids != null && ids.Count > 0)
			{
				int maxQueryNumber = Settings.MAX_BATCH_SIZE;
				...
				if(ids.Count <= maxQueryNumber)
				{
					results = getForOrdersFiltered(ids, filter);
				}
				else
				{
					int batchNumber = ids.Count / maxQueryNumber;
					...
					for(int i = 0; i < batchNumber; i++)
					{
						results.AddRange(getForOrdersFiltered(ids.GetRange(i * maxQueryNumber, maxQueryNumber), filter));
					}
					results.AddRange(getForOrdersFiltered(ids.GetRange(batchNumber * maxQueryNumber, ids.Count - batchNumber * maxQueryNumber), filter));
				}
				return results.Take(10).ToList();
			}
			return new List<Infrastructure.Data.Entities.Tables.MTM.AdressenEntity>();
		}
		private static List<Infrastructure.Data.Entities.Tables.MTM.AdressenEntity> getForOrdersFiltered(List<int> ids, string filter)
		{
			if(ids != null && ids.Count > 0)
			{
				var dataTable = new DataTable();
				using(var sqlConnection = new SqlConnection(Settings.ConnectionString))
				{
					...
					{
						int y = 0;
						if(int.TryParse(filter, out y))
						{
							sqlCommand.CommandText += $" AND ( Lieferantennummer like '{y}%' OR Name1 Like '{filter.SqlEscape()}%')";

						}
						else
						{
							sqlCommand.CommandText += $" AND Name1 Like '{filter.SqlEscape()}%'";
						}
					}
					DbExecution.Fill(sqlCommand, dataTable);
				}
