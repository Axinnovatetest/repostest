...
namespace Infrastructure.Data.Access.Tables.MTM
{
	public class CapacityRequiredAccess
	{
		...
		public static int ArchiveByFa(int userId, int faId, int version)
		{
			...
			{
				sqlConnection.Open();
				string query = $"UPDATE [__MTM_CRP_CapacityRequired] SET [ArchiveTime]=GetDate(), [ArchiveUserId]=@ArchiveUserId, IsArchived=1 WHERE [SourceId] = @faId AND [Version]=@version AND ISNULL(IsArchived,0)=0";
				var sqlCommand = new SqlCommand(query, sqlConnection);
		...
		public static List<Infrastructure.Data.Entities.Tables.PRS.FertigungEntity> GetNew()
		{
			...
			using(var sqlConnection = new SqlConnection(Access.Settings.ConnectionString))
			{
				...
				string query =
	$"SELECT * FROM Fertigung WHERE ID IN (    SELECT DISTINCT f.ID     FROM dbo.Fertigung f   LEFT JOIN dbo.Artikel A ON A.[Artikel-Nr] = f.Artikel_Nr   LEFT JOIN dbo.WorkSchedule W ON W.Article_Id = A.[Artikel-Nr]    LEFT JOIN dbo.WorkScheduleDetails D ON D.WorkScheduleId = W.Id   LEFT JOIN ErpMTM.dbo.__MTM_CRP_CapacityRequired c ON f.Id = c.SourceId    WHERE TRIM(f.Kennzeichen) = 'offen'     AND W.Is_Active = 1     AND D.Id IS NOT NULL    AND c.Id IS NULL);";
		...
		public static List<int> GetIdsByIds(List<int> ids)
		{
			if(ids == null || ids.Count <= 0)
				return null;
	...
	$"FROM DBO.Artikel ar " +
	$"WHERE ar.[Artikel-Nr] IN ({string.Join(",", ids)})";
	...
	.Select(x =>Convert.ToInt32( x[0]))  // or use any separator you like
	.ToList();
			}
			else
			{
		...
		public static int ArchiveAll(int userId)
		{
			int results = -1;
			using(var sqlConnection = new SqlConnection(Settings.ConnectionStringMTM))
			{
				sqlConnection.Open();
				string query = $"UPDATE [__MTM_CRP_CapacityRequired] SET [ArchiveTime]=GetDate(), [ArchiveUserId]=@ArchiveUserId, IsArchived=1 WHERE ISNULL(IsArchived,0)=0";
				var sqlCommand = new SqlCommand(query, sqlConnection);
