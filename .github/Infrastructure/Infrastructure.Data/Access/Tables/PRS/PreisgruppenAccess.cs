...
namespace Infrastructure.Data.Access.Tables.PRS
{
	...
	public class PreisgruppenAccess
	{
		...
		public static int UpdateStatisticsSalesPriceTrans(List<Entities.Tables.PRS.PreisgruppenStatisticsEntity> data, string userName)
		{
			if(data == null || data.Count <= 0)
				return 0;
			...
			*/
			...
			using(var sqlConnection = new SqlConnection(Settings.ConnectionString))
			{
				...
				if(data.Count > maxCmdParams)
				{
					...
					{
						updateStatisticsSalesPrice(i, data.Take(new Range(i * maxCmdParams, (i + 1) * maxCmdParams)).ToList(), userName, sqlConnection, sqlTransaction);
					}
				}
				else
				{
					updateStatisticsSalesPrice(0, data, userName, sqlConnection, sqlTransaction);
				}
				// -
				sqlTransaction.Commit();
			}
		...
		internal static int updateStatisticsSalesPrice(int idx, List<Entities.Tables.PRS.PreisgruppenStatisticsEntity> data, string userName, SqlConnection sqlConnection, SqlTransaction sqlTransaction)
		{
			...
			//using(var sqlConnection = new SqlConnection(Settings.ConnectionString))
			{
				...
				string query = $@"/* BUTTON 15 */
								...
								/* 2023-01-18 - Update Erstmusterpreis?
								UPDATE [__BSD_ArtikelSalesExtension] SET Verkaufspreis = [prix1] 
										FROM #PrixModifier_{connectionId} AS Pr 
	                                    INNER JOIN (Artikel AS A INNER JOIN [__BSD_ArtikelSalesExtension] AS B ON A.[Artikel-Nr] = B.[ArticleNr]) 
	                                    ON Pr.artikelnummer = A.Artikelnummer;*/
	                                    ;";

				// - START
				//SqlTransaction sqlTransaction = sqlConnection.BeginTransaction();
				using(SqlCommand _sqlCommand = new SqlCommand(query, sqlConnection, sqlTransaction))
				{
					for(int i = 0; i < data.Count; i++)
					{
						_sqlCommand.Parameters.AddWithValue($"Artikelnummer_{i}", data[i].Artikelnummer == null ? (object)DBNull.Value : data[i].Artikelnummer.Trim());
