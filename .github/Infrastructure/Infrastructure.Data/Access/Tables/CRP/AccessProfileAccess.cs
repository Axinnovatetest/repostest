		...
		public static int Insert(Infrastructure.Data.Entities.Tables.CRP.AccessProfileEntity item)
		{
			...
			using(var sqlConnection = new SqlConnection(Settings.ConnectionString))
			{
				...
				using(var sqlCommand = new SqlCommand(query, sqlConnection, sqlTransaction))
				{
					...
					sqlCommand.Parameters.AddWithValue("FAUpdateCRP", item.FAUpdateCRP == null ? (object)DBNull.Value : item.FAUpdateCRP);
			...
			if(items != null && items.Count > 0)
			{
				...
				using(var sqlConnection = new SqlConnection(Settings.ConnectionString))
				{
					...
					foreach(var item in items)
					{
						...
						sqlCommand.Parameters.AddWithValue("FAUpdateCRP" + i, item.FAUpdateCRP == null ? (object)DBNull.Value : item.FAUpdateCRP);
