	...
	public class GetReportDelieveryNoteHandler: IHandleAsync<Identity.Models.UserModel, ResponseModel<byte[]>>
	{
		...
		public async Task<ResponseModel<byte[]>> HandleAsync()
		{
			try
			{
				...
				var reportModel = new Infrastructure.Services.Reporting.Models.Itext.CTS_LSReportModel(reportentity, order);
				reportModel.Items = orderItems?
					.Where(x => x.erledigt_pos != true)
					.OrderBy(x => x.Position)
					.Select(x => new Infrastructure.Services.Reporting.Models.Itext.CTC_LSReportItemsModel(x))
					.ToList();
				...
				responseBody = await Infrastructure.Services.Reporting.IText.CTS.GetLS(reportModel, footerData);
