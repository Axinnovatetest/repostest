	...
	public class GetReportHandler: IHandleAsync<Identity.Models.UserModel, ResponseModel<byte[]>>
	{
		...
		public async Task<ResponseModel<byte[]>> HandleAsync()
		{
			try
			{
				...
				var reportModel = new Psz.Core.CustomerService.Reporting.Models.CTS_ABReportModel(reportEntity, orderEntity, buyerEntity, addressEntity, itemsEntity?.Count > 0 ? itemsEntity?[0].USt ?? 0m : 0);
				var itemsReportModel = itemsEntity?.Select(x => new Psz.Core.CustomerService.Reporting.Models.CTS_ABReportItemsModel(x)).ToList();
				...
				var footerData = new Psz.Core.Reporting.Models.CoreDocFooterModel(reportEntity);
				...
				responseBody = await Psz.Core.CustomerService.Reporting.IText.GetItextPDF(new Psz.Core.CustomerService.Reporting.Models.ITextHeaderFooterProps
				{
					...
					BodyTemplate = "CTS_AB_Body",
					FooterCenterText = null,
					FooterData = footerData,
					FooterLeftText = null,
					FooterTemplate = "Core_Footer",
					HasFooter = true,
				...
				//Psz.Core.CustomerService.Reporting.IText.GetABReport(reportModel, footerData);
				//await Infrastructure.Services.Reporting.IText.CTS.GetAB(reportModel, footerData);
