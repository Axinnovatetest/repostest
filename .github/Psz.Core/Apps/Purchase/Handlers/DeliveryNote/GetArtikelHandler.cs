using Psz.Core.Apps.EDI.Models.Order.Element;
...
namespace Psz.Core.Apps.Purchase.Handlers.DeliveryNote
{
	public class GetArtikelHandler: IHandle<Identity.Models.UserModel, ResponseModel<OrderElementModel>>
	{
		...
		public GetArtikelHandler(Identity.Models.UserModel user, string data1, decimal data3)
		{
			this._user = user;
			this._data1 = data1;
			this._data3 = data3;
		...
		public ResponseModel<OrderElementModel> Handle()
		{
			try
			{
				...
				var ArtikelEntity = Infrastructure.Data.Access.Tables.PRS.ArtikelAccess.GetByNumber(this._data1);
				...
				if(ArtikelEntity != null)
				{
					responseBody = new OrderElementModel
					{
						...
						OpenQuantity_Quantity = this._data3,//Convert.ToDecimal(elementDb.Anzahl ?? 0),
						DesiredDate = DateTime.Now.AddDays(+30),//elementDb.DesiredDate ?? DateTime.Now.AddDays(+30),***
						DeliveryDate = null,//elementDb.Liefertermin,
						...
						VAT = Convert.ToDecimal(ArtikelEntity?.Umsatzsteuer ?? 0),//Convert.ToDecimal(elementDb.USt ?? 0),***
						...
						/*to check*/
						UnloadingPoint = string.Empty,//elementDb.Abladestelle,
													  //OpenQuantity_UnitPrice = 0,//Convert.ToDecimal(elementDb.Einzelpreis ?? 0),
													  // = 0,//Convert.ToDecimal(elementDb.Gesamtpreis ?? 0),
						...
						OriginalOrderQuantity = this._data3,//Convert.ToDecimal(elementDb.OriginalAnzahl ?? 0),
						...
						Note2 = string.Empty,//elementDb.Bemerkungsfeld2,
											 // >>>>>>>>>>>>>>>>
						ItemId = ArtikelEntity.ArtikelNr,
					...
					//calculations
					var fixedPrice = ArtikelEntity?.VKFestpreis ?? false;
					...
					//
					var vkUnitPrice = CalculateVkUnitPrice(fixedPrice,
						verkaufspreis,
					...
					var totalPrice = CalculateTotalPrice(responseBody.UnitPriceBasis,
						unitPrice,
						responseBody.OriginalOrderQuantity,
						responseBody.Discount);

					var vKTotalPrice = CalculateVkTotalPrice(responseBody.UnitPriceBasis,
						vkUnitPrice,
						responseBody.OriginalOrderQuantity);
					...
					responseBody.UnitPrice = vkUnitPrice;
