...
namespace Psz.Core.Apps.EDI.Handlers
{
	public partial class Order
	{
			...
			Core.Identity.Models.UserModel currentUser)
		{
			try
			{
				if(currentUser == null || (!currentUser.Access.Purchase.ModuleActivated && !currentUser.Access.CustomerService.ModuleActivated))
				{
				...
				var itemsDb = Infrastructure.Data.Access.Tables.PRS.ArtikelAccess.GetLikeNumberV2(searchText)
					.FindAll(e => e.Freigabestatus != "O").Where(x => !benchmarkKreis.Exists(y => y == x.CustomerPrefix));
