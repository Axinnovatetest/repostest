...
namespace Psz.Core.Apps.EDI.Handlers.Delfor.Production
{
	...
	using System.Linq;
	public class ConfirmCommandHandler: IHandle<Identity.Models.UserModel, ResponseModel<int>>
	{
		private Identity.Models.UserModel _user { get; set; }
		private Models.Delfor.Production.ValidateCommandModel _data { get; set; }
		public ConfirmCommandHandler(Identity.Models.UserModel user, Models.Delfor.Production.ValidateCommandModel command)
		{
			this._user = user;
			this._data = command;
		}
		public ResponseModel<int> Handle()
		{
			lock(Locks.DLF_ProductionLock)
			{
				try
				{
					Infrastructure.Data.Access.Tables.CTS.LineItemPlanAccess.ConfirmProduction(this._data.PositionId, 10, this._user.Id, DateTime.Now);
					...
					//	: null;
					//var URSarticleEntity = Infrastructure.Data.Access.Tables.PRS.ArtikelAccess.Get(this._data.OriginalArticleId ?? -1);
					...
					//	KundenIndex = itemEntity.Index_Kunde,
					//	Kunden_Index_Datum = itemEntity.Index_Kunde_Datum,
					...
					//Psz.Core.CustomerService.Helpers.SpecialHelper.Perform(fertigungEntity);

					////logging
					//var _log = new Helpers.LogHelper((int)fertigungEntity.Fertigungsnummer, 0, 0, "Fertigung", Helpers.LogHelper.LogType.CREATIONFA, "EDI|DELFOR", _user)
		...
		public ResponseModel<int> Validate()
		{
			...
			if(!horizonCheck && !CustomerService.Helpers.HorizonsHelper.ArticleIsTechnic(itemEntity.ArtikelNr))
