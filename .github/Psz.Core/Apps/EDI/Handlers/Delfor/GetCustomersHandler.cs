...
namespace Psz.Core.Apps.EDI.Handlers.Delfor
{
	...
	using System.Linq;
	public class GetCustomersHandler: IHandle<Identity.Models.UserModel, ResponseModel<List<Models.Delfor.GetCustomerModel>>>
	{
		private string _searchText { get; set; }
		private Identity.Models.UserModel _user { get; set; }

		public GetCustomersHandler(Identity.Models.UserModel user, string searchText)
		{
			this._user = user;
		...
		public ResponseModel<List<Models.Delfor.GetCustomerModel>> Handle()
		{
			try
			{
				...
				var errorEntites = Infrastructure.Data.Access.Tables.CTS.ErrorAccess.GetByBuyerDUNS(duns, false);
				...
				if(!string.IsNullOrEmpty(_searchText) && !string.IsNullOrWhiteSpace(_searchText))
					response = response.Where(r => r.CustomerNumber.ToString().Contains(_searchText.ToLower())
					|| r.CustomerName.ToLower().Contains(_searchText.ToLower())).ToList();
