...
namespace Psz.Core.Apps.WorkPlan.Handlers
{
	public partial class WorkStationMachine
	{
		public static Core.Models.ResponseModel<byte[]> ExportToExcel()
		{
			try
			{
				var wStationEntities = Infrastructure.Data.Access.Tables.WPL.WorkStationMachineAccess.Get()
					?.Where(x => !x.DeleteUserId.HasValue)?.ToList();
				if(wStationEntities == null || wStationEntities.Count <= 0)
					return null;

				return Core.Models.ResponseModel<byte[]>.SuccessResponse(SaveToExcelFile(wStationEntities));
		...
		internal static byte[] SaveToExcelFile(
			List<Infrastructure.Data.Entities.Tables.WPL.WorkStationMachineEntity> workStations)
		{
			try
			{
				workStations = workStations.OrderBy(x => x.CountryId)?.ThenBy(x => x.HallId)?.ThenBy(x => x.WorkAreaId)?.ThenBy(x => x.Name)?.ToList();
				var tempFolder = System.IO.Path.GetTempPath();
				var filePath = System.IO.Path.Combine(tempFolder, $"workStations-{DateTime.Now.ToString("yyyyMMddTHHmm")}.xlsx");
				...
				using(var package = new ExcelPackage(file))
				{
					...
					ExcelWorksheet worksheet = package.Workbook.Worksheets.Add($"Work Stations");
					...
					worksheet.Cells[1, 1].Value = $"Work Stations {DateTime.Now.ToString("dd.MM.yyyy HH:mm")}";
					...
					worksheet.Cells[headerRowNumber, startColumnNumber + 3].Value = "Work Area";
					worksheet.Cells[headerRowNumber, startColumnNumber + 4].Value = "Type";
					...
					foreach(var w in workStations)
					{
						...
						var wa = workAreas.Find(x => x.Id == w.WorkAreaId);
						...
						worksheet.Cells[rowNumber, startColumnNumber + 3].Value = wa?.Name;
						worksheet.Cells[rowNumber, startColumnNumber + 4].Value = w?.Type == 1 ? "Machine" : "Station";
					...
					package.Workbook.Properties.Title = "Work Stations";
