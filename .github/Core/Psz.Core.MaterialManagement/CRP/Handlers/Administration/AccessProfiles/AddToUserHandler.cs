...
namespace Psz.Core.MaterialManagement.Handlers.Administration.AccessProfiles
{
	public class AddToUserHandler: IHandle<Identity.Models.UserModel, ResponseModel<int>>
	{
		...
		public AddToUserHandler(Identity.Models.UserModel user, Models.Administration.AccessProfiles.AddToUserRequestModel data)
		{
			this._user = user;
			this._data = data;
		}
		...
		public ResponseModel<int> Handle()
		{
			try
			{
				...
				var existingUserProfiles = Infrastructure.Data.Access.Tables.MTM.AccessProfileUsersAccess.GetByUserId(new List<int> { this._data.UserId });
				...
				if(newUsers != null && newUsers.Count > 0)
				{
					insertedUsers = Infrastructure.Data.Access.Tables.MTM.AccessProfileUsersAccess.Insert(
						newUsers.Select(x => new Infrastructure.Data.Entities.Tables.MTM.AccessProfileUsersEntity
						{
