	...
	public class GetHandler: IHandle<GetRequestModel, ResponseModel<IPaginatedResponseModel<GetResponseModel>>>
	{
		...
		private ResponseModel<IPaginatedResponseModel<GetResponseModel>> Perform(UserModel user, GetRequestModel data)
		{
			...
			if(!string.IsNullOrWhiteSpace(data.OrderType))
			{
				...
				{
					//dataFilter.Add(new Infrastructure.Data.Access.Settings.FilterModel { FilterFieldName = "a.[Rahmenbestellung]", FirstFilterValue = "1", FilterType = Infrastructure.Data.Access.Settings.FilterTypes.Number });
			...
			if(dataResponse == null || dataResponse.Count == 0)
				return ResponseModel<IPaginatedResponseModel<GetResponseModel>>.NotFoundResponse();
			...
			return ResponseModel<IPaginatedResponseModel<GetResponseModel>>.SuccessResponse(
				new IPaginatedResponseModel<GetResponseModel>
				{
					...
					TotalCount = dataResponse.FirstOrDefault().TotalCount,
					TotalPageCount = this.data.PageSize > 0 ? (int)Math.Ceiling(((decimal)(dataResponse.FirstOrDefault().TotalCount > 0 ? dataResponse.FirstOrDefault().TotalCount : 0) / this.data.PageSize)) : 0
				}
				);
		}
