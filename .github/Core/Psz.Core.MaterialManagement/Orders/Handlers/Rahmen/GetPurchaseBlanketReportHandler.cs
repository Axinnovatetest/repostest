using Infrastructure.Services.Reporting.Models.Itext;
using Psz.Core.MaterialManagement.Orders.Models.Rahmen;
using Psz.Core.SharedKernel.Interfaces;

using System.Linq;
using System.Threading.Tasks;

namespace Psz.Core.MaterialManagement.Orders.Handlers.Rahmen
{
	public class GetPurchaseBlanketReportHandler: IHandleAsync<GetPurchaseBlanketReportRequestModel, ResponseModel<byte[]>>
	{
		private UserModel _user { get; set; }
		private GetPurchaseBlanketReportRequestModel _data { get; set; }
		public GetPurchaseBlanketReportHandler(UserModel user, GetPurchaseBlanketReportRequestModel data)
		{
			_user = user;
			_data = data;
		}
		public async Task<ResponseModel<byte[]>> HandleAsync()
		{
			try
			{
				var validationResponse = await this.ValidateAsync();
				if(!validationResponse.Success)
				{
					return validationResponse;
				}

				var reportData = new RahmenReportModel();
				var rahmen = Infrastructure.Data.Access.Tables.PRS.AngeboteAccess.Get(_data.OrderId);
				var rahmenItems = Infrastructure.Data.Access.Tables.PRS.AngeboteneArtikelAccess.GetByAngeboteNr(_data.OrderId, false);

				var rahmenExtension = Infrastructure.Data.Access.Tables.CTS.AngeboteBlanketExtensionAccess.GetByAngeboteNr(_data.OrderId);
				var rahmenItemsExtensions = Infrastructure.Data.Access.Tables.CTS.AngeboteArticleBlanketExtensionAccess.GetByRahmenNr(_data.OrderId);

				var supplier = Infrastructure.Data.Access.Tables.BSD.LieferantenAccess.Get(rahmenExtension.SupplierId ?? -1);
				var supplierAdress = Infrastructure.Data.Access.Tables.PRS.AdressenAccess.Get(supplier?.Nummer ?? -1);
				// Change this to MTM.???
				var user = Infrastructure.Data.Access.Tables.COR.UserAccess.Get(rahmenExtension.CreateUserId);
				var reporting = Infrastructure.Data.Access.Tables.PRS.OrderReportAccess.GetByLanguageAndType(4, 3);
				string logo = "";
				var psz = Infrastructure.Data.Access.Tables.STG.CompanyAccess.GetFirst();
				//REM:  - logo imported in STG should be PNG
				logo = $"data:image/png;base64,{Convert.ToBase64String(psz?.Logo)}";
				if(string.IsNullOrEmpty(logo))
				{
					// UNDONE : Fix logo
					logo = "data:image/png;base64,";
				}

				var articleEntities = Infrastructure.Data.Access.Tables.PRS.ArtikelAccess.Get(rahmenItems?.Select(p => p.ArtikelNr ?? -1)?.ToList())
					?? new List<Infrastructure.Data.Entities.Tables.PRS.ArtikelEntity>();
				var rahmenModel = new Psz.Core.MaterialManagement.Reporting.Models.RahmenModel
				{
					Adress1 = rahmen.Anrede,
					Adress2 = rahmen.Vorname_NameFirma,
					Adress3 = rahmen.Straße_Postfach,
					Adress4 = rahmen.Land_PLZ_Ort,
					Adress5 = supplierAdress.Telefon,
					Adress6 = supplierAdress.Fax,
					Adress7 = user.Name,
					Adress8 = user.TelephoneMobile,
					Adress9 = user.Fax,
					Adress10 = user.Email,

					Nr = rahmen.Bezug,
					Date = rahmen.Datum.HasValue == true ? rahmen.Datum.Value.ToString("dd.MM.yyyy") : "",
					ExtensionDate = rahmen.Datum.HasValue == true ? rahmen.Datum.Value.AddDays(3).ToString("dd.MM.yyyy") : "",
					TradingTerm = rahmen.Versandart,
					PayementTerm = rahmen.Konditionen,
					Customer = rahmen.Ihr_Zeichen,
					Supplier = rahmen.Unser_Zeichen,
					Payement = rahmen.Zahlungsweise,
					Logo = logo,
					Positions = rahmenItems?.OrderBy(x => x.Position)?.ThenBy(x => x.ArtikelNr)?.Select(p =>
					new Psz.Core.MaterialManagement.Reporting.Models.RahmenModel.RahmenPositionsModel
					{
						Position = p.Position.ToString(),
						Artikel = articleEntities.FirstOrDefault(x => x.ArtikelNr == p.ArtikelNr)?.ArtikelNummer,
						Description = articleEntities.FirstOrDefault(x => x.ArtikelNr == p.ArtikelNr)?.Bezeichnung1,
						Quantity = p.Anzahl ?? 0m,
						UOM = p.Einheit,
						Price = p.Preiseinheit.HasValue ? p.Preiseinheit.Value.FormatDecimal(2) : "0",
						Price_UOM = p.Einzelpreis ?? 0m,
						VAT = p.USt ?? 0m,
						Total = p.Gesamtpreis ?? 0m,
						WishData = p.Liefertermin.HasValue ? p.Liefertermin.Value.ToString("dd.MM.yyy") : "",
						Bestellnummer = p.Bestellnummer,
						LT = p.Liefertermin.HasValue ? p.Liefertermin.Value.ToString("dd.MM.yyy") : "",
						MengeImSicherheitslager = _data.SecurityStock
					}).ToList()
				};
				//Footer
				var footerData = new Psz.Core.MaterialManagement.Reporting.Models.DocFooterModel(reporting);
				var response = await Reporting.IText.GetItextPDF(new Reporting.Models.ITextHeaderFooterProps
				{
					BodyData = rahmenModel,
					BodyTemplate = "MTM_RA_Body",
					DocumentTitle = "",
					FooterCenterText = "",
					FooterData = footerData,
					FooterLeftText = "",
					FooterTemplate = "MTM_Footer",
					FooterWithCounter = false,
					HasFooter = true,
					HasHeader = true,
					HeaderFirstPageOnly = false,
					HeaderLogoWithCounter = true,
					HeaderLogoWithText = false,
					HeaderText = "",
					Logo = logo,
					Rotate = false
				});
				//await Psz.Core.MaterialManagement.Reporting.IText.GetRahmenReport(rahmenModel, footerData);
				//await Infrastructure.Services.Reporting.IText.CTS.GetRahmen(rahmenModel, footerData);
				return ResponseModel<byte[]>.SuccessResponse(response);
			} catch(Exception e)
			{
				Infrastructure.Services.Logging.Logger.Log(e);
				throw;
			}
		}
		public Task<ResponseModel<byte[]>> ValidateAsync()
		{
			if(this._user == null/*this._user.Access.____*/)
			{
				return ResponseModel<byte[]>.AccessDeniedResponseAsync();
			}
			var rahmenExtension = Infrastructure.Data.Access.Tables.CTS.AngeboteBlanketExtensionAccess.GetByAngeboteNr(_data.OrderId);
			if(rahmenExtension.StatusId == (int)Enums.BlanketEnums.RAStatus.Draft || rahmenExtension.StatusId == (int)Enums.BlanketEnums.RAStatus.InProgress)
				return ResponseModel<byte[]>.FailureResponseAsync($"RA is {rahmenExtension.StatusName}, Report generation not permitted");
			return ResponseModel<byte[]>.SuccessResponseAsync();
		}
	}
}