/* this sql job will be executed every day in order to fill dashboard table with data till today*/

/* Insert a line in sync table  (delete old sync with same date if exists)*/
--DELETE FROM [stats].[Sync] where SyncDatum=Convert(date, getdate())  
...
WITH TBL_0002 AS (
				SELECT ClientTable.Stufe, Artikel.[Artikel-Nr], Artikel.Artikelnummer, Sum([angebotene Artikel].Anzahl) AS [Bedarf AB], 
				...
				WHERE (((Angebote.Typ) Like 'Auftrags%') AND ((Angebote.erledigt)=0) AND (([angebotene Artikel].erledigt_pos)=0) 
						AND ((Angebote.[Kunden-Nr]) IN (select [clientNumber] from #ClientTable))
					)
				GROUP BY Artikel.[Artikel-Nr], Artikel.Artikelnummer, ClientTable.[clientNumber], ClientTable.[Stufe],
						IIf(([Angebotene Artikel].[Liefertermin]>=2200 OR [Angebotene Artikel].[Liefertermin] IS NULL),
								[Angebotene Artikel].[Wunschtermin],
