...
namespace Psz.Core.ManagementOverview.Administration.Handlers.AccessProfiles
{
	public class AddToUserHandler: IHandle<Identity.Models.UserModel, ResponseModel<int>>
	{
		...
		public AddToUserHandler(Identity.Models.UserModel user, Administration.Models.AccessProfiles.AddToUserRequestModel data)
		{
			this._user = user;
			this._data = data;
		}
		...
		public ResponseModel<int> Handle()
		{
			try
			{
				...
				var existingUserProfiles = Infrastructure.Data.Access.Tables.MGO.AccessProfileUsersAccess.GetByUserId(new List<int> { this._data.UserId });
				...
				if(newUsers != null && newUsers.Count > 0)
				{
					insertedUsers = Infrastructure.Data.Access.Tables.MGO.AccessProfileUsersAccess.Insert(
						newUsers.Select(x => new Infrastructure.Data.Entities.Tables.MGO.AccessProfileUsersEntity
						{
