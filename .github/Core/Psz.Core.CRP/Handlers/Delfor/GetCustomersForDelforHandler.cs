...
namespace Psz.Core.CRP.Handlers.Delfor
{
	...
	{
		public ResponseModel<List<CustomerModel>> GetCustomersForDelfor(UserModel user)
		{
			try
			{
				var validationResponse = this.ValidateGetCustomersForDelfo(user);
				...
				{
					var userCustomers = Infrastructure.Data.Access.Tables.PRS.CustomerUserAccess.GetBy_IsPrimary_UserId(true, user.Id);
		...
		public ResponseModel<List<CustomerModel>> ValidateGetCustomersForDelfo(UserModel user)
		{
