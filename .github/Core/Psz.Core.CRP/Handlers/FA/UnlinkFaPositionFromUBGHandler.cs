...
{
	public class UnlinkFaPositionFromUBGHandler: IHandle<Identity.Models.UserModel, ResponseModel<int>>
	{
		private LinkFaPositionHBGRequestModel _data { get; set; }
		private Identity.Models.UserModel _user { get; set; }
		public UnlinkFaPositionFromUBGHandler(LinkFaPositionHBGRequestModel data, Identity.Models.UserModel user)
		{
			this._user = user;
			this._data = data;
		}
		public ResponseModel<int> Handle()
		{
			...
			try
			{
				...
				logs.Add(new Helpers.LogHelper(ubgFa.Fertigungsnummer ?? -1, 0, 0, "Fertigung", Helpers.LogHelper.LogType.MODIFICATIONFA, "CTS", _user)
						.LogCTS("HBGFAPositionId", $"{ubgFa.HBGFAPositionId}", $"{null}", 0));
				ubgFa.HBGFAPositionId = null;
				...
				logs.Add(new Helpers.LogHelper(hbgFa.Fertigungsnummer ?? -1, 0, 0, "Fertigung", Helpers.LogHelper.LogType.MODIFICATIONFA, "CTS", _user)
						.LogCTS("UBGFertigungsId", $"{hbgFaPosition.UBGFertigungsId}", $"{null}", 0));
				logs.Add(new Helpers.LogHelper(hbgFa.Fertigungsnummer ?? -1, 0, 0, "Fertigung", Helpers.LogHelper.LogType.MODIFICATIONFA, "CTS", _user)
						.LogCTS("UBGFertigungsnummer", $"{hbgFaPosition.UBGFertigungsnummer}", $"{null}", 0));
				hbgFaPosition.UBGFertigungsId = null;
				hbgFaPosition.UBGFertigungsnummer = null;
		...
		public ResponseModel<int> Validate()
		{
			...
			if(ubgFaEntity == null)
			{
				return ResponseModel<int>.FailureResponse($"UBG FA not found");
			}
