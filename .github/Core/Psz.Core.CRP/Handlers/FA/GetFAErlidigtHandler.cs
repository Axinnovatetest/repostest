	...
	public class GetFAErlidigtHandler: IHandle<Identity.Models.UserModel, ResponseModel<int>>
	{
		...
		public ResponseModel<int> Handle()
		{
			try
			{
				...
				var isWarehouseInInventory = Infrastructure.Data.Access.Tables.BSD.LagerorteAccess.IsInInventory(faEntity.Lagerort_id ?? 0);
				if(isWarehouseInInventory)
					return ResponseModel<int>.FailureResponse($"Warehouse [{faEntity.Lagerort_id}] is in inventory, closing FA is not allowed.");
