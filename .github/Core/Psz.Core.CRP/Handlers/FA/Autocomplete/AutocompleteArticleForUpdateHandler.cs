...
{
	public class AutocompleteArticleForUpdateHandler: IHandle<Identity.Models.UserModel, ResponseModel<List<KeyValuePair<string, string>>>>
	{
		private string _data { get; set; }
		private Identity.Models.UserModel _user { get; set; }
		public AutocompleteArticleForUpdateHandler(string data, Identity.Models.UserModel user)
		{
			this._user = user;
			this._data = data;
		}
		public ResponseModel<List<KeyValuePair<string, string>>> Handle()
		{
			try
			{
				...
				var articlesEntity = Infrastructure.Data.Access.Joins.FAPlannung.FAPlannungAccess.GetLikeNumberWithFA(this._data);
				...
				if(articlesEntity != null && articlesEntity.Count > 0)
				{
					...
					{
						response.Add(new KeyValuePair<string, string>(item, item));
					}
				}
