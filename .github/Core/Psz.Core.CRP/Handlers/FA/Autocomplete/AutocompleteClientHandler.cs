...
{
	public class AutocompleteClientHandler: IHandle<Identity.Models.UserModel, ResponseModel<List<KeyValuePair<string, string>>>>
	{
		private string _data { get; set; }
		private Identity.Models.UserModel _user { get; set; }

		public AutocompleteClientHandler(string data, Identity.Models.UserModel user)
		{
			this._user = user;
			this._data = data;
		}
		public ResponseModel<List<KeyValuePair<string, string>>> Handle()
		{
			try
			{
				...
				var articlesEntity = Infrastructure.Data.Access.Tables.CTS.PSZ_Nummerschlüssel_KundeAccess.GetLikeKunde(this._data);
				...
				if(articlesEntity != null && articlesEntity.Count > 0)
				{
					...
					{
						response.Add(new KeyValuePair<string, string>(item.Nummerschlüssel, item.Kunde));
					}
				}
