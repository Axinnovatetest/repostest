...
namespace Psz.Core.CRP.Handlers.FA.Purchase
{
	public class WunshUpdate
	{
		...
		public async static Task<byte[]> GetReport(int id, Identity.Models.UserModel user)
		{
				...
				if(updateEntity != null)
				{
					...
					if(_right)
					{
						...
						var NotUpdated_list = (UpdateDetails != null && UpdateDetails.Count > 0) ? Infrastructure.Data.Access.Joins.FAExcelUpdate.FAWunshUpdateAccess.GetUpdatedWunshAdmin(joint, 1, botransaction.connection, botransaction.transaction)
							: new List<Infrastructure.Data.Entities.Joins.FAExcelUpdate.FAWunshUpdateEntity> { };
						var Updated = Updated_list?.Select(x => new Reporting.Models.FAUpdatedFromExcelModel(x)).ToList();
						var Notupdated = NotUpdated_list?.Select(x => new Reporting.Models.FAUpdatedFromExcelModel(x)).ToList();

						var Lists = new Reporting.Models.FAWunshUpdateReporAdmintModel { IdUpdate = updateEntity.Id, Updated = Updated, NotUpdated = Notupdated };
						response = await Reporting.IText.GetItextPDF(new Reporting.Models.ITextHeaderFooterProps
						{
							BodyData = Lists,
							BodyTemplate = "CRP_FAUpdateWunshAdmin",
							DocumentTitle = "",
						...
						//Module.CRP_ReportingService.GenerateWunshTerminAdminUpdateReport(Infrastructure.Services.Reporting.Helpers.ReportType.CTS_WUNSH_TERMIN_ADMIN, Lists);
						//}
					}
					else
					{
						...
						: new List<Infrastructure.Data.Entities.Joins.FAExcelUpdate.FANotUpdatedwunshUserEntity> { };
						var Updated = Updated_list?.Select(x => new Reporting.Models.FAUpdatedFromExcelModel(x)).ToList();
						var Notupdated = NotUpdated_list?.Select(x => new Reporting.Models.FANotUpdatedwunshUserModel(x)).ToList();

						var Lists = new Reporting.Models.FAWunshUpdateReporUsertModel() { Updated = Updated, NotUpdated = Notupdated, IdUpdate = id };
						response = response = await Reporting.IText.GetItextPDF(new Reporting.Models.ITextHeaderFooterProps
						{
							BodyData = Lists,
							BodyTemplate = "CRP_FAUpdateWunshUser",
							DocumentTitle = "",
							FooterBgColor = new iText.Kernel.Colors.DeviceRgb(220, 220, 220),
							FooterCenterText = "",
							FooterCenterText2 = "",
						...
						//Module.CRP_ReportingService.GenerateWunshTerminUserUpdateReport(Infrastructure.Services.Reporting.Helpers.ReportType.CTS_WUNSH_TERMIN_USER, Lists);
						//}
					}
				}
