				...
				else
				{
					...
					if(FAWerkUpdates != null && FAWerkUpdates.Body.Count > 0)
					{
						...
						foreach(var item in FAWerkUpdates.Body)
						{
							...
							{
								Notupdated.Add(new Infrastructure.Services.Reporting.Models.CTS.FANotUpdatedFromExcelModel(fa, "Termin Werk already updated"));
							}
							else
							{
								if(fa.Termin_Bestatigt2 == null)
								{
									Notupdated.Add(new Infrastructure.Services.Reporting.Models.CTS.FANotUpdatedFromExcelModel(fa, "Cannot update empty Termin Werk"));
								}
								else
								{
									toUpddate.Add(item);
