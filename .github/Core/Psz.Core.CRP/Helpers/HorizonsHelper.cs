...
namespace Psz.Core.CRP.Helpers
{
	public class HorizonsHelper
	{
		...
		public static int GetIsoWeekNumber(DateTime date)
		{
			...
			//int isoWeek = calendar.GetWeekOfYear(date, weekRule, firstDayOfWeek);

			//// ISO week has the first week containing January 4th
			//if(date.Month == 12 && isoWeek == 1)
			//{
			//	DateTime weekStart = new DateTime(date.Year + 1, 1, 4);
			//	if(weekStart.DayOfWeek <= DayOfWeek.Thursday)
			//	{
			//		return 1;
			//	}
			//}
			//else if(date.Month == 1 && isoWeek >= 52)
			//{
			//	DateTime weekStart = new DateTime(date.Year - 1, 12, 28);
			//	if(weekStart.DayOfWeek <= DayOfWeek.Thursday)
			//	{
			//		return isoWeek;
			//	}
