...
namespace Psz.Core.CRP.Helpers
{
	public class CRPHelper
	{
		...
		internal static bool ValidateKWYear(string input)
		{
			...
			var maxWeek = GetMaxIsoWeek(int.TryParse(parts[1], out int mx) ? mx : 0);
			if(parts.Length != 2)
				return false;
		...
		public static int GetMaxIsoWeek(int year)
		{
			// ISO week number of the last Thursday of the year
		...
		private static void CorrectFA(FASystemModel faSystem, WeekQuantityModel item, bool inFrozenZone = false)
		{
			try
			{
				...
				if(index >= 0 && index <= faSystem.FAMovement.Count - 1)
		...
		public static KeyValuePair<decimal, decimal> GetFAPriceAndTime(int articleId, int faQty)
		{
			var salesPriceType = Psz.Core.CustomerService.Enums.OrderEnums.ConvertToMTDSalesItemType("serie");
			var salesPriceExtention = Infrastructure.Data.Access.Tables.BSD.ArtikelSalesExtensionAccess.GetByArticleNrAndTypeId(articleId, (int)salesPriceType);
			var priceGroupEntity = Infrastructure.Data.Access.Tables.PRS.PreisgruppenAccess.GetByArtikelNr(articleId);
			var priceType = salesPriceType != Common.Enums.ArticleEnums.SalesItemType.Serie ? "" : getPriceType(priceGroupEntity, Convert.ToDecimal(faQty));
			var staffPriceEntity = Infrastructure.Data.Access.Tables.PRS.StaffelpreisKonditionzuordnungAccess.GetByArtikelNrAndType(articleId, priceType);
			var itemCalculatoryCostsEntity = Infrastructure.Data.Access.Tables.PRS.ArtikelKalkulatorischeKostenAccess.GetArbeitskostenByArtikelNr(articleId);
			...
			return new KeyValuePair<decimal, decimal>(price, time);
		}
