...
{
	public class GetAllocationVSExpensesByCompanyHandler: IHandle<UserModel, ResponseModel<AmountVsExpensesModel>>
	{
		private readonly UserModel _user;
		private readonly StatsRequestModel _data;

		public GetAllocationVSExpensesByCompanyHandler(UserModel user, StatsRequestModel data)
		{
			_user = user;
			_data = data;
		}
		public ResponseModel<AmountVsExpensesModel> Handle()
		{
			...
			var allocation = Infrastructure.Data.Access.Joins.FNC.Statistics.StatsticsAccess.GetAmountTotal(Infrastructure.Data.Access.Enums.FNCEnums.Amountname.AmountInitial,
				 Infrastructure.Data.Access.Enums.FNCEnums.AmountTable.__FNC_BudgetAllocationCompany,
				  _data.CompanyId ?? -1,
				  _data.Year ?? -1);
			...
			if(_data.CompanyId != null)
			{
				orderEntities = orderEntities.Where(y => (y.CompanyId == _data.CompanyId))?.ToList();
				orderLeasingEntities = orderLeasingEntities.Where(x => x.CompanyId == _data.CompanyId)?.ToList();
			}
			...
			var expenses = Helpers.Processings.Budget.Order.getItemsAmount(
							...
							.Any(z => z.OrderId == x.OrderId) == true).ToList(), false, true, 0)
							+
