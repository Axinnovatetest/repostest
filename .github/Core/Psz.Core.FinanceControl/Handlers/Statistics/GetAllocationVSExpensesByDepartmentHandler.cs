...
{
	public class GetAllocationVSExpensesByDepartmentHandler: IHandle<UserModel, ResponseModel<AmountVsExpensesModel>>
	{
		private readonly UserModel _user;
		private readonly StatsRequestModel _data;

		public GetAllocationVSExpensesByDepartmentHandler(UserModel user, StatsRequestModel data)
		{
			_user = user;
			_data = data;
		}
		public ResponseModel<AmountVsExpensesModel> Handle()
		{
			...
			if(_data.DepartmentId == null)
				return ResponseModel<AmountVsExpensesModel>.SuccessResponse(new AmountVsExpensesModel
				{
					Allocation = 0,
					Expenses = 0
				});
			var allocation = Infrastructure.Data.Access.Joins.FNC.Statistics.StatsticsAccess.GetAmountTotal(Infrastructure.Data.Access.Enums.FNCEnums.Amountname.AmountInitial,
				Infrastructure.Data.Access.Enums.FNCEnums.AmountTable.__FNC_BudgetAllocationDepartment, _data.DepartmentId ?? -1, _data.Year ?? -1);
			...
			if(_data.DepartmentId != null)
			{
				orderEntities = orderEntities.Where(y => (y.DepartmentId == _data.DepartmentId))?.ToList();
				orderLeasingEntities = orderLeasingEntities.Where(x => x.DepartmentId == _data.DepartmentId)?.ToList();
			}
