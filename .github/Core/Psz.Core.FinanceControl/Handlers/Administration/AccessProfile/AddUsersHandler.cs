...
namespace Psz.Core.FinanceControl.Handlers.Administration.AccessProfile
{
	...
	public class AddUsersHandler: IHandle<Identity.Models.UserModel, ResponseModel<int>>
	{
		...
		public AddUsersHandler(Identity.Models.UserModel user, Models.Administration.AccessProfile.AddUsersModel data)
		{
			this._user = user;
			this._data = data;
		}
		...
		public ResponseModel<int> Handle()
		{
			try
			{
				...
				/// 
				var profileEntity = Infrastructure.Data.Access.Tables.FNC.AccessProfileAccess.Get(this._data.ProfileId);
				var profileUsers = Infrastructure.Data.Access.Tables.FNC.UserAccessProfilesAccess.GetByAccessProfileIds(new List<int> { this._data.ProfileId });
				...
				if(newUsers != null && newUsers.Count > 0)
				{
					...
					if(userEntities != null && userEntities.Count > 0)
					{
						insertedUsers = Infrastructure.Data.Access.Tables.FNC.UserAccessProfilesAccess.Insert(
							userEntities.Select(x => new Infrastructure.Data.Entities.Tables.FNC.UserAccessProfilesEntity
							{
