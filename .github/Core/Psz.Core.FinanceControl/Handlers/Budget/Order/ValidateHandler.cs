		...
		public ResponseModel<int> Validate()
		{
			...
			else // internal project
			{
				...
				if(this._data.CurrentStep == (int)Enums.BudgetEnums.ValidationLevels.Draft)
				{
					...
					var orders = Infrastructure.Data.Access.Tables.FNC.BestellungenExtensionAccess.GetByOrderIds(articleEntities?.Select(x => x.OrderId)?.ToList(), null);
					...
					orders = Infrastructure.Data.Access.Tables.FNC.BestellungenExtensionAccess.GetByOrderIds(articleEntities.Select(x => x.OrderId)?.ToList(), null);
					foreach(var item in articleEntities)
					{
						var order = orders?.FirstOrDefault(x => x.OrderId == item.OrderId);
						if(order != null && order.Discount.HasValue && order.Discount.Value > 0)
