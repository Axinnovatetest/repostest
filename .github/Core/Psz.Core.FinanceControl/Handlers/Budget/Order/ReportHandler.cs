...
namespace Psz.Core.FinanceControl.Handlers.Budget.Order
{
	...
	public class ReportHandler: IHandle<UserModel, ResponseModel<string>>
	{
		...
		public ResponseModel<string> Validate()
		{
			...
			//var orderArticleEntities = Infrastructure.Data.Access.Tables.FNC.Article_OrderAccess.GetByOrderId(this._data);
			//if (orderArticleEntities == null || orderArticleEntities.Count <= 0)
			//    return ResponseModel<string>.FailureResponse(key: "1", value: "Order articles list empty");

			// - User access checks
			//if (orderEntity.IssuerId != this._user.Id)
			//{
			//    if(orderEntity.OrderType.ToLower() == "external")
			//    {
			//        var validatorEntities = Infrastructure.Data.Access.Tables.FNC.Validators_ProjectAccess.GetByProjectId(orderEntity.ProjectId ?? -1);
			//        if (validatorEntities == null || validatorEntities.Count <= 0)
			//            return ResponseModel<string>.FailureResponse(key: "1", value: "Empty validator list");

			//        var validatorEntity = validatorEntities.Find(x => x.Id_Validator == this._user.Id);
			//        if (validatorEntity == null)
			//            return ResponseModel<string>.FailureResponse(key: "1", value: "User not found as validator");
			//    }
			//    else // internal order
			//    {
			...
			//        if (deptDirectors?.HeadUserId != this._user.Id && siteDirector.DirectorId != this._user.Id)
			//            return ResponseModel<string>.FailureResponse(key: "1", value: "User not found as validator");
			//    }
			//}
