...
namespace Psz.Core.FinanceControl.Handlers.Budget.Allocation
{
	...
	public class AddForCompanyHandler: IHandle<Identity.Models.UserModel, ResponseModel<int>>
	{
		...
		public AddForCompanyHandler(Identity.Models.UserModel user, Models.Budget.Allocation.Company.UpdateModel model)
		{
			this._user = user;
			this._data = model;
		}
		...
		public ResponseModel<int> Handle()
		{
			try
			{
				...
				this._data.AmountAllocatedToDepartments = 0;
				this._data.AmountAllocatedToProjects = 0;
				this._data.AmountSupplements = 0;
				this._data.AmountSpent = 0;
				this._data.CompanyId = companyExtension.CompanyId;
				var insertedId = Infrastructure.Data.Access.Tables.FNC.BudgetAllocationCompanyAccess.Insert(this._data.ToEntity());
				...
				{
					var company = Infrastructure.Data.Access.Tables.STG.CompanyAccess.Get(companyExtension.CompanyId);
					#region Email
					var emailTitle = "";
					var emailContent = "";
...
"))}
		...
		public ResponseModel<int> Validate()
		{
			...
			var companyExtension = Infrastructure.Data.Access.Tables.FNC.CompanyExtensionAccess.Get(this._data.CompanyId);
			if(Infrastructure.Data.Access.Tables.STG.CompanyAccess.Get(companyExtension.CompanyId) == null)
				return ResponseModel<int>.FailureResponse("Company not found");


			var allocations = Infrastructure.Data.Access.Tables.FNC.BudgetAllocationCompanyAccess.GetByCompanyAndYear(companyExtension.CompanyId, this._data.Year);
			if(allocations != null)
			{
			...
			// - Company's Leasings
			var leasingEntities = Infrastructure.Data.Access.Tables.FNC.BestellungenExtensionAccess.GetLeasingByYearAndCompanies(this._data.Year, new List<int> { companyExtension.CompanyId }, null)
