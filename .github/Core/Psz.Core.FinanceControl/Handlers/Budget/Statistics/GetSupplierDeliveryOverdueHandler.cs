...
namespace Psz.Core.FinanceControl.Handlers.Budget.Statistics
{
	...
	public class GetSupplierDeliveryOverdueHandler: IHandle<Identity.Models.UserModel, ResponseModel<List<Models.Budget.Order.OrderModel>>>
	{
		private Identity.Models.UserModel _user { get; set; }

		public GetSupplierDeliveryOverdueHandler(Identity.Models.UserModel user)
		{
			this._user = user;
		}
		...
		public ResponseModel<List<Models.Budget.Order.OrderModel>> Handle()
		{
			try
			{
				...
				if(this._user.IsGlobalDirector)
					leasingOrderEntities = Infrastructure.Data.Access.Tables.FNC.BestellungenExtensionAccess.GetSupplierDeliveryOverdue();
				else
				{
					...
					{
						leasingOrderEntities.AddRange(Infrastructure.Data.Access.Tables.FNC.BestellungenExtensionAccess.GetSupplierDeliveryOverdue(companyIds: companyDirectorEntities.Select(x => x.Id)?.ToList()));
					}
					...
					{
						leasingOrderEntities.AddRange(Infrastructure.Data.Access.Tables.FNC.BestellungenExtensionAccess.GetSupplierDeliveryOverdue(companyIds: new List<int> { userEntiy.CompanyId ?? -1 }));
					}
					...
					{
						leasingOrderEntities.AddRange(Infrastructure.Data.Access.Tables.FNC.BestellungenExtensionAccess.GetSupplierDeliveryOverdue(companyIds: null, departmentIds: departmentHeadEntities.Select(x => (int)x.Id)?.ToList()));
					}

					// - add user issued PO
					leasingOrderEntities.AddRange(Infrastructure.Data.Access.Tables.FNC.BestellungenExtensionAccess.GetSupplierDeliveryOverdue(companyIds: null, departmentIds: null, employeeId: this._user.Id));
				}
