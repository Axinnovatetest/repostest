...
namespace Psz.Core.FinanceControl.Handlers.Budget.Project
{
	...
	public class CloseHandler: IHandle<Identity.Models.UserModel, ResponseModel<int>>
	{
		private Identity.Models.UserModel _user { get; set; }
		private int _data { get; set; }

		public CloseHandler(Identity.Models.UserModel user, int id)
		{
			this._user = user;
			this._data = id;
		}
		...
		public ResponseModel<int> Handle()
		{
			try
			{
				...
				projectEntity.Closed = true;
				projectEntity.ClosedTime = DateTime.Now;
				projectEntity.ClosedUserId = this._user.Id;
				projectEntity.ClosedUserName = this._user.Name;
				projectEntity.ClosedUserEmail = this._user.Email;
				projectEntity.ProjectStatus = (int)Enums.BudgetEnums.ProjectStatuses.Closed;
				projectEntity.ProjectStatusName = Enums.BudgetEnums.ProjectStatuses.Closed.GetDescription();
				...
				var emailBody = Helpers.Notifications.Email.ExternalProjectTemplate_StatusChange(this._user, projectEntity, "Closed");
				Helpers.Notifications.Email.SendEmail("[Budget] External project status change", emailBody, emailAddresses, null, this._user, null);
				...
				Helpers.Processings.Budget.Project.SaveProjectHistory(projectEntity, Enums.BudgetEnums.ProjectWorkflowActions.Close, this._user);
		...
		public ResponseModel<int> Validate()
		{
			...
			if(orderEntities != null && orderEntities.Count > 0)
				return ResponseModel<int>.FailureResponse("Cannot close project with in-progress orders");
