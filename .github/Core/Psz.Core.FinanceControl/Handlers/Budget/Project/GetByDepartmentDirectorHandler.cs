...
namespace Psz.Core.FinanceControl.Handlers.Budget.Project
{
	...
	public class GetByDepartmentDirectorHandler: IHandle<Identity.Models.UserModel, ResponseModel<List<Models.Budget.Project.ProjectModel>>>
	{
		...
		public ResponseModel<List<Models.Budget.Project.ProjectModel>> Handle()
		{
			try
			{
				...
				///  -
				var deptDirectorEntities = Infrastructure.Data.Access.Tables.STG.DepartmentAccess.GetByDirectorId(this._user.Id);
				if(deptDirectorEntities == null || deptDirectorEntities.Count <= 0)
				{
					return ResponseModel<List<Models.Budget.Project.ProjectModel>>.SuccessResponse(null);
				}

				var projectEntities =
					Infrastructure.Data.Access.Tables.FNC.BudgetProjectAccess.GetByDepartmentIds(deptDirectorEntities?.Select(x => (int)x.Id)?.ToList(), null)
