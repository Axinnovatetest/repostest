...
namespace Psz.Core.FinanceControl.Handlers.Budget.Project
{
	...
	public class GetForUserHandler: IHandle<Identity.Models.UserModel, ResponseModel<List<Models.Budget.Project.ProjectModel>>>
	{
		private Identity.Models.UserModel _user { get; set; }
		private Models.Budget.Project.GetRequestModel _data { get; set; }
		public GetForUserHandler(Identity.Models.UserModel user, Models.Budget.Project.GetRequestModel data)
		{
			this._user = user;
			this._data = data;
		}
		...
		public ResponseModel<List<Models.Budget.Project.ProjectModel>> Handle()
		{
			try
			{
				...
				{
					projectsEntities = Infrastructure.Data.Access.Tables.FNC.BudgetProjectAccess.Get(
						userId: null, companyId: this._data.CompanyId, departmentId: this._data.DepartmentId, minOrderCount: this._data.MinOrderCount, creationYear: this._data.CreationYear,
						type: this._data.Type, status: this._data.Status, approved: this._data.Approved, closed: this._data.Closed, isArchived: this._data.Archived);
				}
				else
				{
					projectsEntities = Infrastructure.Data.Access.Tables.FNC.BudgetProjectAccess.Get(
						userId: this._data.UserId, companyId: this._data.CompanyId, departmentId: this._data.DepartmentId, minOrderCount: this._data.MinOrderCount, creationYear: this._data.CreationYear,
						type: this._data.Type, status: this._data.Status, approved: this._data.Approved, closed: this._data.Closed, isArchived: this._data.Archived);
				}
