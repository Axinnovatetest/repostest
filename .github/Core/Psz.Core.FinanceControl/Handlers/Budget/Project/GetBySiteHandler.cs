...
namespace Psz.Core.FinanceControl.Handlers.Budget.Project
{
	...
	public class GetBySiteHandler: IHandle<Identity.Models.UserModel, ResponseModel<List<Models.Budget.Project.ProjectModel>>>
	{
		private Identity.Models.UserModel _user { get; set; }

		public GetBySiteHandler(Identity.Models.UserModel user)
		{
			this._user = user;
		}
		...
		public ResponseModel<List<Models.Budget.Project.ProjectModel>> Handle()
		{
			try
			{
				...
				var projectEntities = Infrastructure.Data.Access.Tables.FNC.BudgetProjectAccess.GetSiteLevel(companyEntities?.Select(x => x.Id)?.ToList())
				...
				foreach(var item in projectEntities)
				{
					...
					var responsableEntity = Infrastructure.Data.Access.Tables.COR.UserAccess.Get(item.ResponsableId);
