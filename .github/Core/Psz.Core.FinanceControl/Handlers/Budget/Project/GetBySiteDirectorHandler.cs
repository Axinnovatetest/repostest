...
namespace Psz.Core.FinanceControl.Handlers.Budget.Project
{
	...
	public class GetBySiteDirectorHandler: IHandle<Identity.Models.UserModel, ResponseModel<List<Models.Budget.Project.ProjectModel>>>
	{
		private Identity.Models.UserModel _user { get; set; }

		public GetBySiteDirectorHandler(Identity.Models.UserModel user)
		{
			this._user = user;
		}
		...
		public ResponseModel<List<Models.Budget.Project.ProjectModel>> Handle()
		{
			try
			{
				...
				var projectEntities = Infrastructure.Data.Access.Tables.FNC.BudgetProjectAccess.GetBySitesIds(companyEntities?.Select(x => x.Id)?.ToList(), null)
					?? new List<Infrastructure.Data.Entities.Tables.FNC.BudgetProjectEntity>();

				var siteLevelProjects = Infrastructure.Data.Access.Tables.FNC.BudgetProjectAccess.GetSiteLevel(companyEntities?.Select(x => x.Id)?.ToList());
				if(siteLevelProjects?.Count > 0)
				{
					projectEntities.AddRange(siteLevelProjects);
				}
				...
				foreach(var item in projectEntities)
				{
					...
					var userEntity = Infrastructure.Data.Access.Tables.COR.UserAccess.Get(item.ResponsableId);
		...
		public ResponseModel<List<Models.Budget.Project.ProjectModel>> Validate()
		{
			...
			{
				return ResponseModel<List<Models.Budget.Project.ProjectModel>>.FailureResponse("User is not a Site Director");
			}
