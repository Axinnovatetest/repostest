...
namespace Psz.Core.CapitalRequests.Models
{
	...
	public class GetModel
	{
		...
		public GetModel(Infrastructure.Data.Entities.Tables.COR.UserEntity userEntity,
			...
			List<Infrastructure.Data.Entities.Tables.CPL.AccessProfileEntity> accessProfileEntities)
		{
			if(userEntity == null)
				return;
			...
			if(accessProfileEntities != null && accessProfileEntities.Count > 0)
			{
				...
				var entities = MoreEnumerable.DistinctBy(accessProfileEntities, x => x.Id);
				entities?.ForEach(x =>
				{
