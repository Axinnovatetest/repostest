		...
		public class XLS
		{
			...
			public static string GetCellValueFgAsNumerique(OfficeOpenXml.ExcelRange cell)
			{
				if(cell?.Value == null)
					return string.Empty;
				var val = cell.Value;
				var unparsed = val.ToString().Contains("E")
					? decimal.Parse(val.ToString(), NumberStyles.Float, CultureInfo.InvariantCulture).ToString()
					: val.ToString();
				if(val == null || !decimal.TryParse(unparsed, out decimal ValNum))
				{
					throw new InvalidOperationException("Cell value cannot be null");
				}
