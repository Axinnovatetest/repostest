...
{
	public class GetAllValuesHandler: IHandle<Identity.Models.UserModel, ResponseModel<List<KeyValuePair<string, string>>>>
	{
		private Identity.Models.UserModel _user { get; set; }

		public GetAllValuesHandler(Identity.Models.UserModel user)
		{
			this._user = user;
		}
		...
		public ResponseModel<List<KeyValuePair<string, string>>> Handle()
		{
			try
			{
				...
				return ResponseModel<List<KeyValuePair<string, string>>>.SuccessResponse(
					Infrastructure.Data.Access.Tables.BSD.CocTypeAccess.Get()?.Select(x => new KeyValuePair<string, string>(x.Version, x.Name))?.ToList());
			} catch(Exception e)
			{
