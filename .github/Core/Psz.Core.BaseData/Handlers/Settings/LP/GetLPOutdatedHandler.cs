...
{
	public class GetLPOutdatedHandler: IHandle<Identity.Models.UserModel, ResponseModel<List<Models.CustomerSupplierLP.LPModel>>>
	{
		private Identity.Models.UserModel _user { get; set; }
		private GetOutdatedArticlesExcel _data { get; set; }

		public GetLPOutdatedHandler(Identity.Models.UserModel user, GetOutdatedArticlesExcel data)
		{
			this._user = user;
			this._data = data;
		}
		public ResponseModel<List<Models.CustomerSupplierLP.LPModel>> Handle()
		{
			try
			{
				...
				var LPEntity = Infrastructure.Data.Access.Joins.Kunden_LieferantenAccess.GetLPOutdatedWithStrdLiefrenten(this._data.nr, this._data.mnths);
		...
		public byte[] GetDataXLS()
		{
			try
			{
				...
				using(var package = new ExcelPackage(file))
				{
					...
					worksheet.Cells[1, 1].Value = $"Veraltete Artikel für {data.Body?.First().Name1} - {DateTime.Now.ToString("dd.MM.yyyy")}";
					...
					worksheet.Cells[headerRowNumber, startColumnNumber + 6].Value = "Wiederbeschaffungszeitraum";
					worksheet.Cells[headerRowNumber, startColumnNumber + 7].Value = "StandardlieferantName";
					...
					if(data.Success == true && data.Body.Count > 0)
					{
						...
						foreach(var w in data.Body)
						{
							...
							worksheet.Cells[rowNumber, startColumnNumber + 4].Style.Numberformat.Format = "dd.MM.yyyy";
