			...
			try
			{
				lock((Locks.ArticleEditLock.GetOrAdd(this._data.ArticleId, new object())))
				{
					...
					if(salesItemEntity.ArticleSalesTypeId == (int)Common.Enums.ArticleEnums.SalesItemType.Serie)
					{
						...
						{
							//artikelKalkulEntity.Betrag = /*decimal.Parse(_salesItemEntity.Produktionskosten)*/(_salesItemEntity.Produktionskosten != null) ? ParseTextToDecimal(_salesItemEntity.Produktionskosten?.ToString()) : 0/*decimal.TryParse(_salesItemEntity.Produktionskosten.Replace(",", "."), out var betrag) ? betrag : 0:0*/;
					...
					return ResponseModel<int>.SuccessResponse(Infrastructure.Data.Access.Tables.BSD.ArtikelSalesExtensionAccess.Update(_salesItemEntity));
				}
