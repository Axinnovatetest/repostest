		...
		public ResponseModel<List<Models.Article.Overview.BlanketResponseModel>> Handle()
		{
			try
			{
				...
				if(raPosExtensions.Count > 0)
				{
					...
					// - 2026-02-20 - Hejdukova sort by create date (we use the Id instead as date does not exist on the item)
					foreach(var raPosExt in raPosExtensions?.OrderByDescending(x=> x.Id))
					{
