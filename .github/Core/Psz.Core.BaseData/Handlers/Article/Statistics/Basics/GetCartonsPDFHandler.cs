...
namespace Psz.Core.BaseData.Handlers.Article.Statistics.Basics
{
	...
	public class GetCartonsPDFHandler: IHandle<UserModel, ResponseModel<string>>
	{
		...
		public byte[] GetFilePDF(List<Models.Article.Statistics.Basics.CirculationResponseModel> dataEntities)
		{
			try
			{
				...
				// -
				return Module.ReportingService.Generate_BSD_ArticlesStatisticsCartons(Infrastructure.Services.Reporting.Helpers.ReportType.BSD_ART_STATS_CARTONS, data);
			} catch(Exception exception)
			{
