...
namespace Psz.Core.BaseData.Handlers.Article.Statistics.ControllingAnalysis
{
	...
	public class ImportSuperbillArtHandler: IHandle<UserModel, ResponseModel<Infrastructure.Data.Entities.Joins.ArticleStatisticsEntities.ControllingAnalysis_SuperbillROH>>
	{
		private UserModel _user { get; set; }
		private ImportFileRequestBusinessModel _data { get; set; }
		public ImportSuperbillArtHandler(UserModel user, ImportFileRequestBusinessModel data)
		{
			this._user = user;
			this._data = data;
		}
		public ResponseModel<Infrastructure.Data.Entities.Joins.ArticleStatisticsEntities.ControllingAnalysis_SuperbillROH> Handle()
		{
			...
			try
			{
				...
				var articleData = ReadFromExcel_ArticleMenge(this._data.FileByteArray, out errors);
				...
				// -
				var articleEntities = Infrastructure.Data.Access.Tables.PRS.ArtikelAccess.GetByNumbers(articleData.Select(x => x.Item1)?.Distinct()?.ToList());
				Infrastructure.Data.Access.Tables.PRS.ObjectLogAccess.InsertWithTransaction(
					articleData.Select(x =>
					{
						var article = articleEntities.FirstOrDefault(y => y.ArtikelNummer?.ToLower()?.Trim() == x.Item1?.ToLower()?.Trim());
						return
						ObjectLogHelper.getLog(this._user, article?.ArtikelNr ?? 0, "SuperbillROH", "",
						$"Artikelnummer: {x.Item1}, Menge: {x.Item2}",
				...
				var results = Infrastructure.Data.Access.Joins.ArticleStatisticsAccess.ControllingAnalysis.GetSuperbillROH_MultiQuery(
						articleData?.Select(x => new Infrastructure.Data.Entities.Joins.ArticleStatisticsEntities.ControllingAnalysis_SuperbillROHInput
						{
							Artikelnummer = x.Item1,
							Menge = x.Item2
						})?.ToList(),
		...
		internal static List<Tuple<string, decimal>> ReadFromExcel_ArticleMenge(byte[] fileBytes, out List<string> errors)
		{
			...
			try
			{
				...
				if(rows > 1 && columns > 0)
				{
					var xlsData = new List<Tuple<string, decimal>> { };
					...
					for(int i = startRowNumber; i <= rowEnd; i++)
					{
						try
						{
							...
							string menge = Core.Common.Helpers.Formatters.XLS.GetCellValue(worksheet.Cells[i, startColNumber + 1])?.Trim();
							if(decimal.TryParse(Core.Common.Helpers.Formatters.XLS.EscapeDecimalSeparator(menge), out var _menge))
							{
								xlsData.Add(new Tuple<string, decimal>(number, _menge));
							}
							else
							{
		...
		public ResponseModel<byte[]> GetDetailExcelData()
		{
			try
			{
				...
				using(var package = new ExcelPackage(file))
				{
					...
					worksheet.Cells[headerRowNumber, startColumnNumber + 19].Value = "Bestand ROH BETN";
					worksheet.Cells[headerRowNumber, startColumnNumber + 20].Value = "Bestand ROH GZ";

					worksheet.Cells[headerRowNumber, startColumnNumber + 21].Value = "Bestand ROH Obsolete";
					worksheet.Cells[headerRowNumber, startColumnNumber + 22].Value = "Rahmen";
					worksheet.Cells[headerRowNumber, startColumnNumber + 23].Value = "Rahmen Nr";
					worksheet.Cells[headerRowNumber, startColumnNumber + 24].Value = "Rahmenmenge";
					worksheet.Cells[headerRowNumber, startColumnNumber + 25].Value = "Rahmenauslauf";

					worksheet.Cells[headerRowNumber, startColumnNumber + 26].Value = "Mindestbestand ROH CZ";
					worksheet.Cells[headerRowNumber, startColumnNumber + 27].Value = "Mindestbestand ROH TN";
					worksheet.Cells[headerRowNumber, startColumnNumber + 28].Value = "Mindestbestand ROH AL";
					worksheet.Cells[headerRowNumber, startColumnNumber + 29].Value = "Mindestbestand ROH KHTN";
					worksheet.Cells[headerRowNumber, startColumnNumber + 30].Value = "Mindestbestand ROH BETN";
					worksheet.Cells[headerRowNumber, startColumnNumber + 31].Value = "Mindestbestand ROH GZ";

					worksheet.Cells[headerRowNumber, startColumnNumber + 32].Value = "Mindestbestand ROH Obsolete";
					worksheet.Cells[headerRowNumber, startColumnNumber + 33].Value = "ROH Angebotsdatum";
					...
					if(dataEntities != null && dataEntities.Count > 0)
					{
						...
						foreach(var w in dataEntities)
						{
							...
							worksheet.Cells[rowNumber, startColumnNumber + 19].Value = w?.Bestand_ROH_BETN;
							worksheet.Cells[rowNumber, startColumnNumber + 20].Value = w?.Bestand_ROH_GZTN;

							worksheet.Cells[rowNumber, startColumnNumber + 21].Value = w?.Bestand_ROH_Obsolete;
							worksheet.Cells[rowNumber, startColumnNumber + 22].Value = w?.Rahmen;
							worksheet.Cells[rowNumber, startColumnNumber + 23].Value = w?.Rahmen_Nr;
							worksheet.Cells[rowNumber, startColumnNumber + 24].Value = w?.Rahmenmenge;
							worksheet.Cells[rowNumber, startColumnNumber + 25].Value = w?.Rahmenauslauf;

							worksheet.Cells[rowNumber, startColumnNumber + 26].Value = w?.Mindestbestand_ROH_CZ;
							worksheet.Cells[rowNumber, startColumnNumber + 27].Value = w?.Mindestbestand_ROH_TN;
							worksheet.Cells[rowNumber, startColumnNumber + 28].Value = w?.Mindestbestand_ROH_AL;
							worksheet.Cells[rowNumber, startColumnNumber + 29].Value = w?.Mindestbestand_ROH_KHTN;
							worksheet.Cells[rowNumber, startColumnNumber + 30].Value = w?.Mindestbestand_ROH_BETN;
							worksheet.Cells[rowNumber, startColumnNumber + 31].Value = w?.Mindestbestand_ROH_GZTN;

							worksheet.Cells[rowNumber, startColumnNumber + 32].Value = w?.Mindestbestand_ROH_Obsolete;
							worksheet.Cells[rowNumber, startColumnNumber + 33].Value = w?.ROH_Angebotsdatum;
		...
		public ResponseModel<byte[]> GetSumExcelData()
		{
			try
			{
				...
				using(var package = new ExcelPackage(file))
				{
					...
					worksheet.Cells[headerRowNumber, startColumnNumber + 23].Value = "Bestand ROH BETN";
					worksheet.Cells[headerRowNumber, startColumnNumber + 24].Value = "Mindestbestand ROH BETN";
					worksheet.Cells[headerRowNumber, startColumnNumber + 25].Value = "Bestand ROH GZ";
					worksheet.Cells[headerRowNumber, startColumnNumber + 26].Value = "Mindestbestand ROH GZ";
					worksheet.Cells[headerRowNumber, startColumnNumber + 27].Value = "Bestand ROH Obsolete";
					worksheet.Cells[headerRowNumber, startColumnNumber + 28].Value = "Mindestbestand ROH Obsolete";

					worksheet.Cells[headerRowNumber, startColumnNumber + 29].Value = "ROH Angebotsdatum";
					...
					if(dataEntities != null && dataEntities.Count > 0)
					{
						...
						foreach(var w in dataEntities)
						{
							...
							worksheet.Cells[rowNumber, startColumnNumber + 23].Value = w?.Bestand_ROH_BETN;
							worksheet.Cells[rowNumber, startColumnNumber + 24].Value = w?.Mindestbestand_ROH_BETN;
							worksheet.Cells[rowNumber, startColumnNumber + 25].Value = w?.Bestand_ROH_GZTN;
							worksheet.Cells[rowNumber, startColumnNumber + 26].Value = w?.Mindestbestand_ROH_GZTN;
							worksheet.Cells[rowNumber, startColumnNumber + 27].Value = w?.Bestand_ROH_Obsolete;
							worksheet.Cells[rowNumber, startColumnNumber + 28].Value = w?.Mindestbestand_ROH_Obsolete;

							worksheet.Cells[rowNumber, startColumnNumber + 29].Value = w?.ROH_Angebotsdatum;
