...
namespace Psz.Core.BaseData.Handlers.Article.Statistics.ControllingAnalysis
{
	...
	public class GetPszPrioeinkauf2PDFHandler: IHandle<UserModel, ResponseModel<Models.Article.Statistics.ControllingAnalysis.PrioEinkaufResponseModel>>
	{
		private UserModel _user { get; set; }
		private Models.Article.Statistics.ControllingAnalysis.PrioEinkaufRequestModel _data { get; set; }
		public GetPszPrioeinkauf2PDFHandler(UserModel user, Models.Article.Statistics.ControllingAnalysis.PrioEinkaufRequestModel data)
		{
			this._user = user;
			this._data = data;
		}
		public ResponseModel<Models.Article.Statistics.ControllingAnalysis.PrioEinkaufResponseModel> Handle()
		{
			try
			{
				...
				var statisticsEntities = Infrastructure.Data.Access.Joins.ArticleStatisticsAccess.ControllingAnalysis.GetPszPrioeinkauf_report2(this._data.SearchTerms, this._data.RequestedPage, this._data.ItemsPerPage)
					?? new List<Infrastructure.Data.Entities.Joins.ArticleStatisticsEntities.ControllingAnalysis_PszPrioeinkauf_report2>();

				// - 
				var allCount = Infrastructure.Data.Access.Joins.ArticleStatisticsAccess.ControllingAnalysis.GetPszPrioeinkauf_report2_count(this._data.SearchTerms);
				...
				responseBody.Title = $"AB Termindifferenzen";
		...
		public static byte[] GetData(Models.Article.Statistics.ControllingAnalysis.PrioEinkaufRequestModel _data = null)
		{
			if(_data == null)
			{
				return SaveToPdfFile(
					Infrastructure.Data.Access.Joins.ArticleStatisticsAccess.ControllingAnalysis.GetPszPrioeinkauf_report2());
			}
			else
			{
				// - 
				var allCount = Infrastructure.Data.Access.Joins.ArticleStatisticsAccess.ControllingAnalysis.GetPszPrioeinkauf_report2_count(_data.SearchTerms);
				var statisticsEntities = Infrastructure.Data.Access.Joins.ArticleStatisticsAccess.ControllingAnalysis.GetPszPrioeinkauf_report2(_data.SearchTerms, _data.RequestedPage, allCount)
					   ?? new List<Infrastructure.Data.Entities.Joins.ArticleStatisticsEntities.ControllingAnalysis_PszPrioeinkauf_report2>();
		...
		internal static byte[] SaveToPdfFile(List<Infrastructure.Data.Entities.Joins.ArticleStatisticsEntities.ControllingAnalysis_PszPrioeinkauf_report2> dataEntities)
		{
			try
			{
				...
				var data = new Infrastructure.Services.Reporting.Models.BSD.Articles.StatisticsPszPrio2DataModel();

				// -
				data.ReportData = new List<Infrastructure.Services.Reporting.Models.BSD.Articles.StatisticsPszPrio2DataModel.Header> {
					new Infrastructure.Services.Reporting.Models.BSD.Articles.StatisticsPszPrio2DataModel.Header {Title= $"AB Termindifferenzen" }
				};

				// - Suppliers 
				data.Suppliers = dataEntities.Select(x => new Infrastructure.Services.Reporting.Models.BSD.Articles.StatisticsPszPrio2DataModel.Supplier
				{
					Name1 = x.Name1,
					Telefon = x.Telefon,
					Fax = x.Fax
				...
				data.Items = dataEntities
					...
					.Select(x => new Infrastructure.Services.Reporting.Models.BSD.Articles.StatisticsPszPrio2DataModel.Item
					{
						Datum = x.Datum?.ToString("dd.MM.yyyy"),
						Bestellung_Nr = x.Bestellung_Nr,
						Lagerort_id = x.Lagerort_id,
				...
				// -
				return Module.ReportingService.Generate_BSD_ArticlesStatisticsPszPrioeinkauf2(Infrastructure.Services.Reporting.Helpers.ReportType.BSD_ART_STATS_PSZPRIO2, data);
			} catch(Exception exception)
			{
				Infrastructure.Services.Logging.Logger.Log(exception);
				throw;
		...
		internal static byte[] SaveToXlsFile(Models.Article.Statistics.ControllingAnalysis.PrioEinkaufResponseModel data)
		{
			try
			{
				...
				var filePath = System.IO.Path.Combine(tempFolder, $"PszPrioeinkauf2-{DateTime.Now.ToString("yyyyMMddTHHmmff")}.xlsx");
				...
				using(var package = new ExcelPackage(file))
				{
					...
					ExcelWorksheet worksheet = package.Workbook.Worksheets.Add($"AB Termindifferenzen");
					...
					worksheet.Cells[1, 1].Value = $"AB Termindifferenzen";
					...
					package.Workbook.Properties.Title = $"AB Termindifferenzen";
