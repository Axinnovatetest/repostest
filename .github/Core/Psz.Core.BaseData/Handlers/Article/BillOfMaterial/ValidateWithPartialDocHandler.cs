...
namespace Psz.Core.BaseData.Handlers.Article.BillOfMaterial
{
	...
	public class ValidateWithPartialDocHandler: IHandle<UserModel, ResponseModel<List<Models.Article.BillOfMaterial.ValidateBomResponseModel>>>
	{
		private UserModel _user { get; set; }
		private Models.Article.BillOfMaterial.ValidateWPartialDocRequestModel _data { get; set; }
		public ValidateWithPartialDocHandler(UserModel user, Models.Article.BillOfMaterial.ValidateWPartialDocRequestModel data)
		{
			_user = user;
			_data = data;
		}
		...
		/// <summary>
		/// - 2022-07-13 - 
		/// When index is not changed, user can chose to only partially validate the Bom.
		/// This means no new Bom version and the old Snapshot will be overwriten (lost of historical data- as the old snapshot cannot be recovered),
		/// all open FAs with current Bom version will be update
		/// if Article requires CP, then unvalidate CP and ask for upgradable FAs.
		/// </summary>
		/// <returns></returns>
		public ResponseModel<List<Models.Article.BillOfMaterial.ValidateBomResponseModel>> Handle()
		{
			...
			try
			{
				...
				List<string> errors;
				saveBomSnapshot_PartialValidation(this._data.ArticleId, articleExtEntity.BomVersion, this._user.Id, out errors, botransaction.connection, botransaction.transaction);
				...
				var validationLogs = new List<Infrastructure.Data.Entities.Tables.BSD.CAO_Validation_LOGEntity>();
				...
				{
					validationLogs.Add(new Infrastructure.Data.Entities.Tables.BSD.CAO_Validation_LOGEntity
					{
						...
						artikel_nr = cpVersionEntity?.Artikel_Nr
					});
				}
				if(cpVersionEntity != null && cpVersionEntity.CP_version != newCpVersion)
				{
					validationLogs.Add(new Infrastructure.Data.Entities.Tables.BSD.CAO_Validation_LOGEntity
					{
						artikelnummer = articleEntity.ArtikelNummer,
						...
						kunden_index = articleEntity.Index_Kunde,
						username = this._user.Name,
						val_status = $"CP Version changed from [{cpVersionEntity.CP_version}] to [{newCpVersion}]",
						artikel_nr = cpVersionEntity?.Artikel_Nr
					});
				}
				Infrastructure.Data.Access.Tables.BSD.CAO_Validation_LOGAccess.InsertWithTransaction(validationLogs, botransaction.connection, botransaction.transaction);
				...
				if(botransaction.commit())
				{
					...
					return ResponseModel<List<Models.Article.BillOfMaterial.ValidateBomResponseModel>>.SuccessResponse(
						potentialUpgradeFA
						?.Select(x => new Models.Article.BillOfMaterial.ValidateBomResponseModel(x, articleEntities?.FirstOrDefault(y => y.ArtikelNr == x.Artikel_Nr)?.ArtikelNummer))
						?.ToList());
				}
				else
				{
		...
		public ResponseModel<List<Models.Article.BillOfMaterial.ValidateBomResponseModel>> Validate()
		{
			...
			if(this._data.CanPartialValidation && (articleExtEntity == null || articleExtEntity.BomVersion <= 0))
				return ResponseModel<List<Models.Article.BillOfMaterial.ValidateBomResponseModel>>.FailureResponse($"This is the first BOM, Partial Validation not allowed");
			...
			if(this._data.CanPartialValidation && articleEntity.Index_Kunde != lastBOMSnapshot.KundenIndex)
				return ResponseModel<List<Models.Article.BillOfMaterial.ValidateBomResponseModel>>.FailureResponse($"Customer Index [{articleEntity.Index_Kunde}] has changed since previous BOM [{lastBOMSnapshot.KundenIndex}], Partial Validation not allowed");
			...
			if(articleEntity != null)
			{
				...
				if(bomExtensionEntity != null)
				{
						...
						|| Infrastructure.Data.Access.Tables.BSD.StucklistenPositionAlt_SnapshotAccess.Get_count(articleId, version ?? -1, connection, transaction) > 0)
					{
						...
						Infrastructure.Data.Access.Tables.BSD.StucklistenArticleExtension_SnapshotAccess.OverwriteSnapshotWithTransaction(articleId, version ?? -1, userId, connection, transaction);
					}
