...
namespace Psz.Core.BaseData.Handlers.Article.Purchase
{
	public class DeleteArticlePurchaseHandler: IHandle<int, ResponseModel<object>>
	{
		...
		public ResponseModel<object> Validate()
		{
			...
			{
				var openOrders = Infrastructure.Data.Access.Tables.BSD.BestellungenAccess.GetByLieferant(PurchaseEntity.Lieferanten_Nr ?? -1);
				if(openOrders?.Count > 0)
				{
					var openPositions = Infrastructure.Data.Access.Tables.BSD.Bestellte_ArtikelAccess.GetByArticleAndBestellung(PurchaseEntity.ArtikelNr ?? -1, openOrders.Select(x => x.Nr).ToList());
