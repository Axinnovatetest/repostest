	...
	public class GetListHandler: IHandle<UserModel, ResponseModel<List<KeyValuePair<int, string>>>>
	{
		...
		public ResponseModel<List<KeyValuePair<int, string>>> Handle()
		{
			try
			{
				...
				{
					return ResponseModel<List<KeyValuePair<int, string>>>.SuccessResponse(packagingEntites
							.Select(x => new KeyValuePair<int, string>(x.Id, $"{x.Packmittel_Karton} |\t{x.Masse_LxBxH__in_mm_}".Trim())).Distinct().ToList());
				}
