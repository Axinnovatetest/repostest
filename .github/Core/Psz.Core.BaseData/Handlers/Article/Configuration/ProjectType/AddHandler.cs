...
namespace Psz.Core.BaseData.Handlers.Article.ProjectType
{
	...
	public class AddHandler: IHandle<UserModel, ResponseModel<int>>
	{
		...
		private Models.Article.ProjectType.ProjectType _data { get; set; }
		public AddHandler(UserModel user, Models.Article.ProjectType.ProjectType projectType)
		{
			_user = user;
			_data = projectType;
		}
		public ResponseModel<int> Handle()
		{
			try
			{
				...
				var insertedId = Infrastructure.Data.Access.Tables.BSD.ProjectTypeAccess.Insert(vdEntity);
				if(insertedId > 0)
				{
		...
		public ResponseModel<int> Validate()
		{
			...
			if(string.IsNullOrWhiteSpace(this._data.Name))
				return ResponseModel<int>.FailureResponse($"Project Type Name should have a value");

			var projectTypeEntities = Infrastructure.Data.Access.Tables.BSD.ProjectTypeAccess.GetByName(this._data.Name);
			if(projectTypeEntities != null && projectTypeEntities.Count > 0)
				return ResponseModel<int>.FailureResponse($"Project Type [{this._data.Name}] exists");
