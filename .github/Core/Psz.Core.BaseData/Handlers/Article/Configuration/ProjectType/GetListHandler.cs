	...
	public class GetListHandler: IHandle<UserModel, ResponseModel<List<KeyValuePair<int, string>>>>
	{
		...
		public ResponseModel<List<KeyValuePair<int, string>>> Handle()
		{
			try
			{
				...
				{
					return ResponseModel<List<KeyValuePair<int, string>>>.SuccessResponse(projectTypeEntites
							.Select(x => new KeyValuePair<int, string>(x.Id, $"{x.Name}".Trim())).Distinct().ToList());
				}
