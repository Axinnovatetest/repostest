...
public class GetPhotoIdPackagingsFromDirectoryHandler: IHandleAsync<Identity.Models.UserModel, ResponseModel<List<KeyValuePair<int, string>>>>
{

	private Identity.Models.UserModel _user { get; set; }
	private System.Threading.CancellationToken? _token { get; set; }
	private int _articleId;
	public GetPhotoIdPackagingsFromDirectoryHandler(Identity.Models.UserModel user, int articleId, System.Threading.CancellationToken? token = null)
	{
		_user = user;
	...
	public async Task<ResponseModel<List<KeyValuePair<int, string>>>> HandleAsync()
	{
		try
		{
			if(_articleId == 0)
			{
				return await ResponseModel<List<KeyValuePair<int, string>>>.FailureResponseAsync("null article");
			}
