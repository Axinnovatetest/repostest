...
namespace Psz.Core.BaseData.Handlers.ProjectManagment
{
	...
	{
		public ResponseModel<List<KeyValuePair<int, string>>> GetProjectFactories(UserModel user)
		{
			if(user == null)
			...
			try
			{
				...
				var types = Enum.GetValues(typeof(Enums.ProjectManagmentEnums.Factories)).Cast<Enums.ProjectManagmentEnums.Factories>().ToList();
