...
namespace Psz.Core.BaseData.Handlers.ProjectManagment
{
	public partial class ProjectManagmentService
	{
		...
		{
			var validationResponse = ValidateAddProject(user, data);
		...
		private ResponseModel<string> ValidateAddProject(UserModel user, ProjectAddRequestModel data)
		{
			if(user == null)
				return ResponseModel<string>.AccessDeniedResponse();
			if(data.Header.PMManagerId is null)
				return ResponseModel<string>.FailureResponse("PM Manager is required.");
			if(data.Header.CSManagerId is null)
				return ResponseModel<string>.FailureResponse("CS Manager is required.");
			if(data.Header.PMManagerFactoryId is null)
				return ResponseModel<string>.FailureResponse("PM Manager Factory is required.");
