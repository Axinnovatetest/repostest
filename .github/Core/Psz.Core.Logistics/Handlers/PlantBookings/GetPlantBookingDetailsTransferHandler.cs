...
public class GetPlantBookingDetailsTransferHandler: IHandle<int, ResponseModel<PlantBookingDetailsResponseModel>>
{
	private Core.Identity.Models.UserModel _user;
	private PlantBookingRequestPrintModel _data;
	public GetPlantBookingDetailsTransferHandler(Core.Identity.Models.UserModel user, PlantBookingRequestPrintModel data)
	{
		this._user = user;
		this._data = data;
	}
		...
		try
		{
			...
			switch(_data.LagerId)
			{
				...
				case (int)LagerEnum.Eigenfertigung:
					...
					resToReturn = new PlantBookingDetailsResponseModel(fetchedData);
					Previous = Infrastructure.Data.Access.Tables.CTS.PSZ_Eingangskontrolle_CZAccess.GetPlantBookingsPrecedentByLagerIDCZ(fetchedData.LagerortID ?? -1, (int)fetchedData.Verpackungsnr);
					Next = Infrastructure.Data.Access.Tables.CTS.PSZ_Eingangskontrolle_CZAccess.GetPlantBookingsSuivantByLagerIDCZ(fetchedData.LagerortID ?? -1, (int)fetchedData.Verpackungsnr);
				...
				case (int)LagerEnum.Albanien:
					...
					resToReturn = new PlantBookingDetailsResponseModel(fetchedData);
					Previous = Infrastructure.Data.Access.Tables.CTS.PSZ_Eingangskontrolle_ALAccess.GetPlantBookingsPrecedentByLagerIDAL(fetchedData.LagerortID ?? -1, (int)fetchedData.Verpackungsnr);
					Next = Infrastructure.Data.Access.Tables.CTS.PSZ_Eingangskontrolle_ALAccess.GetPlantBookingsSuivantByLagerIDAL(fetchedData.LagerortID ?? -1, (int)fetchedData.Verpackungsnr);
					resToReturn.Previous = Previous == null ? 0 : Previous.Verpackungsnr;
					resToReturn.Next = Next == null ? 0 : Next.Verpackungsnr;
