...
{
	public class RemoveInventoryWarehouseHandler: IHandle<Identity.Models.UserModel, ResponseModel<int>>
	{
		private Identity.Models.UserModel _user { get; set; }
		private Models.Administration.AccessProfiles.AddInventoryWarehouseRequestModel _data { get; set; }
		public RemoveInventoryWarehouseHandler(Identity.Models.UserModel user, Models.Administration.AccessProfiles.AddInventoryWarehouseRequestModel data)
		{
			this._user = user;
			this._data = data;
		}
		...
		public ResponseModel<int> Handle()
		{
			...
			try
			{
				...
				var id = Infrastructure.Data.Access.Tables.Logistics.InventoryStock.AccessProfileWarehouseAccess.DeleteByAccessProfile(
						accessProfileId: this._data.AccessProfileId,
						warehouseIds: this._data.WarehouseIds,
				...
				Infrastructure.Data.Access.Tables.Logistics.Logistics_LogAccess.Insert(
					new Infrastructure.Data.Entities.Tables.Logistics.Logistics_LogEntity
					{
						...
						LogText = $"Warehouse [{string.Join(", ", warehouses?.Select(x=>x.Lagerort))}] removed from Access Profile [{accessProfile.AccessProfileName}] by User [{this._user.Username}]",
						LogType = "Info",
						Origin = "LGT",
						ProjektNr = null,
