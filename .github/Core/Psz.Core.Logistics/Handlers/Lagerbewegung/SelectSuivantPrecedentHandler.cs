...
{
	public class SelectSuivantPrecedentHandler: IHandle<Identity.Models.UserModel, ResponseModel<LagerbewegungCompletModel>>
	{
		...
		public SelectSuivantPrecedentHandler(Identity.Models.UserModel user, string typ, int order, long idLagerbewegung)
		{
			this._user = user;
			this._typ = typ;
			this._order = order;
		...
		public ResponseModel<LagerbewegungCompletModel> Handle()
		{
			try
			{
				...
				var lagerbwegungHeaderEntity = Infrastructure.Data.Access.Tables.Logistics.LagebewegungAccess.GetLagerbewegungSuivantPrecedentByTyp(this._typ, this._order, this._idLagerbewegung);
