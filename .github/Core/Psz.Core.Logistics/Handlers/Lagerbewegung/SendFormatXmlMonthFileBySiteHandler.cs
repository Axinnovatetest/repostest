...
{
	public class SendFormatXmlMonthFileBySiteHandler: IHandle<Identity.Models.UserModel, ResponseModel<bool>>
	{
		private Identity.Models.UserModel _user { get; set; }
		private FormatXmlMonthFileBySiteRequestModel _data { get; set; }

		public SendFormatXmlMonthFileBySiteHandler(FormatXmlMonthFileBySiteRequestModel data, Identity.Models.UserModel user)
		{
			_user = user;
			_data = data;
		}
		public ResponseModel<bool> Handle()
		{
			...
			try
			{
				...
				var xmlResults = GetFormatXmlMonthFileBySiteHandler.GetData(botransaction, _data, out xmlData);
				...
				if(xmlData is not null && xmlData.Count > 0)
				{
					...
					// - 2024-06-27 - save logs
					var _date = DateTime.Now;
					Infrastructure.Data.Access.Joins.Logistics.StatisticsAccess.SaveLogsBySiteForMonth(_user.Id, _user.Username, _data.SiteName, _data.DataDate, (Enums.FormatEnums.TransferTypes)_data.TransferType == Enums.FormatEnums.TransferTypes.Delivery, botransaction.connection, botransaction.transaction);
				}
				#endregion save logs
				#endregion
