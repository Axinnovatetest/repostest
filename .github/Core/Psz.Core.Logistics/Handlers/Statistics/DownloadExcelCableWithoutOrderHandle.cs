...
{
	public class DownloadExcelCableWithoutOrderHandle: IHandle<Identity.Models.UserModel, ResponseModel<byte[]>>
	{
		private Identity.Models.UserModel _user { get; set; }
		private int _data { get; set; }
		public DownloadExcelCableWithoutOrderHandle(Identity.Models.UserModel user, int _data)
		{
			this._user = user;
			this._data = _data;
		...
		public ResponseModel<byte[]> Handle()
		{
			try
			{
				...
				var response = new List<CableWithoutOrderModel>();
				...
				if(PackingListEntity != null && PackingListEntity.Count > 0)
					response = PackingListEntity.Select(k => new CableWithoutOrderModel(k)).ToList();
				return ResponseModel<byte[]>.SuccessResponse(getExcel(response, _data, "Cable Without Order"));
			} catch(Exception e)
			{
		...
		static byte[] getExcel(List<CableWithoutOrderModel> MainRequestModel, int _data, string title)
		{

			string filePath = Path.Combine(Path.GetTempPath(), $"PSZ_Kabel_Bedarfsanalyse_III {DateTime.Now.ToString("yyyy-MM-ddTHHmmss.fff")}.xlsx");
			try
			{
				...
				using(var package = new ExcelPackage(file))
				{
					...
					ExcelWorksheet worksheet = package.Workbook.Worksheets.Add($"PSZ_Kabel_Bedarfsanalyse_III - {DateTime.Now.ToString("yyyy/MM/dd")}");
