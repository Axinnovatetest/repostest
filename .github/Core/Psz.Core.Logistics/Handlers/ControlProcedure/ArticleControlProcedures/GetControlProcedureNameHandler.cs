...
public  class GetControlProcedureNameHandler: IHandle<Core.Identity.Models.UserModel, ResponseModel<List<string>>>
{

	private Core.Identity.Models.UserModel _user;
	private GetProcedurNameModel _data;
	public GetControlProcedureNameHandler(Core.Identity.Models.UserModel user,GetProcedurNameModel data)
	{
		this._user = user;
		this._data = data;
	}
	...
	public ResponseModel<List<string>> Handle()
	{
		try
		{
			...
			var fetchedData = Infrastructure.Data.Access.Tables.Logistics.PlantBookingsArticleSupplierControlProceduresAccess.GetProcedureName(this._data.ProcedureName, this._data.ArtikelNummer);
