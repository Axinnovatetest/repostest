...
{
	public class UpdateWarehouseNotesHandler: IHandle<UserModel, ResponseModel<int>>
	{
		private readonly UserModel _user;
		private readonly Models.InverntoryStockModels.UpdateNotesRequestModel _data;
		public UpdateWarehouseNotesHandler(UserModel user, Models.InverntoryStockModels.UpdateNotesRequestModel data)
		{
			_user = user;
			_data = data;
		}
		...
		public ResponseModel<int> Handle()
		{
			...
			try
			{
				...
				var id = Infrastructure.Data.Access.Tables.Logistics.InventoryStock.InventoryFaStatsAccess.UpdateWarehouseNotesInventory(_data.WarehouseId, _data.RemarksHL, _data.RemarksPL, botransaction.connection, botransaction.transaction);
				...
				Infrastructure.Data.Access.Tables.Logistics.InventoryStock.LogsAccess.InsertWithTransaction(new LogsEntity
				{
					...
					LogDescription = $"The inventory stock [Plant] notes have been updated in Lager [{_data.WarehouseId}] at [{DateTime.Now:yyyy-MM-dd HH:mm}] by [{_user.Name}]",
					LogsType = 2,
