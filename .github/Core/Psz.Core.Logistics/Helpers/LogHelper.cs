...
namespace Psz.Core.Logistics.Helpers
{
	...
	public class PlantBookingLogHelper
	{
			...
			)
		{
			...
			return new Infrastructure.Data.Entities.Tables.Logistics.PlantBookingsLogsEntity()
			{
				...
				LogDescription = $"The Object with Nummer Verpackung [{Nummer_Verpackung}] has been deleted From Lager [{LagerId}] "
			};
		}
			...
			)
		{
			...
			return new Infrastructure.Data.Entities.Tables.Logistics.PlantBookingsLogsEntity()
			{
				...
				LogObjectId = Nummer_Verpackung,
				LogDescription = $"Lager [{LagerId}]: The Object with Nummer Verpackung [{Nummer_Verpackung}]" + AdditionalLogTest
			};
	...
	public class InventoryStockLogHelper
	{
		#region Logs For update task role inventory stock

		public static List<LogsEntity> GenerateLogForUpdateIstTaskStatus(
												Identity.Models.UserModel _user,
												TaskByRoleEntity CurrentValue,
												UpdateTaskRequestModel newValues
												)
		{
			var restoReturn = new List<Infrastructure.Data.Entities.Tables.Logistics.InventroyStock.LogsEntity>();
			if(newValues?.Id != null && CurrentValue.status != newValues?.status)
			{
				restoReturn.Add(
					new Infrastructure.Data.Entities.Tables.Logistics.InventroyStock.LogsEntity()
					{
						LogTime = DateTime.Now,
						LogUserId = _user.Id,
						ObjectId = CurrentValue.Id,
						ObjectName = $"InventoryStock",
						LogDescription = $"The Task Status With name [{CurrentValue.title}] has been updated from [{CurrentValue.status}] to [{newValues.status}]",
						LogsType = 2,
						LogUserName = _user.Name,
						LagerId = CurrentValue.lagerId ?? 0
					});
			}
			return restoReturn;
		}

		#endregion Logs For update task role inventory stock
		public static List<LogsEntity> GenerateLogForStartInventory(
			Identity.Models.UserModel user,
			int lagerId
			)
		{
			var logs = new List<LogsEntity>();

			if(lagerId > 0)
			{
				logs.Add(new LogsEntity
				{
					LogTime = DateTime.Now,
					LogUserId = user.Id,
					ObjectId = lagerId,
					ObjectName = "InventoryStock",
					LogDescription = $"The inventory stock has been started in Lager [{lagerId}] at [{DateTime.Now:yyyy-MM-dd HH:mm}]",
					LogsType = 2,
