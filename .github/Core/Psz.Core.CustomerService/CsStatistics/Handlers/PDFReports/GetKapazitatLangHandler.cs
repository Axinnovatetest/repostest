...
{
	public class GetKapazitatLangHandler: IHandle<Identity.Models.UserModel, ResponseModel<byte[]>>
	{
		...
		public ResponseModel<byte[]> Handle()
		{
			try
			{
				...
				responseBody = Module.CS_ReportingService.GenerateKapazitatLangReport(Enums.ReportingEnums.ReportType.CTS_KAPAZITATLANG, reportData);
		...
		public ResponseModel<byte[]> Validate()
		{
			...
			if(_data.AT_PD.HasValue && _data.AT_PD <= 0)
				return ResponseModel<byte[]>.FailureResponse($"invalid work days");
