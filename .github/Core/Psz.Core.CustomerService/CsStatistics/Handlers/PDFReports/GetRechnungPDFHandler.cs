	...
	public class GetRechnungPDFHandler: IHandleAsync<Identity.Models.UserModel, ResponseModel<byte[]>>
	{
		...
		public async Task<ResponseModel<byte[]>> HandleAsync()
		{
			try
			{
				...
				byte[] response = null;
				var ReportData = new Infrastructure.Services.Reporting.Models.CTS.RechnungModel();
				...
				if(rechnungEntity != null)
				{
					...
					ReportData = new Infrastructure.Services.Reporting.Models.CTS.RechnungModel
					{
						ReportParameters = new Infrastructure.Services.Reporting.Models.CTS.RechnungHeaderModel(RechnungReportParametersEntity, new Infrastructure.Services.Reporting.Models.CTS.RechnungEntryModel
						{
							From = _data.From,
							To = _data.To,
							Lager = _data.Lager,
						...
						Zollatarif = _zollatarif.OrderBy(s => s).Select(z => new Infrastructure.Services.Reporting.Models.CTS.RechnungGroupedZollaTarif
						{
							Zolltarif_nummer = z,
				...
				var Details = rechnungEntity?.Select(r => new Infrastructure.Services.Reporting.Models.CTS.RechnungDetailsModel(r)).ToList()
					?? new List<Infrastructure.Services.Reporting.Models.CTS.RechnungDetailsModel>();
				...
				var footerData = new Infrastructure.Services.Reporting.Models.Itext.CTS_StatsRG_DocFooterModel(reportEntity, _data.Lager);
				response = await Infrastructure.Services.Reporting.IText.CTS.GetCTSRG(ReportData, footerData);
		...
		public static List<Infrastructure.Services.Reporting.Models.CTS.RechnungDetailsModel> SetRepeated(List<Infrastructure.Services.Reporting.Models.CTS.RechnungDetailsModel> data)
		{
			var result = new List<Infrastructure.Services.Reporting.Models.CTS.RechnungDetailsModel>();
			var FAList = data.Select(f => f.Fertigungsnummer).ToList();
			var _repeatedFA = FAList.GroupBy(x => x)
			   .Where(g => g.Count() > 1)
			   .Select(y => y.Key)
			   .ToList();
			result = data.Select(d => new Infrastructure.Services.Reporting.Models.CTS.RechnungDetailsModel
			{
				Ausdr3 = d.Ausdr3,
				Datum = d.Datum,
