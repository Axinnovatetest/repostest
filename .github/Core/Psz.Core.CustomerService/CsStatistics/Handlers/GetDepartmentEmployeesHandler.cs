...
{
	public class GetDepartmentEmployeesHandler: IHandle<Identity.Models.UserModel, ResponseModel<List<KeyValuePair<string, string>>>>
	{
		private Identity.Models.UserModel _user { get; set; }
		public GetDepartmentEmployeesHandler(Identity.Models.UserModel user)
		{
			this._user = user;
		}
		public ResponseModel<List<KeyValuePair<string, string>>> Handle()
		{
			try
			{
				...
				if(EmplyeesEntity != null && EmplyeesEntity.Count > 0)
					resposne = EmplyeesEntity.Select(e => new KeyValuePair<string, string>(e.AV_Mitarbeiter, e.EMAIL)).ToList();
