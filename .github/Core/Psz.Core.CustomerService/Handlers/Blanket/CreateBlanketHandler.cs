...
namespace Psz.Core.CustomerService.Handlers.Blanket
{
	public class CreateBlanketHandler: IHandle<Identity.Models.UserModel, ResponseModel<int>>
	{
		...
		public ResponseModel<int> Handle()
		{
			...
			//var MaxCurrentValue = Module.CTS.raMaxCurrentValue;
			//var MinNewValue = Module.CTS.raMinNewValue;
			...
			//Infrastructure.Data.Access.Tables.PRS.AngeboteAccess.GetMaxAngebotNrByTypeAndSettingsValues(Infrastructure.Data.Access.Tables.PRS.AngeboteAccess.TYP_CONTRACT, MaxCurrentValue, MinNewValue);
			...
			lock(Locks.Locks.OrdersLock)
			{
				...
				try
				{
					...
					if(_data.BlanketTypeId == (int)Enums.BlanketEnums.Types.sale)
					{
						...
						{
							Bezug = string.IsNullOrEmpty(_data.DocumentCustomer) || string.IsNullOrWhiteSpace(_data.DocumentCustomer) ? getUniqueDocumentName(customerNummer ?? -1, Infrastructure.Data.Access.Tables.PRS.AngeboteAccess.TYP_CONTRACT) : _data.DocumentCustomer,
							EDI_Dateiname_CSV = "",
						...
						// >>>>>> Logging
						Infrastructure.Services.Logging.Logger.Log(new Exception($" OrderImport[Purchase] >>>>>> insert orderDb:{JsonConvert.SerializeObject(orderDb)} "));
		...
		public ResponseModel<int> Validate()
		{
			...
			if(_data.BlanketTypeId == (int)Enums.BlanketEnums.Types.purchase)
			{
				...
				if(!string.IsNullOrEmpty(_data.DocumentCustomer) && !string.IsNullOrWhiteSpace(_data.DocumentCustomer))
				{
					...
					if(_existingDocSameNumber != null && _existingDocSameNumber.Count > 0)
						return ResponseModel<int>.FailureResponse(key: "1", value: $"Document Already Exist !");
				}
			}
		...
		{
			var orderDb = Infrastructure.Data.Access.Tables.PRS.AngeboteAccess.GetTopRABezug();
			if(orderDb == null)
			{
				return MANUAL_DOCUMENT_PREFIX + "1";
			}
			// - Same processing for now, but need to allwo same DocNr, if types different (Sale != Purchase).
			if(type == Enums.BlanketEnums.Types.purchase)
			{
