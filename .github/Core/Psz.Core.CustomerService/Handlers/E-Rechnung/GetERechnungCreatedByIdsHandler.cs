...
{
	public class GetERechnungCreatedByIdsHandler: IHandle<Identity.Models.UserModel, ResponseModel<E_RechnungCreatedResponseModel>>
	{
		private Identity.Models.UserModel _user { get; set; }
		private List<int> _data { get; set; }
		public GetERechnungCreatedByIdsHandler(Identity.Models.UserModel user, List<int> data)
		{
			this._user = user;
			this._data = data;
		}
		public ResponseModel<E_RechnungCreatedResponseModel> Handle()
		{
			try
			{
				...
				var invoices = Infrastructure.Data.Access.Tables.CTS.__E_rechnung_CreatedAccess.GetE_RechnungByNr(this._data);
				var angebotNrs = invoices?.Select(x => x.RechnungForfallNr).Distinct().ToList();
				if(angebotNrs != null && angebotNrs.Count > 0)
				{
					angebotNrs.ForEach(x =>
					{
