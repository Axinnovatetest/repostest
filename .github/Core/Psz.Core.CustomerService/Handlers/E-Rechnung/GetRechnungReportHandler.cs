		...
		public ResponseModel<byte[]> Handle()
		{
			...
			try
			{
				...
				if(languageEntities != null || languageEntities.Count > 0)
				{
					...
					// - Ticket #46614 - if language not found set to default german
				...
				for(int i = 0; i < orders.Count; i++)
				{
					...
					for(int j = 0; j < orderItems.Count; j++)
					{
						...
						invoiceItemData.Add(new RechnungReportingItemModel()
						{
							...
							DelFixedText = orderItems[j].DELFixiert.HasValue && orderItems[j].DELFixiert.Value ? "DEL fixiert laut Angebot" : null,
