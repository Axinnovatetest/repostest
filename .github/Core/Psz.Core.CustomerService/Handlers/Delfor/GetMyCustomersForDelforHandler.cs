	...
	public class GetMyCustomersForDelforHandler: IHandle<Core.Identity.Models.UserModel, ResponseModel<List<Psz.Core.CustomerService.Models.OrderProcessing.CustomerModel>>>
	{
		...
		public ResponseModel<List<Psz.Core.CustomerService.Models.OrderProcessing.CustomerModel>> Handle()
		{
			...
			try
			{
				...
				{
					return ResponseModel<List<Psz.Core.CustomerService.Models.OrderProcessing.CustomerModel>>.SuccessResponse(Get(Infrastructure.Data.Access.Tables.PRS.KundenAccess.Get()?.Where(x => archivedCustomerIds?.Contains(x.Nr) == false)?.ToList(), _data));
				...
				return ResponseModel<List<Psz.Core.CustomerService.Models.OrderProcessing.CustomerModel>>.SuccessResponse(Get(Infrastructure.Data.Access.Tables.PRS.KundenAccess.GetByNummers(customersNumbers)?.Where(x => archivedCustomerIds?.Contains(x.Nr) == false)?.ToList(), _data));
