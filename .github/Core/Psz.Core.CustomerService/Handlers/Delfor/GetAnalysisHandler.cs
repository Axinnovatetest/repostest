	...
	public class GetAnalysisHandler: IHandleAsync<Identity.Models.UserModel, ResponseModel<AnalysisResponseModel>>
	{
		...
		public async Task<ResponseModel<AnalysisResponseModel>> HandleAsync()
		{
			try
			{
				...
				var results = Infrastructure.Data.Access.Joins.CTS.DelforStatistics.GetAnalysis(this._data.IsManual, this._data.CustomerNumber, this._data.DocumentNumber, this._data.OnlyLastVersion ?? true, this._data.OnlyOpen ?? true, this._data.DateFrom, this._data.DateTo, sortFieldName, this._data.SortDesc, this._data.RequestedPage, this._data.PageSize);
				var allCount = await Infrastructure.Data.Access.Joins.CTS.DelforStatistics.GetAnalysis_count(this._data.IsManual, this._data.CustomerNumber, this._data.DocumentNumber, this._data.OnlyLastVersion ?? true, this._data.OnlyOpen ?? true, this._data.DateFrom, this._data.DateTo);
				//- 
		...
		private static void AddWorkSheet(List<AnalysisResponseModel.AnalysisItem> items, string customerName, ExcelPackage package)
		{
			...
			worksheet.Cells[headerRowNumber, startColumnNumber + 19].Value = "Lieferadresse";
			...
			if(items != null && items.Count > 0)
			{
				...
				foreach(var w in items)
				{
					...
					worksheet.Cells[rowNumber, startColumnNumber + 19].Value = w?.Lieferadresse;
