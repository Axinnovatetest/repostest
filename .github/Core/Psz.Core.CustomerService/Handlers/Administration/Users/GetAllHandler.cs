...
namespace Psz.Core.CustomerService.Handlers.Administration.Users
{
	public class GetAllHandler: IHandle<Identity.Models.UserModel, ResponseModel<List<Models.Administration.Users.GetModel>>>
	{
		...
		public ResponseModel<List<Models.Administration.Users.GetModel>> Handle()
		{
			try
			{
				...
				var uprofileEntities = Infrastructure.Data.Access.Tables.CTS.AccessProfileUsersAccess.GetByUserId(userEntities?.Select(x => x.Id)?.ToList());
				var profileEntites = Infrastructure.Data.Access.Tables.CTS.AccessProfileAccess.Get(uprofileEntities?.Select(x => x.AccessProfileId)?.ToList())
