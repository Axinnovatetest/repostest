...
namespace Psz.Core.CustomerService.Handlers.InsideSales
{
	...
	{
		public ResponseModel<INSOverviewVK_Summe_ungebuchte_ABDetailsResponseModel> Get_INSOverviewVK_Summe_ungebuchte_ABDetails(UserModel user, INSOverviewVK_Summe_ungebuchte_ABDetailsRequestModel data)
		{
			if(user == null)
				return ResponseModel<INSOverviewVK_Summe_ungebuchte_ABDetailsResponseModel>.AccessDeniedResponse();
			...
			try
			{
				...
				int? userId = user.IsAdministrator || user.IsCorporateDirector || user.IsGlobalDirector || user.Access.Purchase.AllCustomers
						? null
						: user.Id;
				var entities = Infrastructure.Data.Access.Joins.CTS.INSOverviewAccess.Get_INSOverviewVK_Summe_ungebuchte_ABDetails(date, week, year, data.OlderDate, data.SearchText, data.CustomerNumber, data.userId, dataPaging, dataSorting, userId);
				var count = Infrastructure.Data.Access.Joins.CTS.INSOverviewAccess.Get_INSOverviewVK_Summe_ungebuchte_ABDetails_Count(date, week, year, data.OlderDate, data.CustomerNumber, data.userId, userId, data.SearchText);

				var response = new INSOverviewVK_Summe_ungebuchte_ABDetailsResponseModel
				{
					Items = entities?.Select(e => new INSOverviewVK_Summe_ungebuchte_ABDetailsModel(e)).ToList(),
				...
				return ResponseModel<INSOverviewVK_Summe_ungebuchte_ABDetailsResponseModel>.SuccessResponse(response);
