...
{
	public class GetDeliveryNotesCompilationHandler: IHandle<Identity.Models.UserModel, ResponseModel<DeliveryNotesCompilationResponseModel>>
	{
		...
		public ResponseModel<DeliveryNotesCompilationResponseModel> Handle()
		{
		...
		public ResponseModel<DeliveryNotesCompilationResponseModel> Validate()
		{
			if(_user == null || (!_user.Access.CustomerService.ModuleActivated && !_user.Access.Purchase.ModuleActivated))
			{
				return ResponseModel<DeliveryNotesCompilationResponseModel>.AccessDeniedResponse();
			}
		...
		public ResponseModel<byte[]> GetPDF()
		{
			...
			// -
			return ResponseModel<byte[]>.SuccessResponse(Infrastructure.Services.Reporting.IText.CTS.GetPDF(new Infrastructure.Services.Reporting.Models.Itext.DeliveryNoteModel
			{
				...
				VAT_ID = response.Body.VAT_ID
								...
								response.Body.Items?.Select(x => new Infrastructure.Services.Reporting.Models.Itext.DeliveryNoteModel.DeliveryNoteItemModel
								{
