...
namespace Psz.Api.Areas.WorkPlan.Controllers
{
	...
	{
		private const string MODULE = "WorkPlan";
		...
		public IActionResult GetByHallArticle(int HallId, int ArticleId)
		{
			...
			try
			{
				...
				{
					workScheduleDb.ForEach(wsdb => new List<Psz.Core.Apps.WorkPlan.Models.WorkPlan.WorkScheduleViewModel>() { new Psz.Core.Apps.WorkPlan.Models.WorkPlan.WorkScheduleViewModel(wsdb) });
				}

				if(errors.Count == 0)
				{
					return Ok(new { response = new Api.Models.Response<List<Psz.Core.Apps.WorkPlan.Models.WorkPlan.WorkScheduleViewModel>>() { Success = true, ResponseBody = new List<Psz.Core.Apps.WorkPlan.Models.WorkPlan.WorkScheduleViewModel>(), Errors = errors } });
				}
				else
				{
		...
		public IActionResult Get(int Id)
		{
			...
			try
			{
				...
				{
					var workScheduleVM = new Psz.Core.Apps.WorkPlan.Models.WorkPlan.WorkScheduleViewModel(workScheduleDb);

					return Ok(new { response = new Api.Models.Response<Psz.Core.Apps.WorkPlan.Models.WorkPlan.WorkScheduleViewModel>() { Success = true, ResponseBody = workScheduleVM, Errors = errors } });
				}
		...
		[HttpPost]
		[SwaggerOperation(Tags = new[] { "WorkPlan" })]
		public IActionResult Add(Psz.Core.Apps.WorkPlan.Models.WorkPlan.WorkScheduleViewModel data)
		{
			var errors = new List<string>();
			try
			{
				...
				if(user == null)
				{
					return Ok(new
					{
						response = new Api.Models.Response<string>
						{
							Success = false,
				...
				var articleDb = Infrastructure.Data.Access.Tables.WPL.ArticleAccess.Get(data.ArticleId);
				...
				Helpers.Article.EditArticle(user, data.ArticleId);
				...
				Helpers.Log.NewLog(Core.Apps.WorkPlan.Enums.LogEnums.LogType.WorkSchedule,
					$"{user.Username} Added a Work Schedule  AP {name} for {hallDb?.Name} and {articleDb?.Name}",
					user.Id);
		...
		[HttpPost]
		[SwaggerOperation(Tags = new[] { "WorkPlan" })]
		public IActionResult Edit(Psz.Core.Apps.WorkPlan.Models.WorkPlan.WorkScheduleViewModel model)
		{
			var errors = new List<string>();
			try
			{
				...
				else
				{
					if(model.IsActive == true)
					{
						...
						if(errors.Count == 0)
						{
							Helpers.Article.EditArticle(user, model.ArticleId);
		...
		public IActionResult ActiveState(int WorkScheduleId)
		{
			...
			try
			{
				...
				else
				{
					...
					return Ok(new { response = new Api.Models.Response<Boolean>() { Success = true, ResponseBody = workScheduleDb.IsActive, Errors = errors } });
				}
			...
			try
			{
				...
				else
				{
					...
					if(IsActive == 1)
					{
						...
						if(errors.Count == 0)
						{
							Helpers.Article.EditArticle(user, workScheduleDb.ArticleId);
							workScheduleDb.IsActive = true;
							Infrastructure.Data.Access.Tables.WPL.WorkPlanAccess.Update(workScheduleDb);
						}
					}
		...
		public IActionResult Delete(int Id)
		{
			...
			try
			{
				...
				else
				{
					...
					if(Helpers.Article.EditArticle(user, workScheduleDb.ArticleId))
				...
				if(errors.Count == 0)
				{
					...
					Helpers.Log.NewLog(Core.Apps.WorkPlan.Enums.LogEnums.LogType.WorkSchedule,
						$"{user.Username} deleted a Work Schedule {workScheduleDb.Name} for {hallDb.Name} and {articleDb.Name}.",
						user.Id);
					return Ok(new { response = new Api.Models.Response<string>(true, $"Edit Success. Log Add success", errors) });
				}
				else
		...
		public IActionResult GetByCountryIdArticleId(int CountryId, int ArticleId)
		{
			...
			try
			{
				...
				{
					var workScheduleVM = new List<Psz.Core.Apps.WorkPlan.Models.WorkPlan.WorkScheduleViewModel>();
					workScheduleDb.ForEach(wsdb => workScheduleVM.Add(new Psz.Core.Apps.WorkPlan.Models.WorkPlan.WorkScheduleViewModel(wsdb)));

					return Ok(new { response = new Api.Models.Response<List<Psz.Core.Apps.WorkPlan.Models.WorkPlan.WorkScheduleViewModel>>() { Success = true, ResponseBody = workScheduleVM, Errors = errors } });
				}
		...
		public ActionResult ExportScheduleToExcel(int Id, string lang)
		{
			try
			{
				var fileBytesResponse = Psz.Core.Apps.WorkPlan.Handlers.WorkSchedule.ExportToExcel(Id, lang/* this.GetCurrentUser()?.SelectedLanguage*/);
		...
		public ActionResult ExportScheduleFAToExcel(int Id, string lang, int faQuantity)
		{
			try
			{
				var fileBytesResponse = Psz.Core.Apps.WorkPlan.Handlers.WorkSchedule.ExportToExcel(Id, lang, faQuantity/* this.GetCurrentUser()?.SelectedLanguage*/);
				if(fileBytesResponse.Success)
				{
					return File(fileBytesResponse.Body, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
						, $"WorkSchedule-{DateTime.Now.ToString("yyyy-MM-ddTHHmmss.fff")}.xlsx");
		...
		public IActionResult ImportScheduleFromExcel([FromForm] Models.WorkSchedule.ImportExcelModel model)
		{
			try
			{
				...
				if(file.Length > 0)
				{
					...
					return Ok(Psz.Core.Apps.WorkPlan.Handlers.WorkSchedule.ExtractFromExcel(filePath, model.Id, user));
				}
				else
				{
		...
		public ActionResult ExportScheduleAllToExcel()
		{
			try
			{
				...
				if(wsd != null)
				{
					...
					{
						var fileBytesResponse = Psz.Core.Apps.WorkPlan.Handlers.WorkSchedule.ExportToExcel(item.Id, this.GetCurrentUser()?.SelectedLanguage);
						if(fileBytesResponse.Success)
						{
					...
					if(files.Count > 0)
					{
						foreach(var file in files)
						{
							...
							using(var zip = System.IO.Compression.ZipFile.Open(zipFilePath, System.IO.Compression.ZipArchiveMode.Update))
							{
								// Add the entry for each file
								zip.CreateEntryFromFile(file, System.IO.Path.GetFileName(file), System.IO.Compression.CompressionLevel.Optimal);
							}
						}
					}
		...
		[HttpPost]
		[SwaggerOperation(Tags = new[] { "WorkPlan" })]
		[ProducesResponseType(typeof(Core.Models.ResponseModel<Psz.Core.Apps.WorkPlan.Models.WorkSchedule.GetSimulationResponseModel>), 200)]
		public IActionResult GetSimulation(Psz.Core.Apps.WorkPlan.Models.WorkSchedule.GetSimulationRequestModel data)
		{
			try
			{
		...
		[ProducesResponseType(typeof(Core.Common.Models.ResponseModel<List<Psz.Core.Apps.WorkPlan.Models.WorkSchedule.ArticlesWoWPLModel>>), 200)]
		public ActionResult GetArticleswoWPL(Psz.Core.Apps.WorkPlan.Models.WorkSchedule.ArticlesWoWorkPlanRequestModel data)
		{
			try
			{
		...
		[HttpPost]
		[SwaggerOperation(Tags = new[] { MODULE })]
		public ActionResult ExportArticleswoWPL(Psz.Core.Apps.WorkPlan.Models.WorkSchedule.ArticlesWoWorkPlanRequestModel data)
		{
			try
			{
		...
		[HttpGet]
		[SwaggerOperation(Tags = new[] { MODULE })]
		[ProducesResponseType(typeof(Core.Common.Models.ResponseModel<List<Psz.Core.Apps.WorkPlan.Models.WorkSchedule.ArticleWPLTimeDiffModel>>), 200)]
		...
		[HttpGet]
		[SwaggerOperation(Tags = new[] { MODULE })]
		[ProducesResponseType(typeof(Core.Common.Models.ResponseModel<List<Psz.Core.Apps.WorkPlan.Models.WorkSchedule.ArticleFaTimeDiffModel>>), 200)]
		...
		[HttpGet]
		[SwaggerOperation(Tags = new[] { MODULE })]
		[ProducesResponseType(typeof(Core.Common.Models.ResponseModel<List<Psz.Core.Apps.WorkPlan.Models.WorkSchedule.CountryModel>>), 200)]
