...
namespace Psz.Api.Areas.EDI.Controllers
{
	...
	public class DelforController: ControllerBase
	{
		...
		public IActionResult GetForecastPDFByLineItem(int id)
		{
			try
			{
				var response = new Core.Apps.EDI.Handlers.Delfor.GetForecastPDFHandler(this.GetCurrentUser(), id).Handle();
				if(response.Success)
				{
