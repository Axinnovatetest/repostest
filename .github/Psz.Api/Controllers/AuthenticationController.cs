...
namespace Psz.Api.Controllers
{
	...
	public class AuthenticationController: ControllerBase
	{
		...
		[HttpPost]
		[SwaggerOperation(Tags = new[] { "No Area" })]
		public IActionResult RefreshToken([FromBody] Models.RefreshTokenRequestModel data)
		{
			try
			{
				if(data == null || string.IsNullOrWhiteSpace(data.Token) || string.IsNullOrWhiteSpace(data.Password))
				{
					return BadRequest(new { response = new Models.Response<object> { Success = false, Errors = new List<string> { "Token and password are required." } } });
				}

				string decodedPassword = Base64Decode(data.Password);
				var user = Core.Apps.Main.Handlers.Security.TokensManager.getUserByToken(data.Token);
				...
				else
				{
					return Ok(new
					{
						...
						{
							Success = true,
							ResponseBody = Core.Apps.Main.Handlers.Security.TokensManager.GenerateRefreshToken(data.Token),
						}
					});
				}
