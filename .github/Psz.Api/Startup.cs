...
namespace Psz.Api
{
	public class Startup
	{
		...
		public Startup(IConfiguration configuration, IWebHostEnvironment environment)
		{
			...
			Infrastructure.Services.Module.Initiate(Configuration["FilesPath"], tempFolderPath, Configuration.GetSection("Email").Get<Infrastructure.Services.Email.EmailParamtersModel>(),
				ctsData.FAHorizons.H1LengthInDays,
				impersonateData.ImpersonateUsername, impersonateData.ImpersonatePassword, impersonateData.ImpersonateDomain);
		}
		...
		public void ConfigureServices(IServiceCollection services)
		{
			...
			services.AddSingleton<Infrastructure.Services.Files.Parsing.ILogParser>(x => new Infrastructure.Services.Files.Parsing.LogParser(Configuration["LogFolderPath"], Configuration.GetSection("ExcludeUserIds").Get<List<int>>(), Configuration.GetSection("Email").Get<Infrastructure.Services.Email.EmailParamtersModel>()));
		...
		public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
		{
			...
			// - Reloads for new Front version (index.html, not the js builds)
			...
			var options = new DashboardOptions
			{
				...
				//		{
				//			Login = hgSettings?.User ?? "",
				//			PasswordClear =  hgSettings?.Password?? ""
				//		}
				//	}
				//}) }
			};
